...
#define TIZEN_DISABLE_SSL3_FOR_POODLE_ATTACK
...
g_tls_connection_gnutls_init_priorities (void)
{
  ...
  if (!base_priority)
#ifdef TIZEN_DISABLE_SSL3_FOR_POODLE_ATTACK
...
#ifdef TIZEN_DISABLE_SSL3_FOR_POODLE_ATTACK
  // SSL3 is disabled now, In fallback case use VERS-TLS1.0
  ssl3_priority = g_strdup_printf ("%s:!VERS-TLS1.2:!VERS-TLS1.1", base_priority);
#else
...
#ifdef TIZEN_DISABLE_SSL3_FOR_POODLE_ATTACK
  // SSL3 is disabled now, In fallback case use VERS-TLS1.0
  ssl3_unsafe_rehandshake_priority = g_strdup_printf ("%s:!VERS-TLS1.2:!VERS-TLS1.1:%%UNSAFE_RENEGOTIATION", base_priority);
#else
...
{
#if ENABLE_TIZEN_REORDER_CERTIFICATE_CHAIN
  GTlsCertificate **cert, *tmp, *chain;
  int j;
#else
...
#if ENABLE_TIZEN_REORDER_CERTIFICATE_CHAIN
  cert = g_new (GTlsCertificate *, num_certs);
  //Assign NULL issuer for every certificate
  for (i = num_certs - 1; i >= 0; i--)
    {
      cert[i] = g_tls_certificate_gnutls_new (&certs[i], NULL);
    }
  //Reorder the certificates
  for (i = 0; i < num_certs; i++)
    {
      for (j = i + 1; j < num_certs; j++)
        {
          if (gnutls_x509_crt_check_issuer (g_tls_certificate_gnutls_get_cert (G_TLS_CERTIFICATE_GNUTLS (cert[i])),
					g_tls_certificate_gnutls_get_cert (G_TLS_CERTIFICATE_GNUTLS(cert[j]))))
           {
             //cert[j] is an issuer for cert[i]. so swap cert[j] and cert[i + 1]
             if (j != i + 1)
              {
                 tmp = cert[j];
                 cert[j] = cert[i + 1];
            ...
            g_tls_certificate_gnutls_add_issuer (G_TLS_CERTIFICATE_GNUTLS (cert[i]), G_TLS_CERTIFICATE_GNUTLS (cert[i + 1]));
            break;
           }
        }
        //No match found so break the chain here.
        if (j == num_certs)
          break;
    }
#else
