...
#ifdef RIFFMEDIA_MODIFICATION
        if (strf->av_bps > strf->blockalign * strf->rate) {
          GST_WARNING ("fixing av_bps (%d) to (%d) of alaw audio",
              strf->av_bps,strf->blockalign * strf->rate);
          strf->av_bps = strf->blockalign * strf->rate;
        }
#endif
...
#ifdef RIFFMEDIA_MODIFICATION
        if (strf->av_bps > strf->blockalign * strf->rate) {
          GST_WARNING ("fixing av_bps (%d) to (%d) of mulaw audio",
              strf->av_bps,strf->blockalign * strf->rate);
          strf->av_bps = strf->blockalign * strf->rate;
        }
#endif
