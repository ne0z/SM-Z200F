...
#ifdef GST_EXT_ID3_MODIFICATION
      if ((encoding != ID3V2_ENCODING_UTF16) && (encoding != ID3V2_ENCODING_UTF16BE)) {
        gint string_len = (gint)strlen(g_array_index (fields, gchar *, 0));
        gint null_index = work->parse_size - 1;
        gint i = 0;

        for (i = 0; i < (work->parse_size - 1); i++)
        {
          if ((g_array_index (fields, gchar *, 0))[i] == '\0') {
            null_index = i;
            GST_INFO ("string terminator(NULL) is detected when index is %d", i);
          }
        }

        /* Concern the specific case : last character of the string is NULL. (www.djpunjab.com) */
        if ((work->parse_size - 1 != string_len) && (null_index != string_len)) {
          GST_WARNING ("size of text ID frame is %d. but real size is %d", work->parse_size - 1, string_len);
          fields = NULL;
          parse_split_strings (ID3V2_ENCODING_UTF16, (gchar *) work->parse_data + 1,
              work->parse_size - 1, &fields);

          if (fields) {
            if (fields->len > 0) {
              GST_WARNING ("Read %d fields again from Text ID frame of size %d with encoding %d"
                ". First is '%s'", fields->len, work->parse_size - 1, ID3V2_ENCODING_UTF16,
                g_array_index (fields, gchar *, 0));
            }
          }
        } else {
          GST_LOG ("Not read fields from Text ID frame of size %d with encoding %d",
              work->parse_size - 1, ID3V2_ENCODING_UTF16);
        }
      }
#endif
    } else {
