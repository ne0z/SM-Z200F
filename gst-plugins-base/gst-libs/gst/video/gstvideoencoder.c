...
#ifdef GST_EXT_VIDEOENCODER
  gint input_log_count;
  gint output_log_count;
#endif
...
forced_key_unit_event_new (GstClockTime running_time, gboolean all_headers, guint count)
{
...
static void gst_video_encoder_init (GstVideoEncoder * enc, GstVideoEncoderClass * klass);

static void gst_video_encoder_finalize (GObject * object);

static gboolean gst_video_encoder_setcaps (GstVideoEncoder * enc, GstCaps * caps);
static GstCaps *gst_video_encoder_sink_getcaps (GstVideoEncoder * encoder, GstCaps * filter);
static gboolean gst_video_encoder_src_event (GstPad * pad, GstObject * parent, GstEvent * event);
static gboolean gst_video_encoder_sink_event (GstPad * pad, GstObject * parent, GstEvent * event);
static GstFlowReturn gst_video_encoder_chain (GstPad * pad, GstObject * parent, GstBuffer * buf);
static GstStateChangeReturn gst_video_encoder_change_state (GstElement * element, GstStateChange transition);
static gboolean gst_video_encoder_sink_query (GstPad * pad, GstObject * parent, GstQuery * query);
static gboolean gst_video_encoder_src_query (GstPad * pad, GstObject * parent, GstQuery * query);
static GstVideoCodecFrame *gst_video_encoder_new_frame (GstVideoEncoder * encoder, GstBuffer * buf, GstClockTime pts, GstClockTime dts, GstClockTime duration);

static gboolean gst_video_encoder_sink_event_default (GstVideoEncoder * encoder, GstEvent * event);
static gboolean gst_video_encoder_src_event_default (GstVideoEncoder * encoder, GstEvent * event);
static gboolean gst_video_encoder_decide_allocation_default (GstVideoEncoder * encoder, GstQuery * query);
static gboolean gst_video_encoder_propose_allocation_default (GstVideoEncoder * encoder, GstQuery * query);
static gboolean gst_video_encoder_negotiate_default (GstVideoEncoder * encoder);
static gboolean gst_video_encoder_negotiate_unlocked (GstVideoEncoder * encoder);

static gboolean gst_video_encoder_sink_query_default (GstVideoEncoder * encoder, GstQuery * query);
static gboolean gst_video_encoder_src_query_default (GstVideoEncoder * encoder, GstQuery * query);
...
gst_video_encoder_get_type (void)
{
  ...
  if (g_once_init_enter (&type)) {
    ...
    _type = g_type_register_static (GST_TYPE_ELEMENT, "GstVideoEncoder", &info, G_TYPE_FLAG_ABSTRACT);
    g_type_add_interface_static (_type, GST_TYPE_PRESET, &preset_interface_info);
...
gst_video_encoder_class_init (GstVideoEncoderClass * klass)
{
  ...
  GST_DEBUG_CATEGORY_INIT (videoencoder_debug, "videoencoder", 0, "Base Video Encoder");
  ...
  gstelement_class->change_state = GST_DEBUG_FUNCPTR (gst_video_encoder_change_state);
...
gst_video_encoder_reset (GstVideoEncoder * encoder, gboolean hard)
{
  ...
  g_list_foreach (priv->force_key_unit, (GFunc) forced_key_unit_event_free, NULL);
  ...
  if (hard) {
    ...
    priv->current_frame_events = _flush_events (encoder->srcpad, encoder->priv->current_frame_events);
  }

#ifdef GST_EXT_VIDEOENCODER
  priv->input_log_count = 0;
  priv->output_log_count = 0;
#endif
...
gst_video_encoder_init (GstVideoEncoder * encoder, GstVideoEncoderClass * klass)
{
  ...
  gst_pad_set_event_function (pad, GST_DEBUG_FUNCPTR (gst_video_encoder_sink_event));
  gst_pad_set_query_function (pad, GST_DEBUG_FUNCPTR (gst_video_encoder_sink_query));
  ...
  gst_pad_set_query_function (pad, GST_DEBUG_FUNCPTR (gst_video_encoder_src_query));
  gst_pad_set_event_function (pad, GST_DEBUG_FUNCPTR (gst_video_encoder_src_event));
    ...
    gint64 * dest_value)
{
  ...
  if (G_UNLIKELY (src_format == *dest_format || src_value == 0 || src_value == -1)) {
    if (dest_value)
...
gst_video_encoder_set_headers (GstVideoEncoder * video_encoder, GList * headers)
{
  ...
  if (video_encoder->priv->headers) {
    g_list_foreach (video_encoder->priv->headers, (GFunc) gst_buffer_unref, NULL);
...
gst_video_encoder_setcaps (GstVideoEncoder * encoder, GstCaps * caps)
{
  ...
  if (encoder->priv->input_state) {
    GST_DEBUG_OBJECT (encoder, "Checking if caps changed old %" GST_PTR_FORMAT " new %" GST_PTR_FORMAT, encoder->priv->input_state->caps, caps);
  ...
  if (encoder->priv->input_state && gst_video_info_is_equal (&state->info, &encoder->priv->input_state->info)) {
 ...
 */
GstCaps *
gst_video_encoder_proxy_getcaps (GstVideoEncoder * encoder, GstCaps * caps, GstCaps * filter)
{
  GstCaps *templ_caps;
  GstCaps *allowed;
   ...
   */
  templ_caps = caps ? gst_caps_ref (caps) : gst_pad_get_pad_template_caps (encoder->sinkpad);
...
static gboolean
gst_video_encoder_decide_allocation_default (GstVideoEncoder * encoder, GstQuery * query)
{
...
static gboolean
gst_video_encoder_propose_allocation_default (GstVideoEncoder * encoder, GstQuery * query)
{
  GstCaps *caps;
  GstVideoInfo info;
  GstBufferPool *pool;
...
static gboolean
gst_video_encoder_sink_query_default (GstVideoEncoder * encoder, GstQuery * query)
{
...
static gboolean
gst_video_encoder_sink_query (GstPad * pad, GstObject * parent, GstQuery * query)
{
...
gst_video_encoder_check_and_push_tags (GstVideoEncoder * encoder)
{
  if (encoder->priv->tags && encoder->priv->tags_changed) {
    gst_video_encoder_push_event (encoder, gst_event_new_tag (gst_tag_list_ref (encoder->priv->tags)));
    encoder->priv->tags_changed = FALSE;
  }
}

static gboolean
gst_video_encoder_sink_event_default (GstVideoEncoder * encoder, GstEvent * event)
{
  GstVideoEncoderClass *encoder_class;
  gboolean ret = FALSE;
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_CUSTOM_DOWNSTREAM:
    {
      if (gst_video_event_is_force_key_unit (event)) {
        ...
        if (gst_video_event_parse_downstream_force_key_unit (event, NULL, NULL, &running_time, &all_headers, &count)) {
          ForcedKeyUnitEvent *fevt;

          GST_OBJECT_LOCK (encoder);
          fevt = forced_key_unit_event_new (running_time, all_headers, count);
          encoder->priv->force_key_unit = g_list_append (encoder->priv->force_key_unit, fevt);
          GST_OBJECT_UNLOCK (encoder);

          GST_DEBUG_OBJECT (encoder, "force-key-unit event: running-time %" GST_TIME_FORMAT ", all_headers %d, count %u",
  ...
  if (event) {
    if (!GST_EVENT_IS_SERIALIZED (event)
      ...
      GST_VIDEO_ENCODER_STREAM_LOCK (encoder);
      encoder->priv->current_frame_events = g_list_prepend (encoder->priv->current_frame_events, event);
      GST_VIDEO_ENCODER_STREAM_UNLOCK (encoder);
      ret = TRUE;
    }
  }
...
static gboolean
gst_video_encoder_sink_event (GstPad * pad, GstObject * parent, GstEvent * event)
{
  GstVideoEncoder *enc;
  GstVideoEncoderClass *klass;
  gboolean ret = TRUE;
  ...
  GST_DEBUG_OBJECT (enc, "received event %d, %s", GST_EVENT_TYPE (event), GST_EVENT_TYPE_NAME (event));
...
static gboolean
gst_video_encoder_src_event_default (GstVideoEncoder * encoder, GstEvent * event)
{
  gboolean ret = FALSE;
...
gst_video_encoder_src_query_default (GstVideoEncoder * enc, GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    case GST_QUERY_CONVERT:
    {
      ...
      res = gst_video_encoded_video_convert (priv->bytes, priv->time, src_fmt, src_val, &dest_fmt, &dest_val);
      if (!res)
        goto error;
    ...
    case GST_QUERY_LATENCY:
    {
      ...
      if (res) {
        ...
        GST_DEBUG_OBJECT (enc, "Peer latency: live %d, min %" GST_TIME_FORMAT " max %" GST_TIME_FORMAT, live,
...
gst_video_encoder_new_frame (GstVideoEncoder * encoder, GstBuffer * buf, GstClockTime pts, GstClockTime dts, GstClockTime duration)
{
  GstVideoEncoderPrivate *priv = encoder->priv;
...
gst_video_encoder_chain (GstPad * pad, GstObject * parent, GstBuffer * buf)
{
...
#ifdef GST_EXT_VIDEOENCODER
  if ((encoder->priv->input_log_count < MAX_BUFFER_LOG_CNT) ||!(encoder->priv->input_log_count%BUFFER_LOG_INTERVAL)) {
    GST_WARNING_OBJECT (encoder, "IN_BUF[%02d]: gst_buf= %p ts= %" GST_TIME_FORMAT " size= %lu duration: %" GST_TIME_FORMAT ,
    encoder->priv->input_log_count, buf, GST_TIME_ARGS(pts), gst_buffer_get_size (buf), GST_TIME_ARGS(duration));
    encoder->priv->input_log_count++;
  } else {
    GST_LOG_OBJECT (encoder, "IN_BUF: gst_buf= %p ts= %" GST_TIME_FORMAT " size= %lu duration: %" GST_TIME_FORMAT ,
      buf, GST_TIME_ARGS(pts), gst_buffer_get_size (buf), GST_TIME_ARGS(duration));
    encoder->priv->input_log_count++;
  }
#else
  ...
  if (!gst_segment_clip (&encoder->input_segment, GST_FORMAT_TIME, start, stop, &cstart, &cstop)) {
  ...
  frame = gst_video_encoder_new_frame (encoder, buf, cstart, GST_CLOCK_TIME_NONE, duration);
  ...
  if (priv->force_key_unit) {
    ...
    if (fevt) {
      ...
      GST_DEBUG_OBJECT (encoder, "Forcing a key unit at running time %" GST_TIME_FORMAT, GST_TIME_ARGS (running_time));
  ...
  GST_LOG_OBJECT (encoder, "passing frame pfn %d to subclass", frame->presentation_frame_number);
...
gst_video_encoder_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      ...
      GST_WARNING_OBJECT(encoder, "GST_STATE_CHANGE_NULL_TO_READY");
      if (encoder_class->open && !encoder_class->open (encoder))
        goto open_failed;
      break;
    case GST_STATE_CHANGE_READY_TO_PAUSED:
      /* Initialize device/library if needed */
      GST_WARNING_OBJECT(encoder, "GST_STATE_CHANGE_READY_TO_PAUSED");
    ...
    case GST_STATE_CHANGE_PAUSED_TO_PLAYING:
      GST_WARNING_OBJECT(encoder, "GST_STATE_CHANGE_PAUSED_TO_PLAYING");
    default:
      break;
  }
  ...
  switch (transition) {
    case GST_STATE_CHANGE_PLAYING_TO_PAUSED:
      GST_WARNING_OBJECT(encoder, "GST_STATE_CHANGE_PLAYING_TO_PAUSED");
#ifdef GST_EXT_VIDEOENCODER
      encoder->priv->input_log_count = 0;
      encoder->priv->output_log_count = 0;
#endif
      break;
    case GST_STATE_CHANGE_PAUSED_TO_READY:
      GST_WARNING_OBJECT(encoder, "GST_STATE_CHANGE_PAUSED_TO_READY");
      GST_VIDEO_ENCODER_STREAM_LOCK (encoder);
      gst_video_encoder_reset (encoder, TRUE);
      GST_VIDEO_ENCODER_STREAM_UNLOCK (encoder);
    ...
    case GST_STATE_CHANGE_READY_TO_NULL:
      ...
      GST_WARNING_OBJECT(encoder, "GST_STATE_CHANGE_READY_TO_NULL");
...
open_failed:
  {
    GST_ELEMENT_ERROR (encoder, LIBRARY, INIT, (NULL), ("Failed to open encoder"));
    return GST_STATE_CHANGE_FAILURE;
  }

start_failed:
  {
    GST_ELEMENT_ERROR (encoder, LIBRARY, INIT, (NULL), ("Failed to start encoder"));
    return GST_STATE_CHANGE_FAILURE;
  }

stop_failed:
  {
    GST_ELEMENT_ERROR (encoder, LIBRARY, INIT, (NULL), ("Failed to stop encoder"));
    return GST_STATE_CHANGE_FAILURE;
  }

close_failed:
  {
    GST_ELEMENT_ERROR (encoder, LIBRARY, INIT, (NULL), ("Failed to close encoder"));
    return GST_STATE_CHANGE_FAILURE;
  }
}
...
gst_video_encoder_negotiate_default (GstVideoEncoder * encoder)
{
  ...
  GST_DEBUG_OBJECT (encoder, "ALLOCATION (%d) params: %" GST_PTR_FORMAT, ret, query);
...
gst_video_encoder_allocate_output_buffer (GstVideoEncoder * encoder, gsize size)
{
  ...
  buffer = gst_buffer_new_allocate (encoder->priv->allocator, size, &encoder->priv->params);
  if (!buffer) {
 ...
 */
GstFlowReturn
gst_video_encoder_allocate_output_frame (GstVideoEncoder * encoder, GstVideoCodecFrame * frame, gsize size)
{
  gboolean needs_reconfigure = FALSE;
...
static void
gst_video_encoder_release_frame (GstVideoEncoder * enc, GstVideoCodecFrame * frame)
{
  GList *link;
 ...
 */
GstFlowReturn
gst_video_encoder_finish_frame (GstVideoEncoder * encoder, GstVideoCodecFrame * frame)
{
  GstVideoEncoderPrivate *priv = encoder->priv;
  GstFlowReturn ret = GST_FLOW_OK;
  GstVideoEncoderClass *encoder_class;
  ...
  GST_LOG_OBJECT (encoder, "finish frame fpn %d", frame->presentation_frame_number);

  GST_LOG_OBJECT (encoder, "frame PTS %" GST_TIME_FORMAT ", DTS %" GST_TIME_FORMAT, GST_TIME_ARGS (frame->pts),
  ...
  if (!frame->output_buffer) {
    GST_DEBUG_OBJECT (encoder, "skipping frame %" GST_TIME_FORMAT, GST_TIME_ARGS (frame->pts));
    goto done;
  }
  ...
  if (GST_VIDEO_CODEC_FRAME_IS_SYNC_POINT (frame) && priv->force_key_unit) {
    ...
    running_time =
        gst_segment_to_running_time (&encoder->output_segment, GST_FORMAT_TIME, frame->pts);
    ...
    if (fevt) {
      stream_time = gst_segment_to_stream_time (&encoder->output_segment, GST_FORMAT_TIME, frame->pts);

      ev = gst_video_event_new_downstream_force_key_unit (frame->pts, stream_time, running_time, fevt->all_headers, fevt->count);
      ...
      GST_DEBUG_OBJECT (encoder, "Forced key unit: running-time %" GST_TIME_FORMAT ", all_headers %d, count %u",
   ...
   * so a good guess is the lowest unsent PTS (all being OK) */
  {
    ...
    if (!GST_CLOCK_TIME_IS_VALID (frame->dts) && !seen_none) {
      frame->dts = min_ts;
      GST_DEBUG_OBJECT (encoder, "no valid DTS, using oldest PTS %" GST_TIME_FORMAT, GST_TIME_ARGS (frame->pts));
    }
  }
...
#ifdef GST_EXT_VIDEOENCODER
  if ((encoder->priv->output_log_count < MAX_BUFFER_LOG_CNT) ||!(encoder->priv->output_log_count%BUFFER_LOG_INTERVAL)) {
    GST_WARNING_OBJECT (encoder, "OUT_BUF[%02d]: gst_buf= %p ts= %" GST_TIME_FORMAT " size= %lu duration: %" GST_TIME_FORMAT ,
    encoder->priv->output_log_count, buffer, GST_TIME_ARGS(GST_BUFFER_PTS(buffer)), gst_buffer_get_size (buffer), GST_TIME_ARGS(GST_BUFFER_DURATION(buffer)));
    encoder->priv->output_log_count++;
  } else {
    GST_LOG_OBJECT (encoder, "OUT_BUF: gst_buf= %p ts= %" GST_TIME_FORMAT " size= %lu duration: %" GST_TIME_FORMAT ,
      buffer, GST_TIME_ARGS(GST_BUFFER_PTS(buffer)), gst_buffer_get_size (buffer), GST_TIME_ARGS(GST_BUFFER_DURATION(buffer)));
    encoder->priv->output_log_count++;
  }
#endif
...
#ifdef GST_EXT_VIDEOENCODER
    if ((encoder->priv->output_log_count < MAX_BUFFER_LOG_CNT)||(encoder->priv->output_log_count % BUFFER_LOG_INTERVAL == 0)) {
      GST_WARNING_OBJECT (encoder, "gst_pad_push end[%d]: return = %d", encoder->priv->output_log_count, ret);
    } else {
      GST_LOG_OBJECT (encoder, "gst_pad_push end: return = %d", ret);
    }
#endif
  }
 ...
 */
GstVideoCodecState *
gst_video_encoder_set_output_state (GstVideoEncoder * encoder, GstCaps * caps, GstVideoCodecState * reference)
{
  GstVideoEncoderPrivate *priv = encoder->priv;
  GstVideoCodecState *state;
