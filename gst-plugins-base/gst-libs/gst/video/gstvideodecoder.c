...
#ifdef GST_EXT_VIDEODECODER
  decoder->self_buffer_pool = FALSE;
#endif
...
_new_output_state (GstVideoFormat fmt, guint width, guint height, GstVideoCodecState * reference)
{
  ...
  GST_DEBUG ("setting caps %" GST_PTR_FORMAT, state->caps);
...
gst_video_decoder_negotiate_default (GstVideoDecoder * decoder)
{
  ...
  /* Negotiate pool */
#ifdef GST_EXT_VIDEODECODER
  /* Not use downstream buffer pool by xvimagesink */
  if (!(decoder->self_buffer_pool))
#endif
