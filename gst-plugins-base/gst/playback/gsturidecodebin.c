...
configure_stream_buffering (GstURIDecodeBin * decoder)
{
  ...
  if (!queue)
    return;
/*
[prasenjit.c] for mp3 playback based WebApps , Queuesize was coming around 20KB due to Bitrate calculation and Default size of 2 sec data.
This was resulting in frequent pause-resume during playback due to 0% buffering getting posted as 20KB was getting consumed fast.
So disabled the routine for modifying the max-size-bytes for Queue2 , so we we have enough queuesize to have a proper playback.
*/
  if ((bitrate > 0) && !(strstr(GST_ELEMENT_NAME(queue), "queue2"))) {
    guint64 time;
    guint bytes;
...
// by default, webkit gets buffering message from multi-queue
// so if uridecodebin enables "use-buffering" here then webkit gets buffering messages from two elements.
// this causes flickering/sound breaking issue when seek happens ( at mp3 streaming case like gaana web app)
// buffering message from multiqueue is suitable for A/V streaming - yoserb.yi
// (queue2 can only control whole stream but multiqueue can control each stream)

/*[prasenjit.c] "use-buffering" can be disabled for Audio mp4 contents where qtdemux and multiqueue gets added.
But for audio/mp3 streaming contents , queue2 is the only queue , so buffering should be enabled.
As per Qtdemux capabilities , the mime types it supports are :
    Capabilities:
    video/quicktime , video/mj2 , audio/x-m4a , application/x-3gp
Added check for all the above cases.
*/

#ifdef GST_EXT_URIDECODEBIN_MODIFICATION
      if ((strstr(media_type,"video/quicktime")) || (strstr(media_type,"video/mj2")) ||
          (strstr(media_type,"audio/x-m4a")) || (strstr(media_type,"application/x-3gp")))
        GST_WARNING_OBJECT (decoder, "Disable use-buffer in queue2");
      else
