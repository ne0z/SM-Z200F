...
struct _GstDecodeBin
{
  ...
  gboolean is_adaptive_mq_size_needed;   /* Whether to adaptively change multiqueue size based on resolution */
  gboolean is_audio_only_hls;   /* Whether the content is a Audio-Only HLS Content */
  gboolean is_audio_only_hls_adaptive_caching_enabled;   /* Whether Audio-Only HLS Adaptive Caching enabled or not */
  GstElement *hls_multiqueue_downloader;         /* this holds the First Multiqueue object for HLS */
  GstPad *hls_multiqueue_downloader_srcpad;
  gboolean is_downloader_mq_blocked;
  guint mq_blocker_probe_id;
  GstElement *hls_buffering_multiqueue;            /* this holds the Second Multiqueue object for HLS */
#endif
...
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
#define DEFAULT_ADAPTIVE_MULTIQUEUE_PROPERTY     		FALSE
#define ADAPTIVE_LOW_RESOLUTION_MQ_SIZE             		2 * 1024 * 1024
#define ADAPTIVE_MEDIUM_RESOLUTION_MQ_SIZE       		6 * 1024 * 1024
#define ADAPTIVE_HIGH_RESOLUTION_MQ_SIZE            		10 * 1024 * 1024
#define ADAPTIVE_VERY_HIGH_RESOLUTION_MQ_SIZE    	20 * 1024 * 1024

#define DEFAULT_QVGA_HEIGHT							240
#define DEFAULT_QVGA_WIDTH							320
#define DEFAULT_VGA_HEIGHT							480
#define DEFAULT_VGA_WIDTH								640
#define DEFAULT_HD_HEIGHT								720
#define DEFAULT_HD_WIDTH								1280
#define DEFAULT_IS_AUDIO_ONLY_HLS        				FALSE
#define DEFAULT_AUDIO_ONLY_HLS_ADAPTIVE_CACHING      FALSE
#define DEFAULT_AUDIO_HLS_MAX_SIZE_BUFFERS  			800
#endif
...
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
  PROP_ADAPTIVE_MULTIQUEUE_SIZE_NEEDED,
  PROP_AUDIO_ONLY_HLS_DBIN,
  PROP_AUDIO_ONLY_HLS_ADAPTIVE_CACHING,
#endif
  PROP_LAST
};
...
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
   /**
   * GstDecodeBin::is-adaptive-mq-size-needed
   *
   * Property to dynamically set the Multiqueue size depending on Video Resolution
   *
   */
  g_object_class_install_property (gobject_klass, PROP_ADAPTIVE_MULTIQUEUE_SIZE_NEEDED,
      g_param_spec_boolean ("is-adaptive-mq-size-needed", "Is Adaptive Multiqueue Size Setting Needed",
          "Property to whether dynamically set the Multiqueue size depending on Video Resolution",
          DEFAULT_ADAPTIVE_MULTIQUEUE_PROPERTY,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

   /**
   * GstDecodeBin::audio-only-hls-content
   *
   * Property to set and get if it is Audio-Only HLS Clip
   *
   */
  g_object_class_install_property (gobject_klass, PROP_AUDIO_ONLY_HLS_DBIN,
      g_param_spec_boolean ("audio-only-hls-content", "Audio Only HLS Content Setting",
          "Property to set or get whether the content is a Audio-Only HLS Content",
          DEFAULT_IS_AUDIO_ONLY_HLS,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

   /**
   * GstDecodeBin::enable-audio-only-hls-adaptive-caching
   *
   * Property to set and get Adaptive Caching Mechanism for Audio-Only HLS Contents
   *
   */
  g_object_class_install_property (gobject_klass, PROP_AUDIO_ONLY_HLS_ADAPTIVE_CACHING,
      g_param_spec_boolean ("enable-audio-only-hls-adaptive-caching", "To enable audio only hls adaptive caching",
          "Property to set or get the property to adaptively caching HLS Audio Fragments. Need to set audio-only-hls-content property first.",
          DEFAULT_AUDIO_ONLY_HLS_ADAPTIVE_CACHING,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#endif
...
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
  decode_bin->is_adaptive_mq_size_needed = DEFAULT_ADAPTIVE_MULTIQUEUE_PROPERTY;
  decode_bin->is_audio_only_hls= DEFAULT_IS_AUDIO_ONLY_HLS;
  decode_bin->hls_multiqueue_downloader = NULL;
  decode_bin->hls_multiqueue_downloader_srcpad = NULL;
  decode_bin->is_downloader_mq_blocked = FALSE;
  decode_bin->hls_buffering_multiqueue = NULL;
#endif
}
...
gst_decode_bin_dispose (GObject * object)
{
 ...
 #ifdef GST_EXT_DECODEBIN2_MODIFICATION
 if (decode_bin->hls_multiqueue_downloader_srcpad)
  {
	if (decode_bin->is_downloader_mq_blocked)
	{
           gst_pad_remove_probe(decode_bin->hls_multiqueue_downloader_srcpad, decode_bin->mq_blocker_probe_id);
	    decode_bin->is_downloader_mq_blocked = FALSE;
	}
	gst_object_unref(decode_bin->hls_multiqueue_downloader_srcpad);
  }
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_CONNECTION_SPEED:
      ...
      break;
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
    case PROP_ADAPTIVE_MULTIQUEUE_SIZE_NEEDED:
	dbin->is_adaptive_mq_size_needed = g_value_get_boolean (value);
	GST_DEBUG_OBJECT (dbin, "adaptive-multiqueue-size property set to %d",dbin->is_adaptive_mq_size_needed);
	break;
    case PROP_AUDIO_ONLY_HLS_DBIN:
	dbin->is_audio_only_hls = g_value_get_boolean (value);
	GST_DEBUG_OBJECT (dbin, "is-audio-only-hls-content set to %d",dbin->is_audio_only_hls);
	break;
    case PROP_AUDIO_ONLY_HLS_ADAPTIVE_CACHING:
	if (!dbin->is_audio_only_hls)
		GST_WARNING_OBJECT (dbin, "Can be enabled only for Audio-Only HLS Contents !! Need to set is-audio-only-hls-content property first !");
	else
	{
		dbin->is_audio_only_hls_adaptive_caching_enabled = g_value_get_boolean (value);
		GST_DEBUG_OBJECT (dbin, "enable-audio-only-hls-adaptive-caching set to %d",dbin->is_audio_only_hls_adaptive_caching_enabled);

		if (G_IS_OBJECT(dbin->hls_multiqueue_downloader))
		{
		  GST_DEBUG_OBJECT (dbin, "Changing Audio-HLS Buffer Limit to Max Fragments !!");
		  g_object_set (G_OBJECT (dbin->hls_multiqueue_downloader),"max-size-buffers", (guint)DEFAULT_AUDIO_HLS_MAX_SIZE_BUFFERS, NULL);
		}

		if (G_IS_OBJECT(dbin->hls_buffering_multiqueue))
		{
		  GST_DEBUG_OBJECT (dbin, "Specifying Audio-HLS type in MQ !!");
		  g_object_set (G_OBJECT (dbin->hls_buffering_multiqueue),"is-audio-only-hls-mq", TRUE, NULL);
		}
	}
	break;
#endif
...
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
    case PROP_ADAPTIVE_MULTIQUEUE_SIZE_NEEDED:
      g_value_set_boolean (value, dbin->is_adaptive_mq_size_needed);
      break;
    case PROP_AUDIO_ONLY_HLS_DBIN:
      g_value_set_boolean (value, dbin->is_audio_only_hls);
      break;
    case PROP_AUDIO_ONLY_HLS_ADAPTIVE_CACHING:
      g_value_set_boolean (value, dbin->is_audio_only_hls_adaptive_caching_enabled);
      break;
#endif
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
...
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
    if ((strstr((GST_OBJECT_NAME (GST_OBJECT_PARENT(pad))),"multiqueue")) && (strstr((GST_OBJECT_NAME (pad)),"src")) && caps)
    {
	gchar *caps_str = gst_caps_to_string(caps);
	if (strstr(caps_str, "video/mpegts"))
	{
	   dbin->hls_multiqueue_downloader_srcpad = gst_object_ref(pad);
	}
	g_free( caps_str );
    }
#endif
...
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
static void
mqueue_lowbuffering (GstElement * mqueue, gpointer user_data)
{
   GstDecodeBin *dbin = (GstDecodeBin *)user_data;

   if (dbin->is_downloader_mq_blocked)
   {
     GST_DEBUG_OBJECT (dbin, "HLS Audio-Only MQ0 Probe Removed !!!");
     gst_pad_remove_probe(dbin->hls_multiqueue_downloader_srcpad, dbin->mq_blocker_probe_id);
     dbin->is_downloader_mq_blocked = FALSE;
   }
}
#endif
    ...
    gboolean preroll, gboolean seekable, gboolean adaptive_streaming)
{
  ...
  if (preroll || use_buffering) {
...
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
    dbin->hls_buffering_multiqueue = multiqueue;
    GST_DEBUG_OBJECT (multiqueue, "assigned 2nd mq instance to dbin->hls_buffering_multiqueue !!");
    g_signal_connect (dbin->hls_buffering_multiqueue, "lowbuffering", G_CALLBACK (mqueue_lowbuffering), dbin);
#endif
    ...
    max_bytes = 0;
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
    GST_DEBUG_OBJECT (multiqueue, "overwritten first mq instance to dbin->hls_multiqueue_downloader for Adaptive HLS Contents !!");
    dbin->hls_multiqueue_downloader = multiqueue;
#endif
  } else {
    ...
    gpointer user_data)
{
  ...
  GstDecodePad *dpad = user_data;
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
  GstDecodeChain *chain = dpad->chain;
  GstDecodeBin *dbin = dpad->dbin;
#endif
  gboolean res = TRUE;

#ifdef GST_EXT_DECODEBIN2_MODIFICATION
  GST_LOG_OBJECT (pad, "%s dpad:%p,dbin->is_adaptive_mq_size_needed(%d)", GST_EVENT_TYPE_NAME (event), dpad,dbin->is_adaptive_mq_size_needed);

  if ((GST_EVENT_TYPE (event) == GST_EVENT_CAPS)&&(dbin->is_adaptive_mq_size_needed == TRUE))
  {
	GstCaps *caps;
	GstStructure *structure = NULL;
	gint video_caps_width = 0;
	gint video_caps_height = 0;
	guint max_bytes;
	gst_event_parse_caps (event, &caps);
	if ((chain->parent != NULL)&&(chain->parent->multiqueue !=NULL))
	{
		GST_DEBUG_OBJECT (pad, "GST_EVENT_CAPS Decodebin2 : %"GST_PTR_FORMAT",GST_ELEMENT_NAME (group->multiqueue) = %s", caps,GST_ELEMENT_NAME (chain->parent->multiqueue));

		structure = gst_caps_get_structure(caps, 0);
		if (!gst_structure_get_int (structure, "width", &video_caps_width) || !gst_structure_get_int (structure, "height", &video_caps_height)) {
			GST_WARNING_OBJECT (pad, "height/width not found !! Ignoring !! ");
		}
		else
		{
			if ((video_caps_width * video_caps_height) <= (DEFAULT_QVGA_WIDTH * DEFAULT_QVGA_HEIGHT))
			{
				GST_WARNING_OBJECT (pad, "Small Multiqueue Size set !!");
				max_bytes = ADAPTIVE_LOW_RESOLUTION_MQ_SIZE;
			}
			else if ((video_caps_width * video_caps_height) <= (DEFAULT_VGA_WIDTH * DEFAULT_VGA_HEIGHT))
			{
				GST_WARNING_OBJECT (pad, "Medium Multiqueue Size set !!");
				max_bytes = ADAPTIVE_MEDIUM_RESOLUTION_MQ_SIZE;
			}
			else if ((video_caps_width * video_caps_height) <= (DEFAULT_HD_WIDTH * DEFAULT_HD_HEIGHT))
			{
				GST_WARNING_OBJECT (pad, "High Multiqueue Size set !!");
				max_bytes = ADAPTIVE_HIGH_RESOLUTION_MQ_SIZE;
			}
			else
			{
				GST_WARNING_OBJECT (pad, "Very High Multiqueue Size set !!");
				max_bytes = ADAPTIVE_VERY_HIGH_RESOLUTION_MQ_SIZE;
			}

			g_object_set (chain->parent->multiqueue, "max-size-bytes", (guint32)max_bytes, "max-size-time", (guint64)0, NULL);
		}
	}
	else
		 GST_DEBUG_OBJECT (pad, "chain->parent->multiqueue or chain->parent is NULL !!");
  }
#else
...
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
static GstPadProbeReturn
mq0_src_cb_blocked (GstPad          *pad,
            GstPadProbeInfo *info,
            gpointer         user_data)
{
  GstDecodeBin *dbin =user_data;
  int mq1_perc = -1;

   if (G_IS_OBJECT(dbin->hls_buffering_multiqueue) && G_IS_OBJECT(dbin->hls_multiqueue_downloader))
   {
	g_object_get (dbin->hls_buffering_multiqueue, "audio-only-buffering-percent", &mq1_perc, NULL);
...
gst_decode_bin_handle_message (GstBin * bin, GstMessage * msg)
{
  ...
  gboolean drop = FALSE;
#ifdef GST_EXT_DECODEBIN2_MODIFICATION
  int mq0_perc = -1 , mq1_perc = -1;

   if (G_IS_OBJECT(dbin->hls_buffering_multiqueue) && G_IS_OBJECT(dbin->hls_multiqueue_downloader) && dbin->is_audio_only_hls_adaptive_caching_enabled)
   {
	g_object_get (dbin->hls_multiqueue_downloader, "audio-only-buffering-percent", &mq0_perc, NULL);
	g_object_get (dbin->hls_buffering_multiqueue, "audio-only-buffering-percent", &mq1_perc, NULL);

	GST_DEBUG_OBJECT (dbin, "Current Buffering Message mq0_perc = %d , mq1_perc = %d , dbin->is_downloader_mq_blocked = %d",mq0_perc,mq1_perc,dbin->is_downloader_mq_blocked);

	if (mq0_perc > 60 && !dbin->is_downloader_mq_blocked)
	{
		if (dbin->hls_multiqueue_downloader_srcpad && mq1_perc > 90 )
		{
			dbin->mq_blocker_probe_id = gst_pad_add_probe (dbin->hls_multiqueue_downloader_srcpad, GST_PAD_PROBE_TYPE_BLOCK_DOWNSTREAM,(GstPadProbeCallback) mq0_src_cb_blocked, dbin, NULL);
			dbin->is_downloader_mq_blocked = TRUE;
	              GST_DEBUG_OBJECT (dbin, "HLS Audio-Only MQ0 Srcpad Blocked !!!");
		}
	}

	if (dbin->is_downloader_mq_blocked && mq1_perc <= 10 )
	{
		GST_DEBUG_OBJECT (dbin, "HLS Audio-Only MQ0 Probe Removed !!!");
		gst_pad_remove_probe(dbin->hls_multiqueue_downloader_srcpad, dbin->mq_blocker_probe_id);
		dbin->is_downloader_mq_blocked = FALSE;
