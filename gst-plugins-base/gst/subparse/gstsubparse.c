...
gst_sub_parse_dispose (GObject * object)
{
  ...
  if (subparse->state.msl_language) {
    g_free (subparse->state.msl_language);
    subparse->state.msl_language = NULL;
  }
#endif
...
parse_subrip_time (const gchar * ts_string, GstClockTime * t)
{
  ...
  g_assert (p != NULL);
#ifdef SUBPARSE_MODIFICATION
  if(p){/*g_assert isn't doing the job for us*/
#endif
  ++p;
  len = strlen (p);
  if (len > 3) {
    ...
    GST_WARNING ("failed to parse subrip millisec timestamp'");
  }
#endif
...
parser_state_init (ParserState * state)
{
  ...
  if (state->msl_language) {
    g_free (state->msl_language);
    state->msl_language = NULL;
  }
...
#ifdef SUBPARSE_MODIFICATION
  data = g_strndup (self->textbuf->str, FORMAT_DETECTION_TEXT_STRING);
#else
  ...
  while (!self->flushing && (line = get_next_line (self))) {
    ...
    if (self->parser_type == GST_SUB_PARSE_FORMAT_SAMI) {
      do {
        ...
        GST_DEBUG_OBJECT (self, "parse_line returned %s", subtitle);
...
#ifndef SUBPARSE_MODIFICATION
      /*Dont un-set parser type as it is used for parser de-init in dispose fn*/
      self->parser_type = GST_SUB_PARSE_FORMAT_UNKNOWN;
#endif
      break;
    default:
