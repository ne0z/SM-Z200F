...
_ecore_buffer_con_init(void)
{
   ...
   EINA_LOG_ERR("profiling Begin: %s", __FUNCTION__);
   ...
   EINA_LOG_ERR("profiling Begin: %s wl_display_connect", __FUNCTION__);
   if (!(_connection->display = wl_display_connect(name)))
     {
        ERR("Failed to connect to Queue Server");
        goto err_connect;
     }
   EINA_LOG_ERR("profiling End: %s wl_display_connect", __FUNCTION__);
   ...
   wl_callback_add_listener(callback, &_ecore_buffer_con_init_sync_listener,
                            _connection);
   EINA_LOG_ERR("profiling End: %s", __FUNCTION__);
   return EINA_TRUE;;
err_get_registry:
