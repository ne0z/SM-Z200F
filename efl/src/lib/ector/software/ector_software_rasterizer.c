...
_blend_color_argb(int count, const SW_FT_Span *spans, void *user_data)
{
   ...
   uint *buffer = data->raster_buffer.buffer + ((data->raster_buffer.stride/4) * data->offy + data->offx);

   while (count--)
     {
        uint *target = buffer + ((data->raster_buffer.stride/4) * spans->y + spans->x);
...
_blend_gradient(int count, const SW_FT_Span *spans, void *user_data)
{
  ...
  // move to the offset location
   unsigned int *destbuffer = data->raster_buffer.buffer + ((data->raster_buffer.stride/4) * data->offy + data->offx);

   while (count--)
     {
        unsigned int *target = destbuffer + ((data->raster_buffer.stride/4) * spans->y + spans->x);
