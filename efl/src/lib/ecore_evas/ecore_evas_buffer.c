...
#ifdef __ECORE_EVAS_PRODUCT_H
        if (bdata->alloc_func_with_stride)
          {
             bdata->pixels = bdata->alloc_func_with_stride(bdata->data,
...
_ecore_evas_buffer_ecore_evas_create(int w, int h, void *alloc_func, void *free_func, const void *data, const Eina_Bool use_stride)
{
   ...
   if(use_stride)
     {
        bdata->alloc_func_with_stride = alloc_func;
        bdata->alloc_func = NULL;
     }
   else
     {
        bdata->alloc_func_with_stride = NULL;
#endif
...
_ecore_evas_buffer_evas_create(Ecore_Evas* ee, int w,int h, const Eina_Bool  use_stride)
{
   ...
   bdata = (Ecore_Evas_Engine_Buffer_Data *)ee->engine.data;
   EINA_SAFETY_ON_NULL_RETURN_VAL(bdata, EINA_FALSE);
...
#ifdef __ECORE_EVAS_PRODUCT_H
   if(use_stride)
     bdata->pixels = bdata->alloc_func_with_stride(bdata->data, w * h * sizeof(int), &buf_stride, &buf_bpp);
   else
#endif
...
#ifdef __ECORE_EVAS_PRODUCT_H
EAPI Ecore_Evas *
ecore_evas_buffer_allocfunc_with_stride_new(int w, int h,
...
ecore_evas_buffer_allocfunc_set(Ecore_Evas *ee, void *(*alloc_func_with_stride) (void *data, int size, int *stride, int *bpp))
{
   ...
   bdata->alloc_func_with_stride = alloc_func_with_stride;
   bdata->alloc_func = NULL;
   if (bdata->alloc_func_with_stride)
     {
        bdata->pixels = bdata->alloc_func_with_stride(bdata->data,
