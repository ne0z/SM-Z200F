...
extern Ecore_X_Atom ECORE_X_ATOM_E_COMP_BUSY_NOTIFY;
extern Ecore_X_Atom ECORE_X_ATOM_E_COMP_EVAS_WIN;
extern Ecore_X_Atom ECORE_X_ATOM_E_APPPROFILE_GPU_WIN;

static const char *busy_evas_atom_name = "_E_COMP_BUSY_APP_NOTIFY_EVAS_WIN"; // Atomic ID string
static const char *devopt_gpu_win_atom_name = "_E_APPPROFILE_WIN_IS_GPU"; // Atomic ID string
   ...
   if (ee->prop.window)
     {
        ...
        //get the atom
        ECORE_X_ATOM_E_COMP_EVAS_WIN = ecore_x_atom_get(busy_evas_atom_name);

        if (ECORE_X_ATOM_E_COMP_EVAS_WIN)
          {
             //Set ATOM to true.
             ecore_x_window_prop_card32_set(ee->prop.window, ECORE_X_ATOM_E_COMP_EVAS_WIN, &val, 1);
          }
     }

   //to check if this window is opengl based / gpu based
   if (ee->prop.window && ee->driver && !strcmp(ee->driver, "opengl_x11"))
     {
        int val = 1;

        //get the atom
        ECORE_X_ATOM_E_APPPROFILE_GPU_WIN = ecore_x_atom_get(devopt_gpu_win_atom_name);

        if (ECORE_X_ATOM_E_APPPROFILE_GPU_WIN)
          {
             //Set ATOM to true
             ecore_x_window_prop_card32_set(ee->prop.window, ECORE_X_ATOM_E_APPPROFILE_GPU_WIN, &val, 1);
          }
     }
...
EAPI Ecore_Evas *
ecore_evas_software_x11_pixmap_allocfunc_new(const char *disp_name, Ecore_X_Window parent, int x, int y, int w, int h, unsigned int (*alloc_func) (void *data, unsigned int, int w, int h, int depth), void (*free_func) (void *data, unsigned int pixmap), void *data)
{
   Ecore_Evas *(*new)(const char *, Ecore_X_Window, int, int, int, int, unsigned int (*)(void *, unsigned int, int, int, int), void (*)(void *, unsigned int), void *);
   Eina_Module *m = _ecore_evas_engine_load("x");
   EINA_SAFETY_ON_NULL_RETURN_VAL(m, NULL);

   new = eina_module_symbol_get(m, "ecore_evas_software_x11_pixmap_allocfunc_new");
   EINA_SAFETY_ON_NULL_RETURN_VAL(new, NULL);

   return new(disp_name, parent, x, y, w, h, alloc_func, free_func, data);
}
...
EAPI Ecore_Evas *
ecore_evas_gl_x11_pixmap_allocfunc_new(const char *disp_name, Ecore_X_Window parent, int x, int y, int w, int h, unsigned int (*alloc_func) (void *data, unsigned int parent, int w, int h, int depth), void (*free_func) (void *data, unsigned int pixmap), void *data)
{
   Ecore_Evas *(*new)(const char *, Ecore_X_Window, int, int, int, int, unsigned int (*)(void *, unsigned int, int, int, int), void (*)(void *, unsigned int), void *);
   Eina_Module *m = _ecore_evas_engine_load("x");
   EINA_SAFETY_ON_NULL_RETURN_VAL(m, NULL);

   new = eina_module_symbol_get(m, "ecore_evas_gl_x11_pixmap_allocfunc_new");
   EINA_SAFETY_ON_NULL_RETURN_VAL(new, NULL);
...
{
   void *(*pixels_get)(Ecore_Evas*);
   Eina_Module *m = _ecore_evas_engine_load("extn");
   EINA_SAFETY_ON_NULL_RETURN(m);

   pixels_get = eina_module_symbol_get(m,
         "ecore_evas_extn_socket_pixels_get_internal");
   EINA_SAFETY_ON_NULL_RETURN(pixels_get);

   return pixels_get(ee);
}
