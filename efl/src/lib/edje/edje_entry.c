                  ...
                  if (!ll)
                    {
                       ...
                       sel = calloc(1, sizeof(Sel));
                       an->sel = eina_list_append(an->sel, sel);
                       // All sources may not be used so better not to create edje object
   ...
   if (!strcmp(ev->key, "Escape"))
            ...
            (!strcmp(ev->key, "KP_Up") && !ev->string))
     {
        ...
        if (multiline)
          ...
          // TIZEN ONLY: (20150828): Focus Change issue by cursor control on SIP
          if (en->imf_context &&
               (ecore_imf_context_input_panel_state_get(en->imf_context) == ECORE_IMF_INPUT_PANEL_STATE_SHOW))
            ev->event_flags |= EVAS_EVENT_FLAG_ON_HOLD;
         // TIZEN ONLY
...
_edje_entry_real_part_init(Edje *ed, Edje_Real_Part *rp, Ecore_IMF_Context *ic)
{
...
#ifdef EFL_FEATURE_CHANGEABLE_UI
   color_parent = evas_object_data_get(ed->obj, "color_overlay");
   if (color_parent)
   ...
   color_parent = evas_object_data_get(ed->obj, "color_overlay");
   if (color_parent)
      evas_object_data_set(en->cursor_fg2, "color_overlay", color_parent);
#endif
...
_edje_entry_text_markup_set(Edje_Real_Part *rp, const char *text)
{
   ...
   _edje_entry_imf_context_reset(rp);
   // set text as markup
   int old_pos = _edje_entry_cursor_pos_get(rp, EDJE_CURSOR_MAIN);
   _sel_clear(en->ed, en->cursor, rp->object, en);
   evas_object_textblock_text_markup_set(rp->object, text);
   _edje_entry_cursor_pos_set(rp, EDJE_CURSOR_MAIN, old_pos);
...
_edje_entry_imf_event_commit_cb(void *data, Ecore_IMF_Context *ctx EINA_UNUSED, void *event_info)
{
   ...
   if ((rp->part->entry_mode == EDJE_ENTRY_EDIT_MODE_PASSWORD) &&
     ...
     {
     // TIZEN_ONLY(20150629): In case of non-english language preedit status is needed to be maintained
     // so markup text comes from imf
        char *markup_text = evas_textblock_text_utf8_to_markup(NULL, commit_str);
        info = _text_filter_markup_prepend(ed, en, en->cursor, markup_text,
...
#ifdef EFL_FEATURE_CHANGEABLE_UI
static Eina_Bool
_get_color_value(const char *format, const char *tag, char *buf, Eina_Bool is_user)
{
   int count;
   char *start, *end;
   count = 0;
   ...
   if (!is_user)
     start = strstr(start + strlen(tag), tag);
   if ((!start) || (strlen(start) <= strlen(tag)))
     return EINA_FALSE;
...
_edje_entry_imf_event_preedit_changed_cb(void *data, Ecore_IMF_Context *ctx EINA_UNUSED, void *event_info EINA_UNUSED)
{
   ...
   // TIZEN_ONLY(20150629): Need to clear the selection text else preedit text
   //remains causing repaeted character
