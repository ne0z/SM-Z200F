...
extern int evas_object_is_inside(Evas_Object *obj, Evas_Coord x, Evas_Coord y);
static Eina_Bool _control_masking_signal = EINA_FALSE;
static Evas_Coord_Point _touch_down_point = {0, 0};
_Edje_Touch_Enhancement_Cb _edje_touch_enhancement_cb = NULL;
static Evas_Coord _click_area_threshold = 0;
Eina_Bool _edje_touch_debug = EINA_FALSE;

static _Edje_Touch_Core_Info _edje_mouse_info[2];
...
_edje_mouse_down_signal_cb(void *data, Eo *obj, const Eo_Event_Description *desc EINA_UNUSED, void *event_info)
{
   ...
   if (_edje_touch_enhancement_cb && ev)
     {
        _edje_mouse_info[0].type = EVAS_CALLBACK_MOUSE_DOWN;
        _edje_mouse_info[0].button = ev->button;
        _edje_mouse_info[0].canvas.x = ev->canvas.x;
        _edje_mouse_info[0].canvas.y = ev->canvas.y;
        _edje_mouse_info[0].timestamp = ev->timestamp;
     }
   ...
   _touch_down_point.x = ev->canvas.x;
   _touch_down_point.y = ev->canvas.y;

   // samsung product only
   if (_edje_touch_debug)
     {
        ERR("[down] pos:[%d,%d], flag:[%d,%d,%d], time:0x%x, %s", 
          ev->canvas.x, ev->canvas.y, ev->flags, ev->event_flags, (int)rp->still_in, ev->timestamp, rp->part->name);
     }
...
_edje_mouse_up_signal_cb(void *data, Eo *obj, const Eo_Event_Description *desc EINA_UNUSED, void *event_info)
{
   ...
   if (_edje_touch_enhancement_cb && ev)
     {
        _edje_mouse_info[1].type = EVAS_CALLBACK_MOUSE_UP;
        _edje_mouse_info[1].button = ev->button;
        _edje_mouse_info[1].canvas.x = ev->canvas.x;
        _edje_mouse_info[1].canvas.y = ev->canvas.y;
        _edje_mouse_info[1].timestamp = ev->timestamp;
     }
   ...
   dx = abs(ev->canvas.x - _touch_down_point.x);
   dy = abs(ev->canvas.y - _touch_down_point.y);
   distance = sqrt((dx * dx) + (dy * dy));

   // samsung product only
   if (_edje_touch_enhancement_cb &&
     _edje_touch_enhancement_cb(&_edje_mouse_info[0], &_edje_mouse_info[1]) == EINA_TRUE)
     {
        rp->still_in = EINA_TRUE;
        ignored = EINA_FALSE;
     }
   ...
   if ((rp->still_in) && (rp->clicked_button == ev->button) && (!ignored)
   	&& (_click_area_threshold == 0 || distance < _click_area_threshold))
     {
        if (_control_masking_signal)
          {
             if (evas_object_precise_is_inside_get(obj) && !evas_object_is_inside(obj, ev->canvas.x, ev->canvas.y))
               {
                  ERR("Out of masking area unclicked event : name=%s, 0x%x, %s", rp->part->name, obj, evas_object_type_get(obj));
               }
             else
               {
     ...
     {
        INF("Mouse unclicked event : name=%s", rp->part->name);
        snprintf(buf, sizeof(buf), "mouse,unclicked,%i", ev->button);
        _edje_emit(ed, buf, rp->part->name);
     }
   ...
   if (_edje_touch_debug)
     {
        ERR("[up] pos:[%d,%d], flag:[%d,%d,%d,%d], time:0x%x, %s", 
        ev->canvas.x, ev->canvas.y, ev->flags, ev->event_flags, (int)rp->still_in, clicked, ev->timestamp, rp->part->name);
     }
...
// samsung product only
EAPI void
edje_mouse_control_masking_signal(Eina_Bool signal)
{
   _control_masking_signal = signal;
}

EAPI void
_edje_mouse_click_area_threshold_set(Evas_Coord click_area_threshold)
{
   _click_area_threshold = click_area_threshold;
}

EAPI Evas_Coord
_edje_mouse_click_area_threshold_get(void)
{
   return _click_area_threshold;
}

EAPI void
_edje_touch_enhancement_cb_set(_Edje_Touch_Enhancement_Cb cb)
{
   _edje_touch_enhancement_cb = cb;
}
