...
_edje_recalc_do(Edje *ed)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_EDJE, "group(%s)", ed->group);
#endif
...
//recalc color only when color_class is changed
static void
_edje_part_recalc_color(Edje *ed, Edje_Real_Part *ep)
{
   Edje_Part_Description_Common *desc = ep->param1.description;
   Edje_Color *color = &(ep->param1.p.color);
   Edje_Color *color2 = &(ep->param1.p.type.text.color2);
   Edje_Color *color3 = &(ep->param1.p.type.text.color3);
   ...
   if (ep->part->type != EDJE_PART_TYPE_SPACER)
     {
        ...
        if (cc)
          {
             color->r = (((int)cc->r + 1) * desc->color.r) >> 8;
             color->g = (((int)cc->g + 1) * desc->color.g) >> 8;
             color->b = (((int)cc->b + 1) * desc->color.b) >> 8;
             color->a = (((int)cc->a + 1) * desc->color.a) >> 8;
          }
        else
          {
             color->r = desc->color.r;
             color->g = desc->color.g;
             color->b = desc->color.b;
             color->a = desc->color.a;
          }
        ...
        if (ep->part->type == EDJE_PART_TYPE_TEXT ||
            ep->part->type == EDJE_PART_TYPE_TEXTBLOCK)
          {
             ...
             if (cc)
               {
                  color2->r = (((int)cc->r2 + 1) * text_desc->common.color2.r) >> 8;
                  color2->g = (((int)cc->g2 + 1) * text_desc->common.color2.g) >> 8;
                  color2->b = (((int)cc->b2 + 1) * text_desc->common.color2.b) >> 8;
                  color2->a = (((int)cc->a2 + 1) * text_desc->common.color2.a) >> 8;

                  color3->r = (((int)cc->r3 + 1) * text_desc->text.color3.r) >> 8;
                  color3->g = (((int)cc->g3 + 1) * text_desc->text.color3.g) >> 8;
                  color3->b = (((int)cc->b3 + 1) * text_desc->text.color3.b) >> 8;
                  color3->a = (((int)cc->a3 + 1) * text_desc->text.color3.a) >> 8;
               }
             else
               {
                  color2->r = text_desc->common.color2.r;
                  color2->g = text_desc->common.color2.g;
                  color2->b = text_desc->common.color2.b;
                  color2->a = text_desc->common.color2.a;

                  color3->r = text_desc->text.color3.r;
                  color3->g = text_desc->text.color3.g;
                  color3->b = text_desc->text.color3.b;
                  color3->a = text_desc->text.color3.a;
               }
          }
     }
           ...
           switch (effect & EDJE_TEXT_EFFECT_MASK_BASIC)
             {
              ...
              case EDJE_TEXT_EFFECT_SHADOW:
                 evas_object_text_shadow_color_set(ep->object,
                                                   (color3->r * color3->a) / 255,
                                                   (color3->g * color3->a) / 255,
                                                   (color3->b * color3->a) / 255,
                                                   color3->a);
                 break;
...
void
_edje_recalc_color(Edje *ed)
{
   unsigned int i;
   Eina_Bool recalc_all = EINA_FALSE;

   // if any part is not calculated or
   // the part is in the complex scenario between another part's colors,
   // for example map or image tweens, recalc all
   for (i = 0; i < ed->table_parts_size; i++)
     {
        Edje_Real_Part *ep;

        ep = ed->table_parts[i];
        if ((ep->calculated == FLAG_NONE) ||
            (ep->param1.description && ep->param1.description->map.on) ||
            ep->param2)
          {
             recalc_all = EINA_TRUE;
             break;
          }
     }
             ...
             if ((chosen_desc->text.min_x) || (chosen_desc->text.min_y))
               {
                  ...
                  if (!chosen_desc->text.min_x)
                    {
                       int min_width = 0;
                       if (*minw > params->eval.w)
                          min_width = *minw;
                       else
                          min_width = params->eval.w;

                       if (*maxh > 0)
                          evas_object_resize(ep->object, min_width, *maxh);
                       else
                          evas_object_resize(ep->object, min_width, params->eval.h);
        ...
        if ((chosen_desc->text.max_x) || (chosen_desc->text.max_y))
          {
             ...
             if (!chosen_desc->text.max_x)
               {
                  evas_object_resize(ep->object, params->eval.w, 99999);
                  evas_object_textblock_size_formatted_get(ep->object, &tw, &th);
               }
             else
               evas_object_textblock_size_native_get(ep->object, &tw, &th);
                                 ...
                                 int *maxw, int *maxh)
{
   *minw = desc->min.w;
   ...
   if ((ep->type == EDJE_RP_TYPE_SWALLOW) &&
       (ep->typedata.swallow))
     {
        //TIZEN_ONLY (20151014): Do not compare with un scaled value
...
#if defined(_TTRACE_EDJE_)
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_EDJE, "part_name(%s)", ep->part->name);
#endif
