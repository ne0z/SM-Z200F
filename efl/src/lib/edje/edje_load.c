...
_edje_object_file_set_internal(Evas_Object *obj, const Eina_File *file, const char *group, const char *parent, Eina_List *group_path, Eina_Array *nested)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_EDJE, "file(%s) group(%s)", eina_file_filename_get(file), group);
#endif
...
#if defined(_TTRACE_EDJE_)
               UPF_TRACE_BEGIN(UPF_TRACE_TAG_EDJE);
#endif
                              ...
                              if (rp->part->entry_mode > EDJE_ENTRY_EDIT_MODE_NONE)
                                {
                                   ...
                                   evas_object_textblock_editable_set(rp->object, EINA_TRUE);
                                   //
                                   if (!ed->has_entries)
