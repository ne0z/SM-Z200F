...
#ifdef EFL_OVERHEAD_LOG
extern unsigned char _evas_object_smart_recalculate_cycle_get(Evas_Object *eo_obj);
#endif
...
static Eina_Bool __edje_object_part_swallow(Evas_Object *obj EINA_UNUSED, Edje *ed, const char *part, Evas_Object *obj_swallow, Eina_Bool recalc);
static void __edje_object_part_unswallow(Evas_Object *obj EINA_UNUSED, Edje *ed, Evas_Object *obj_swallow, Eina_Bool recalc);
static Evas_Object* __edje_object_part_swallow_get(Evas_Object *obj EINA_UNUSED, Edje *ed, const char *part, Eina_Bool recalc);
static void __edje_user_definition_free2(Edje_User_Defined *eud, Eina_Bool recalc);
...
{
   __edje_user_definition_free2(eud, EINA_TRUE);
}

static void
__edje_user_definition_free2(Edje_User_Defined *eud, Eina_Bool recalc)
{
...
//TIZEN_ONLY: (20160602) Apply para direction to all edje text object
EAPI void
edje_text_paragraph_direction_apply()
{
   Eina_List *l;
   Evas_Object *obj;
   ...
   EINA_LIST_FOREACH(_edje_edjes, l, obj)
     {
        edje_object_text_direction_apply(obj);
     }
}
//
   ...
   if (members)  // TIZEN ONLY (20150209) Fix ERR log of eina_hash_iterator_data_new
     {
        ...
        EINA_ITERATOR_FOREACH(it, er)
          {
#ifdef EFL_FEATURE_CHANGEABLE_UI
             _edje_textblock_styles_cache_free(er->ed, color_class);
             _edje_textblock_style_all_update(er->ed);
             _edje_recalc_color(er->ed);
#else
...
update_color_class:
   ...
   if (parent == ed->obj)
        ...
        if (members)
          {
             ...
             EINA_ITERATOR_FOREACH(it, er)
               {
                  if (parent == evas_object_data_get(er->ed->obj, "color_overlay"))
                    {
                       _edje_textblock_styles_cache_free(er->ed, color_class);
                       _edje_textblock_style_all_update(er->ed);
                       _edje_recalc_color(er->ed);
   ...
   else
     {
        ...
        _edje_recalc_color(ed);
        _edje_emit(ed, "color_class,set", color_class);
     }
#else // EFL_FEATURE_CHANGEABLE_UI
   ed->dirty = EINA_TRUE;
...
// TIZEN ONLY(20150708): for genlist cache
Eina_List *
edje_object_color_class_list(Evas_Object *obj)
{
#ifdef EFL_FEATURE_CHANGEABLE_UI
   Edje *ed;
   Edje_List_Foreach_Data fdata;
   ...
   ed = _edje_fetch(obj);
   if ((!ed) || (!ed->color_classes)) return NULL;

   memset(&fdata, 0, sizeof(Edje_List_Foreach_Data));
   eina_hash_foreach(ed->color_classes,
                     _edje_color_class_list_foreach, &fdata);
        ...
        if (members)
          {
             ...
             EINA_ITERATOR_FOREACH(it, er)
               {
                  if (parent == evas_object_data_get(er->ed->obj, "font_overlay"))
                    {
                       er->ed->dirty = EINA_TRUE;
                       er->ed->recalc_call = EINA_TRUE;
#ifdef EDJE_CALC_CACHE
...
edje_object_text_class_del(Evas_Object *obj, const char *text_class)
{
   ...
   /* for each text_class in the object */
   EINA_LIST_FOREACH(ed->text_classes, l, tc)
     {
        if ((tc->name) && (!strcmp(tc->name, text_class)))
          {
             ed->text_classes = eina_list_remove_list(ed->text_classes, l);
             eina_stringshare_del(tc->name);
             eina_stringshare_del(tc->font);
             free(tc);
             break;
...
Eina_List *
edje_object_text_class_list(Evas_Object *obj)
{
#ifdef EFL_FEATURE_CHANGEABLE_UI
   Edje *ed;
   ...
   EINA_LIST_FOREACH(ed->text_classes, l, tc)
     {
        ret = eina_list_append(ret, eina_stringshare_add(tc->name));
     }
...
__edje_object_part_object_get(Eo *obj EINA_UNUSED, Edje *ed, const char *part, Eina_Bool recalc)
{
   Edje_Real_Part *rp;
...
const Evas_Object*
_edje_object_part_object_get2(Evas_Object *obj, const char *part)
{
   Edje *ed = _edje_fetch(obj);
   if (!ed) return NULL;

   return __edje_object_part_object_get(obj, ed, part, EINA_FALSE);
}

EOLIAN const Evas_Object*
_edje_object_part_object_get(Eo *obj EINA_UNUSED, Edje *ed, const char *part)
{
   return __edje_object_part_object_get(obj, ed, part, EINA_TRUE);
}
...
{
   return __edje_object_part_swallow(obj, ed, part, obj_swallow, EINA_TRUE);
}

Eina_Bool
_edje_object_part_swallow2(Evas_Object *obj, const char *part, Evas_Object *obj_swallow, Eina_Bool recalc)
{
   Edje *ed = _edje_fetch(obj);
   if (!ed) return EINA_FALSE;

   return __edje_object_part_swallow(obj, ed, part, obj_swallow, recalc);
}

static Eina_Bool
__edje_object_part_swallow(Evas_Object *obj EINA_UNUSED, Edje *ed, const char *part, Evas_Object *obj_swallow, Eina_Bool recalc)
{
...
{
   __edje_object_part_unswallow(obj, ed, obj_swallow, EINA_TRUE);
}

void
_edje_object_part_unswallow2(Evas_Object *obj EINA_UNUSED, Evas_Object *obj_swallow, Eina_Bool recalc)
{
   Edje *ed = _edje_fetch(obj);
   if (!ed) return;
   __edje_object_part_unswallow(obj, ed, obj_swallow, recalc);
}

static void
__edje_object_part_unswallow(Evas_Object *obj EINA_UNUSED, Edje *ed, Evas_Object *obj_swallow, Eina_Bool recalc)
{
   Edje_Real_Part *rp;
             ...
             else
               {
                  EINA_LIST_FOREACH(ed->user_defined, l, eud)
                    if (eud->type == EDJE_USER_SWALLOW && eud->u.swallow.child == obj_swallow)
                      {
                         __edje_user_definition_free2(eud, recalc);
                         return;
                      }
               }
          }
...
{
   return __edje_object_part_swallow_get(obj, ed, part, EINA_TRUE);
}

Evas_Object*
_edje_object_part_swallow_get2(Evas_Object *obj EINA_UNUSED, const char *part, Eina_Bool recalc)
{
   Edje *ed = _edje_fetch(obj);
   if (!ed) return NULL;

   return __edje_object_part_swallow_get(obj, ed, part, recalc);
}

static Evas_Object*
__edje_object_part_swallow_get(Evas_Object *obj EINA_UNUSED, Edje *ed, const char *part, Eina_Bool recalc)
{
   Edje_Real_Part *rp;
...
//TIZEN_ONLY: (20160602) Apply para direction to all edje text object
EOLIAN void
_edje_object_text_direction_apply(Eo *obj EINA_UNUSED, Edje *ed)
{
   if (!ed) return;
         ...
         if (ep->typedata.text)
           {
              if (ep->part->type == EDJE_PART_TYPE_TEXTBLOCK)
                {
                   text = evas_object_textblock_text_markup_get(ep->object);
                   if (text)
                     {
                ...
                {
                   text = evas_object_text_text_get(ep->object);
                   if (text)
                     {
...
_edje_object_size_min_restricted_calc(Eo *obj EINA_UNUSED, Edje *ed, Evas_Coord *minw, Evas_Coord *minh, Evas_Coord restrictedw, Evas_Coord restrictedh)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_EDJE, "%p", obj);
#endif // _TTRACE_EDJE_
...
#endif
#ifdef EFL_OVERHEAD_LOG
        if (EINA_LIKELY(print_log == EINA_FALSE))
          {
             if (EINA_UNLIKELY(_evas_object_smart_recalculate_cycle_get(ed->obj) > 240))
               print_log = EINA_TRUE;
          }
        else
          ERR("++ [obj=%p ed=%p path=%s group=%s cycle=%u]", ed->obj, ed, ed->path, ed->group, _evas_object_smart_recalculate_cycle_get(ed->obj));
#endif
        _edje_recalc_do(ed);
#ifdef EFL_OVERHEAD_LOG
        if (EINA_UNLIKELY(print_log == EINA_TRUE))
          ERR("cycle=%u parts_size=%u reset_maxwh=%d", _evas_object_smart_recalculate_cycle_get(ed->obj), ed->table_parts_size, reset_maxwh);
#endif
        if (reset_maxwh)
          {
             maxw = 0;
...
#ifdef EFL_OVERHEAD_LOG
                            if (EINA_UNLIKELY(print_log == EINA_TRUE))
                              ERR("[part=%s state=%s ep->w=%d ep-req.w=%d] okw=1 maxw=%d w=%d", ep->part->name, ep->chosen_description->state.name, ep->w, ep->req.w, maxw, w);
#endif
                            maxw = w;
                            okw = 1;
                            pep = ep;
...
#ifdef EFL_OVERHEAD_LOG
                                 if (EINA_UNLIKELY(print_log == EINA_TRUE))
                                   ERR("[part=%s state=%s ep->h=%d ep->req.h=%d] okh=1 maxh=%d h=%d", ep->part->name, ep->chosen_description->state.name, ep->h, ep->req.h, maxh, h);
#endif
                                 maxh = h;
                                 okh = 1;
                                 pep = ep;
