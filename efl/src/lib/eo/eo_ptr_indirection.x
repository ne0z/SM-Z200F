   ...
   if (_eo_ids_tables[mid_table_id] && TABLE_FROM_IDS)
     {
        ...
        if (!entry)
           ERR("eo_id 0x%x's entry is NULL", obj_id);
        if (entry && !entry->active)
           ERR("eo_id 0x%x's acvie is  NULL, entry: 0x%x, 0x%x, %d", obj_id, entry, entry->ptr, entry->active);
        if (entry && !(entry->generation == generation))
           ERR("eo_id 0x%x's generation is not matched, entry: 0x%x, 0x%x, %d, %d", obj_id, entry, entry->ptr, entry->generation, generation);
     }
