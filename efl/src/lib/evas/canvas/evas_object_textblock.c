...
// TIZEN ONLY (20150905) : Font effect for tizen.
#include <math.h>
...
#define _OLD_REPLACEMENT_CHAR 0xFFFC
...
#define _OLD_REPLACEMENT_CHAR_UTF8 "\xEF\xBF\xBC"
...
struct _Evas_Object_Textblock_Format
{
   ...
   // TIZEN_ONLY(20150908): Add pressed_color tag feature
   ...
   struct {
	...
	strikethrough,
  pressed_color; // TIZEN_ONLY(20150908): Add pressed_color tag feature
   } color;
   struct {
      int               l, r;
   ...
   // TIZEN_ONLY(20150908): Add pressed_color tag feature 474
...
static void
_textblock_mouse_down_cb(void *data EINA_UNUSED, Evas *e EINA_UNUSED, Evas_Object *obj, void *event_info)
{
   Evas_Coord x, y, w, h;
   ...
   Evas_Object_Protected_Data *temp = eo_data_scope_get(obj, EVAS_OBJECT_CLASS);
   Evas_Textblock_Data *o = (Evas_Textblock_Data*) temp->private_data;
...
static void
_textblock_mouse_up_cb(void *data EINA_UNUSED, Evas *e EINA_UNUSED, Evas_Object *obj, void *event_info EINA_UNUSED)
{
   Evas_Object_Protected_Data *temp = eo_data_scope_get(obj, EVAS_OBJECT_CLASS);
   Evas_Textblock_Data *o = (Evas_Textblock_Data*) temp->private_data;
   if (o->pressed_format)
...
_format_unref_free(const Evas_Object *eo_obj, Evas_Object_Textblock_Format *fmt)
{
   ...
   // TIZEN_ONLY(20150908): Add pressed_color tag feature
   Evas_Textblock_Data *o = (Evas_Textblock_Data*) obj->private_data;
   //
   fmt->ref--;
   if (fmt->ref > 0) return;
   ...
   // TIZEN_ONLY(20150908): Add pressed_color tag feature
   else if (cmd == pressed_colorstr)
     {
        ...
        if (!o->has_pressed_color)
          {
             evas_object_event_callback_add(eo_obj, EVAS_CALLBACK_MOUSE_DOWN, _textblock_mouse_down_cb, NULL);
             evas_object_event_callback_add(eo_obj, EVAS_CALLBACK_MOUSE_UP, _textblock_mouse_up_cb, NULL);
   ...
   if (par->text_node)
     {
        ...
        //TIZEN_ONLY (20150926): Special case for RTL language in LTR system
        Eina_Bool is_text_rtl = EINA_FALSE;
        size_t len = 0;
        //
        ...
        if (text && o->editable)
          {
             len = eina_ustrbuf_length_get(par->text_node->unicode);
             is_text_rtl = evas_bidi_is_rtl_str(text);
          }
        //
        switch (par_dir)
          {
           case EVAS_BIDI_DIRECTION_LTR:
              ...
              //TIZEN_ONLY (20150926): Special case for RTL language in LTR system
              if (o->editable && text && is_text_rtl && (len > 0))
                {
   ...
   if (c->align_auto && c->ln)
     {
        ...
        size_t len = 0;
        Eina_Bool is_text_rtl = EINA_FALSE;
        text = eina_ustrbuf_string_get(c->par->text_node->unicode);

        if (text && c->o->editable)
          {
             len = eina_ustrbuf_length_get(c->par->text_node->unicode);
             is_text_rtl = evas_bidi_is_rtl_str(text);
          }
        //
        ...
        if (par_dir == EVAS_BIDI_DIRECTION_LTR)
          {
             //TIZEN_ONLY (20150926): Special case for RTL language in LTR system
             if (c->o->editable && text && is_text_rtl && (len > 0))
               {
                  return 1.0;
               }
             //
...
_layout_calculate_format_item_size(const Evas_Object *eo_obj,
      ...
      Evas_Coord *maxascent, Evas_Coord *maxdescent EINA_UNUSED,
      Evas_Coord *_y, Evas_Coord *_w, Evas_Coord *_h)
{
   Evas_Object_Protected_Data *obj = eo_data_scope_get(eo_obj, EVAS_OBJECT_CLASS);
   /* Adjust sizes according to current line height/scale */

   //TIZEN_ONLY (20150902): Correct alignment of item
   //Evas_Coord w, h;
...
//TIZEN_ONLY (20151019): Use max asc/dsc for line height in case the line has one.
/*
static Evas_Coord
...
{
   Evas_Object_Textblock_Item *it = NULL,*last_it = NULL;
   Evas_Coord x = 0;
   //TIZEN_ONLY (20150917): Last asc/desc gets applied if line contains item before the wrap
   Eina_Bool have_item = EINA_FALSE;
   //
...
loop_advance:
   ...
   if (c->ascent > c->maxascent)
     c->ln->h += c->ascent;
   else
     c->ln->h += c->maxascent;

   if (c->descent > c->maxdescent)
     c->ln->h += c->descent;
   else
     c->ln->h += c->maxdescent;
   //
...
_layout_item_text_split_strip_white(Ctxt *c,
      Evas_Object_Textblock_Text_Item *ti, Eina_List *lti EINA_UNUSED, size_t cut)
{
   const Eina_Unicode *ts;
   Evas_Object_Textblock_Text_Item *new_ti = NULL, *white_ti = NULL;
   size_t cut_offset = cut;
   ...
   if (!IS_AT_END(ti, cut) && (ti->text_props.text_len > 0))
     {
        ...
        while ((cut_offset >= 0) && (!evas_common_text_props_split(&ti->text_props,
                                     &new_ti->text_props, cut_offset)))
          cut_offset--;
        new_ti->parent.text_pos = ti->parent.text_pos + cut_offset;
        // TIZEN_ONLY(20150905): Fix text disappear issue when text is made up with multiple items.
   ...
   if ((cut_offset >= 1) && _is_white(ts[cut_offset - 1]) && (ti->text_props.text_len > 0))
     {
        if (cut_offset - 1 > 0)
          {
             size_t white_cut = cut_offset - 1;
             ...
             while ((queue->start + queue->off) < (run_start + run_len))
               {
                  ...
                  // TIZEN_ONLY (20151105): Treat each cluster non glyph character to be one glyph
                  // as mid cluster tag set is not possible
                  // FIXME: Cannot find previous cluster example if found please handle here
                  if (!evas_common_text_props_split(&ti->text_props, &new_ti->text_props,
                                                    new_ti->parent.text_pos - ti->parent.text_pos))
                    {
                       int i = 1, pos = new_ti->parent.text_pos - ti->parent.text_pos;

                       while ((ti->parent.text_pos + pos + i < run_start + run_len) &&
                                (evas_common_text_props_index_find(&ti->text_props, pos + i) == -1))
                           i++;

                       new_ti->parent.text_pos += i;
          ...
          if (it->type == EVAS_TEXTBLOCK_ITEM_FORMAT)
            {
               ...
               if (_IS_LINE_SEPARATOR(fi->item) || _IS_PARAGRAPH_SEPARATOR_SIMPLE(fi->item))
                 {
                    int ascent = 0, descent = 0, maxasc = 0, maxdesc = 0;
                    _layout_item_ascent_descent_adjust(c->obj, &ascent, &descent, it, it->format);
					
                    if (c->position == TEXTBLOCK_POSITION_START)
                       _layout_item_max_ascent_descent_calc(c->obj, &maxasc, &maxdesc, it, TEXTBLOCK_POSITION_SINGLE);
                    else
                       _layout_item_max_ascent_descent_calc(c->obj, &maxasc, &maxdesc, it, TEXTBLOCK_POSITION_END);
...
evas_object_textblock_add(Evas *e)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_EVAS, "%p", e);
...
{
   if (o->markup_text != text && o->markup_text)
     {
        eina_stringshare_del(o->markup_text);
        o->markup_text = NULL;
     }
        ...
        for (;;)
          {
             ...
             else if (!strncmp(_OLD_REPLACEMENT_CHAR_UTF8, p,
                      text_len = strlen(_OLD_REPLACEMENT_CHAR_UTF8)) ||
   ...
   for (;;)
     {
        ...
        else if (ch == _OLD_REPLACEMENT_CHAR)
           // TIZEN_ONLY(20150811): Replace the replacement character to 0x00A0 which is empty character.
           //eina_strbuf_append(sbuf, "&#xfffc;");
           eina_strbuf_append(sbuf, "&#xa0;");
           //
...
// TIZEN_ONLY(20150803): Only space is considered for word selection
...
_evas_textblock_changed(Evas_Textblock_Data *o, Evas_Object *eo_obj)
{
   ...
   if (o->has_pressed_color)
     {
        evas_object_event_callback_del(eo_obj, EVAS_CALLBACK_MOUSE_DOWN, _textblock_mouse_down_cb);
        evas_object_event_callback_del(eo_obj, EVAS_CALLBACK_MOUSE_UP, _textblock_mouse_up_cb);
   ...
   if ((it1 == it2) && (it1->type == EVAS_TEXTBLOCK_ITEM_TEXT))
     {
        ...
        // TIZEN_ONLY: (20160511) Fix single RTL character selection
        if (x2 <= x1)
          {
             Evas_Coord tmp;
             tmp = x1;
        ...
        if (w > 0)
          {
             ...
             tr->y = ln->par->y + ln->y;
             tr->h = ln->h;
             //TIZEN_ONLY (20151125): Check for cursor pos before giving range width
             //tr->w = w;
             if (start == end)
               {
                  tr->w = 0;
   ...
   ITEM_WALK()
     {
        ...
        DRAW_FORMAT_CHECK_PRESSED(strikethrough, (ln->h / 2), line_thickness);
        //
        ...
        // TIZEN_ONLY(20150908): Add pressed_color tag feature
        DRAW_FORMAT_CHECK_PRESSED(underline, ln->baseline + ln->underline_position, ln->underline_thickness);
        //
        ...
        // TIZEN_ONLY(20150908): Add pressed_color tag feature
        DRAW_FORMAT_CHECK_PRESSED(underline2, ln->baseline + ln->underline_position + ln->underline_thickness +
              ln->underline_position, ln->underline_thickness);
        //
     }
   ITEM_WALK_END();
...
//Internal api
EAPI void
evas_object_textblock_editable_set(const Evas_Object *eo_obj, Eina_Bool editable)
{
   TB_HEAD();

   o = eo_data_scope_get(eo_obj, MY_CLASS);
   if (!o) return;
   if (o->editable == editable) return;
   o->editable = editable;
}
//
