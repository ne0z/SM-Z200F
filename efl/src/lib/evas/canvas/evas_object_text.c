...
static void
_layout_text_item_split(Evas_Object_Text_Item *ti, int offset, Evas_BiDi_Direction bidi_dir, double ellipsis)
{
   Evas_Text_Props new_text_props;
   int len = ti->text_props.len;

   memset(&new_text_props, 0, sizeof (new_text_props));
   while ((offset >= 0) && (!evas_common_text_props_split(&ti->text_props, &new_text_props, offset)))
      offset--;
   if (ellipsis == 0.0)
     {
 ...
 * discarded. */
static Eina_Bool
_layout_text_item_trim(Evas_Object_Protected_Data *obj, Evas_Text_Data *o, Evas_Object_Text_Item *ti, int idx)
{
   int len = (int) ti->text_props.text_len;
   int offset = 0;
   double ellipsis = -1.0;
   if (idx >= len)
      return EINA_FALSE;

   ellipsis = o->cur.ellipsis;
   if (o->bidi_dir == EVAS_BIDI_DIRECTION_RTL)
     {
        ellipsis = 1.0 - ellipsis;
     }
   offset = idx;
   _layout_text_item_split(ti, offset, ti->text_props.bidi_dir, ellipsis);
   ...
   //
     {
        ...
        /* Reorder before applying ellipsis */
        ...
        if (ellipsis == 1.0)
          ...
          {
             WRN("Ellipsis value %lf between 0.0 to 1.0 not defined, Refer doc!!!",o->cur.ellipsis);
          }
        ...
        if (start_ellip_it)
          {
             ...
             Evas_Coord ellipsis_coord = ellipsis * (width - ellip_frame);
             advance = 0;
             int cut = 1;
             // TIZEN_ONLY(20150905): Fix Evas Text truncated text case and evas_object_text_horiz_width_get() is added.
             ...
             if (itr && (itr != start_ellip_it))
               {
                 if (o->bidi_dir == EVAS_BIDI_DIRECTION_RTL)
                   {
                      ellipsis_coord = itr->adv - (ellipsis_coord - (advance + l + r));
                   }
                 else
                   {
                      ellipsis_coord = ellipsis_coord - (advance + l + r);
                   }
        ...
        if (end_ellip_it)
          {
             ...
             if (o->bidi_dir == EVAS_BIDI_DIRECTION_RTL)
               {
                  ellip_frame = itr->adv - (ellip_frame - (advance + l + r));
                  cut = 1;
               }
             else
               {
                  ellip_frame = ellip_frame - (advance + l + r);
                  cut = 0;
               }
             cut += ENFN->font_last_up_to_pos(ENDT,
                   o->font,
