...
static Eina_Bool _evas_event_callback_block = EINA_FALSE;
...
_eo_evas_object_cb(void *data, Eo *eo_obj, const Eo_Event_Description *desc EINA_UNUSED, void *event_info)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_EVAS, "%s, %d", func_string[0], info->type);
#endif // _TTRACE_EVAS_
...
_evas_post_event_callback_call(Evas *eo_e, Evas_Public_Data *e)
{
   ...
   if (_evas_event_callback_block == EINA_TRUE)
      return;
...
_Evas_Input_Debug_Cb _evas_input_debug_history_cb = NULL;

EAPI void
_evas_input_debug_cb_set(_Evas_Input_Debug_Cb cb)
{
   _evas_input_debug_history_cb = cb;
}


// samsung product only
EAPI Eina_Inlist*
_evas_object_callback_list_get(Evas_Object *eo_obj)
{
   Evas_Object_Protected_Data *obj = eo_data_scope_get(eo_obj, EVAS_OBJECT_CLASS);
   if (obj)
...
EAPI void
_evas_event_callback_call_block(Eina_Bool block)
{
   _evas_event_callback_block = block;
}
