...
#if defined(_DEBUG_SYMBOL)
# include <execinfo.h>
#endif // _DEBUG_SYMBOL

//#define _TTRACE_EVAS_
...
typedef struct
{
   ...
   char **func_string;
#endif // _DEBUG_SYMBOL
#if defined(_DEBUG_BACKTRACE)
   void *backtrace[256];
   char **backtrace_strings;
#endif //_DEBUG_BACKTRACE
...
#if defined(_TTRACE_EVAS_)
   void *bt_func[1] = { NULL, };
   bt_func[0] = info->func;
   char** func_string = backtrace_symbols((void **)bt_func, 1);

   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_EVAS, "%s, %s", func_string[0], info->event->name);
#endif // _TTRACE_EVAS_
...
_evas_object_smart_member_del(Eo *smart_obj, Evas_Smart_Data *_pd EINA_UNUSED, Evas_Object *eo_obj)
{
   ...
   if (e && e->proxy_event_in)
      e->proxy_event_in = eina_list_remove(e->proxy_event_in, eo_obj);
...
evas_object_smart_callback_priority_add(Evas_Object *eo_obj, const char *event, Evas_Callback_Priority priority, Evas_Smart_Cb func, const void *data)
{
...
#if defined(_DEBUG_SYMBOL)
   {
      void *bt_func[1] = { NULL, };
      bt_func[0] = func;
      cb_info->func_string = backtrace_symbols((void **)bt_func, 1);
   }

#endif // _DEBUG_SYMBOL
#if defined(_DEBUG_BACKTRACE)
   {
      int bt_len = backtrace((void **)cb_info->backtrace, 256);
      cb_info->backtrace_strings = backtrace_symbols((void **)cb_info->backtrace, bt_len);
   }
#endif // _DEBUG_BACKTRACE
}
        ...
        if ((info->func == func) && (info->event == eo_desc))
          {
             ...
             if (info->func_string)
               {
                  free(info->func_string);
                  info->func_string = NULL;
               }
#endif // _DEBUG_SYMBOL
#if defined(_DEBUG_BACKTRACE)
             if (info->backtrace_strings)
               {
                  free(info->backtrace_strings);
                  info->backtrace_strings = NULL;
               }
#endif // _DEBUG_BACKTRACE
...
EAPI unsigned char
_evas_object_smart_recalculate_cycle_get(Evas_Object *eo_obj)
{
   Evas_Smart_Data *o;
