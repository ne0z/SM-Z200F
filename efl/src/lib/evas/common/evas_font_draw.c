   ...
   EINA_INARRAY_FOREACH(glyphs->array, glyph)
     {
        ...
        // TIZEN_ONLY(20150923): there are two path to draw the font , but fg->ext_data point have only one for each.
        // ext_dat  should be pair with render backend (gl or sw )
        // In case of mismatch,  ext_data should be reset.
        if( fg->ext_dat)
         {
                 if (((fg->ext_dat_free == _evas_font_image_free)&& dc->font_ext.func.gl_image_new_from_data )  ||
                        ((fg->ext_dat_free != _evas_font_image_free)&& !dc->font_ext.func.gl_image_new_from_data ) )
                 {
