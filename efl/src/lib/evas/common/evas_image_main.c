...
_evas_common_rgba_image_ram_usage(Image_Entry *ie)
{
   ...
   /*For Tizen 2.4 issues.
	This below code is inproper for gl backend. im->image.data is freed after texture uploading.
	if the evas_object_image is created with mmap then Image_entry has a eina_file. and many Image_entry could be add to cache.
	this causes fd full. 
     */
   //if (im->image.data)
     {
