...
Eina_Bool __ecore_input_debug = EINA_FALSE;
                 ...
                 XKeyEvent *xevent)
{
   ...
   if (event == ECORE_EVENT_KEY_DOWN)
   {
      if (__ecore_input_debug == EINA_TRUE)
         ERR("Ecore KeyEvent:press time=%u keyname = %s key = %s keycode=%d", xevent->time, keyname, key, xevent->keycode);

      ERR("Ecore KeyEvent:press time=%u", xevent->time);
   }
   else if (event == ECORE_EVENT_KEY_UP)
   {
      if (__ecore_input_debug == EINA_TRUE)
         ERR("Ecore KeyEvent:release time=%u keyname = %s key = %s keycode=%d", xevent->time, keyname, key, xevent->keycode);

      ERR("Ecore KeyEvent:release time=%u", xevent->time);
   }
   else
      ERR("Ecore KeyEvent:invalid key");
...
_ecore_x_event_handle_button_press(XEvent *xevent)
{
   ...
   if (__ecore_input_debug == EINA_TRUE)
      ERR("ButtonEvent:press time=%u x=%d y=%d button=%d", (unsigned int)xevent->xbutton.time, (int)xevent->xbutton.x, (int)xevent->xbutton.y, xevent->xbutton.button);

   ERR("ButtonEvent:press time=%u", (unsigned int)xevent->xbutton.time);
...
_ecore_x_event_handle_button_release(XEvent *xevent)
{
   _ecore_x_last_event_mouse_move = 0;
    if (__ecore_input_debug == EINA_TRUE)
      ERR("ButtonEvent:release time=%u x=%d y=%d button=%d", (unsigned int)xevent->xbutton.time, (int)xevent->xbutton.x, (int)xevent->xbutton.y, xevent->xbutton.button);

   ERR("ButtonEvent:release time=%u", (unsigned int)xevent->xbutton.time);
 ...
 */
   if (__ecore_input_debug == EINA_TRUE)
      ERR("ButtonEvent:move time=%u x=%d y=%d", (unsigned int)xevent->xbutton.time, (int)xevent->xmotion.x, (int)xevent->xmotion.y);
...
_ecore_x_event_handle_enter_notify(XEvent *xevent)
{
   ...
   {
      if (__ecore_input_debug == EINA_TRUE)
         ERR("ButtonEvent:enter move time=%u x=%d y=%d", xevent->xcrossing.time, xevent->xcrossing.x, xevent->xcrossing.y);
...
_ecore_x_event_handle_leave_notify(XEvent *xevent)
{
   ...
   {
      if (__ecore_input_debug == EINA_TRUE)
         ERR("ButtonEvent:leave move time=%u x=%d y=%d", xevent->xcrossing.time, xevent->xcrossing.x, xevent->xcrossing.y);
