...
static Eina_Bool _tick_source_backup = EINA_FALSE;
...
struct _Ecore_Evas_Engine_Data_X11 {
   ...
   Eina_Bool manual_render_by_resize : 1; /**< mask this true if the manual render was set by resize */
   struct
     {
        int count;
        int front_b, back_b;
        ...
        unsigned int (*alloc_func) (void *data, Ecore_X_Pixmap parent, int w, int h, int depth);
...
static unsigned int
_ecore_evas_x_pixmap_alloc(void *data EINA_UNUSED, Ecore_X_Pixmap parent, int w, int h, int depth)
{
   return ecore_x_pixmap_new(parent, w, h, depth);
}

static void
_ecore_evas_x_pixmap_free(void *data EINA_UNUSED, unsigned int pixmap)
{
...
_ecore_evas_x_deiconify_job(void *data)
{
   ...
   if (!ecore_evas_manual_render_get(ee))
     {
        edata->manual_render_by_ecore_evas = EINA_TRUE;
        ecore_evas_manual_render_set(ee, EINA_TRUE);
     }

   // add a manual render idle enterer
   if (edata->manual_render_idle_enterer)
     ecore_idle_enterer_del(edata->manual_render_idle_enterer);

   // mark to send deiconify approve after manual rendering in idle enterer
   edata->deiconify_approve_after_manual_render = EINA_TRUE;
   edata->manual_render_by_resize = EINA_TRUE;

   /* queue an idle enterer to render manually until each object finishes
      changing its size */
   edata->manual_render_idle_enterer =
...
_ecore_evas_x_manual_render_idle_enterer_cb(void *data)
{
   ...
   edata->manual_render_idle_enterer = NULL;

   // first manual render
   ecore_evas_manual_render(ee);

   // second manual render
   if (edata->manual_render_by_resize)
     {
        ...
        ecore_evas_manual_render(ee);
        edata->manual_render_by_resize = EINA_FALSE;
     }

   // unset the manual_render only when ecore_evas set the manual render
   if (edata->manual_render_by_ecore_evas)
     {
        edata->manual_render_by_ecore_evas = EINA_FALSE;
        ecore_evas_manual_render_set(ee, EINA_FALSE);
     }

   // send deiconify approve after doing manual rending if it's set true
   if (edata->deiconify_approve_after_manual_render)
     {
        edata->deiconify_approve_after_manual_render = EINA_FALSE;
   ...
   if (einfo)
     {
        ...
        einfo->atom = ECORE_X_ATOM_E_WINDOW_ROTATION_TRANSFORM_HINT;
...
//                 (prev.prop.modal != ee->prop.modal) ||
   ...
   else if (e->atom == ECORE_X_ATOM_E_COMP_BUSY_NOTIFY)
     {
        int val = 0;
        INF("BUSY:-_ecore_evas_x window propertychange e->win(%u)", e->win);

        ecore_x_window_prop_card32_get(e->win, ECORE_X_ATOM_E_COMP_BUSY_NOTIFY, &val, 1);
        if (val == 1)
          {
             val = 0;
             ecore_x_window_prop_card32_set(e->win, ECORE_X_ATOM_E_COMP_BUSY_NOTIFY, &val, 1);
          }
     }
   ...
   if (e->fully_obscured)
     {
        ...
        if (ecore_animator_source_get() == ECORE_ANIMATOR_SOURCE_CUSTOM)
          {
             _tick_source_backup = EINA_TRUE;
             INF("tick source : false");
   ...
   else
     {
        ...
        if (_tick_source_backup)
          {
             INF("tick source : true");
             _vsync_animator_tick_source_set(ee, EINA_TRUE);
          }
     }
   return ECORE_CALLBACK_PASS_ON;
...
_ecore_evas_x_event_client_message(void *data EINA_UNUSED, int type EINA_UNUSED, void *event)
{
   ...
   if (e->message_type == ECORE_X_ATOM_E_COMP_SYNC_BEGIN)
   ...
   else if (e->message_type == ECORE_X_ATOM_E_DEICONIFY_APPROVE)
     {
        ...
        if (edata->deiconify_job)
          ecore_job_del(edata->deiconify_job);

        /* queue a deiconify job to give the chance to other jobs */
        edata->deiconify_job = ecore_job_add(_ecore_evas_x_deiconify_job,
                                             ee);
     }
...
_ecore_evas_x_event_window_configure(void *data EINA_UNUSED, int type EINA_UNUSED, void *event)
{
   ...
   if (ee->prop.iconified == 1 && e->w == 2 && e->h == 1) return ECORE_CALLBACK_PASS_ON; /* pass on event for 2x1 feature */
   ...
   if ((ee->w != e->w) || (ee->h != e->h) ||
       ...
       (ee->req.rotation != ee->rotation))
     {
        // Set manual render when ecore_evas is resized
        if (!ecore_evas_manual_render_get(ee) && !ee->prop.iconified)
          {
             edata->manual_render_by_ecore_evas = EINA_TRUE;
             ecore_evas_manual_render_set(ee, EINA_TRUE);
          }
...
_ecore_evas_x_event_window_show(void *data EINA_UNUSED, int type EINA_UNUSED, void *event)
{
   ...
   if (ee->gl_sync_draw_done != 0 && ee->gl_sync_draw_done != 1)
     {
...
_ecore_evas_x_free(Ecore_Evas *ee)
{
   ...
   for (i = 0; i < edata->pixmap.count; i++)
     {
        edata->pixmap.free_func(edata->pixmap.cbdata, edata->pixmap.b[i].pixmap);
        edata->pixmap.b[i].pixmap = 0;
     }
   ...
   if (!ee->prop.window)
     {
        ...
        if ((edata->pixmap.b[edata->pixmap.back_b].w != w) || (edata->pixmap.b[edata->pixmap.back_b].h != h))
          {
             /* free the backing pixmap */
             if (edata->pixmap.b[edata->pixmap.back_b].pixmap)
               {
                  edata->pixmap.free_func(edata->pixmap.cbdata, edata->pixmap.b[edata->pixmap.back_b].pixmap);
                  edata->pixmap.b[edata->pixmap.back_b].pixmap = 0;
               }
             edata->pixmap.b[edata->pixmap.back_b].w = 0;
             edata->pixmap.b[edata->pixmap.back_b].h = 0;
          }
     }
...
{
   char *id = NULL;
   Ecore_X_Atom atom_evas_win;
   Ecore_X_Atom atom_evas_gpu_win;
   ...
   if (!strcmp(ee->driver, "software_x11"))
   ...
   else if (!strcmp(ee->driver, "opengl_x11"))
     {
        ...
        if ((!ee->prop.window) && (edata->pixmap.count))
          {
             int i;
             einfo = (Evas_Engine_Info_GL_X11 *)evas_engine_info_get(ee->evas);
             if (!einfo) return;
             ...
             for (i = 0; i < edata->pixmap.count; i++)
               {
                  if (edata->pixmap.b[i].pixmap)
                    edata->pixmap.free_func(edata->pixmap.cbdata, edata->pixmap.b[i].pixmap);
                  edata->pixmap.b[i].pixmap =
                      (Ecore_X_Pixmap)edata->pixmap.alloc_func(edata->pixmap.cbdata, edata->win_root, ee->w, ee->h, edata->pixmap.depth);
                  edata->pixmap.b[i].w = ee->w;
                  edata->pixmap.b[i].h = ee->h;
               }
     ...
     //        ecore_x_window_shape_mask_set(ee->prop.window, 0);
             ...
             //to check if this window is opengl based / gpu based
             if (ee->prop.window)
               {
                  int val = 1;

                  //get the atom
                  atom_evas_gpu_win = ecore_x_atom_get("_E_APPPROFILE_WIN_IS_GPU");

                  if (atom_evas_gpu_win)
                    {
                       //Set ATOM to true
                       ecore_x_window_prop_card32_set(ee->prop.window, atom_evas_gpu_win, &val, 1);
                    }
               }
          }
     }

   //for checking if some non evas based windows
   if (ee->prop.window)
     {
        int val = 1;

        //get the atom
        atom_evas_win = ecore_x_atom_get("_E_COMP_BUSY_APP_NOTIFY_EVAS_WIN");

        if (atom_evas_win)
          {
             //Set ATOM to true.
             ecore_x_window_prop_card32_set(ee->prop.window, atom_evas_win, &val, 1);
          }
        ...
        if (einfo)
          {
              ...
              * size and depth. If the back buffer is not the proper size or depth, destroy it and
              * create a new one with the proper size and depth */
             if ((edata->pixmap.b[edata->pixmap.back_b].w != ee->w) ||
                 (edata->pixmap.b[edata->pixmap.back_b].h != ee->h) ||
                 (edata->pixmap.depth != einfo->info.depth))
               {
                  int i;
                  ...
                  for (i = 0; i < edata->pixmap.count; i++)
                    {
                       if (edata->pixmap.b[i].pixmap)
                         edata->pixmap.free_func(edata->pixmap.cbdata, edata->pixmap.b[i].pixmap);
                       edata->pixmap.b[i].pixmap =
                         (Ecore_X_Pixmap)edata->pixmap.alloc_func(edata->pixmap.cbdata, edata->win_root, ee->w, ee->h,
                                                                  edata->pixmap.depth);
                       edata->pixmap.b[i].w = ee->w;
                       edata->pixmap.b[i].h = ee->h;
                    }
               ...
               einfo->info.drawable = edata->pixmap.b[edata->pixmap.back_b].pixmap;
        ...
        if (einfo)
          {
             ...
             if ((edata->pixmap.b[edata->pixmap.back_b].w != ee->w) ||
               ...
               if (edata->pixmap.count == PIXMAP_DOUBLE)
                 {
                    einfo->info.drawable = edata->pixmap.b[edata->pixmap.back_b].pixmap;
                    einfo->info.drawable_back = edata->pixmap.b[edata->pixmap.front_b].pixmap;
                 }
               else
                 {
                    einfo->info.drawable = edata->pixmap.b[edata->pixmap.front_b].pixmap;
                 }
   ...
   if ((!ee->prop.window) && (edata->pixmap.count))
     {
        ...
        edata->pixmap.front_b = edata->pixmap.count - 1 - edata->pixmap.front_b;
        edata->pixmap.back_b = edata->pixmap.count - 1 - edata->pixmap.back_b;
...
# ifdef BUILD_ECORE_EVAS_OPENGL_X11
             ...
             if (einfo)
               {
                  einfo->info.offscreen = edata->pixmap.back_b;
...
ecore_evas_software_x11_pixmap_allocfunc_new(const char *disp_name, Ecore_X_Window parent,
                                            int x, int y, int w, int h,
                                            unsigned int (*alloc_func) (void *data, unsigned int parent, int w, int h, int depth),
                                            void (*free_func) (void *data, unsigned int pixmap), void *data)
{
   Evas_Engine_Info_Software_X11 *einfo;
   Ecore_Evas_Interface_X11 *iface;
   Ecore_Evas_Interface_Software_X11 *siface;
   ...
   if (einfo)
     {
        ...
        edata->pixmap.count = PIXMAP_DOUBLE;
        {
           char *c = getenv("ECORE_EVAS_PIXMAP_COUNT");
           if (c)
             edata->pixmap.count = atoi(c);
           if (edata->pixmap.count > NBUF)
             edata->pixmap.count = NBUF;
        }
...
# endif
        ...
        if (alloc_func)
          edata->pixmap.alloc_func = alloc_func;
        else
          edata->pixmap.alloc_func = _ecore_evas_x_pixmap_alloc;

        if (free_func)
          edata->pixmap.free_func = free_func;
        else
          edata->pixmap.free_func = _ecore_evas_x_pixmap_free;

        edata->pixmap.cbdata = data;

        for (i = 0; i < edata->pixmap.count; i++)
          {
             edata->pixmap.b[i].pixmap =
               (Ecore_X_Pixmap)edata->pixmap.alloc_func(edata->pixmap.cbdata, parent, w, h, edata->pixmap.depth);
             edata->pixmap.b[i].w = w;
             edata->pixmap.b[i].h = h;
          }

        if (edata->pixmap.count == PIXMAP_DOUBLE)
          {
             edata->pixmap.front_b = 1;
             edata->pixmap.back_b = 0;
             einfo->info.drawable = edata->pixmap.b[edata->pixmap.back_b].pixmap;
          }
        else
          {
             edata->pixmap.front_b = 0;
             edata->pixmap.back_b = 0;
             einfo->info.drawable = edata->pixmap.b[edata->pixmap.back_b].pixmap;
          }
...
_ecore_evas_software_x11_pixmap_get(const Ecore_Evas *ee)
{
   if (!(!strcmp(ee->driver, "software_x11"))) return 0;
   Ecore_Evas_Engine_Data_X11 *edata = ee->engine.data;
   return (Ecore_X_Pixmap) edata->pixmap.b[edata->pixmap.front_b].pixmap;
}
   ...
   if (einfo)
     {
        ...
        einfo->drawable_type = EVAS_ENGINE_GL_X11_DRAWABLE_TYPE_PIXMAP;
