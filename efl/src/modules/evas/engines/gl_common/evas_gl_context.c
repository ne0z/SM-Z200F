...
evas_gl_common_context_new(void)
{
   ...
   if (_evas_gl_common_context)
     {
        ...
        _evas_gl_common_viewport_set(_evas_gl_common_context, 1);
   ...
   gc->state.current.target = GL_TEXTURE_2D;
...
evas_gl_common_context_newframe(Evas_Engine_GL_Context *gc)
{
   ...
   if (dbgflushnum) ERR("----prev-flushnum: %i -----------------------------------", gc->flushnum);
//   fprintf(stderr, "------------------------\n");
   ...
   if (tex_only)
     {
        if (tex->pt->dyn.img)
          {
             if (tex->pt->dyn.target == GL_TEXTURE_EXTERNAL_OES)
               {
 ...
 */
        ...
        gc->state.current.target   = gc->pipe[i].shader.target;
...
//        gc->state.current.ch        = gc->pipe[i].shader.ch;
   ...
   if (dbgflushnum == 1)
     {
        if (pipe_done > 0) ERR("DONE (pipes): %i", pipe_done);
     }
