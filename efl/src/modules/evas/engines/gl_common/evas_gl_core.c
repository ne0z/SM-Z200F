...
static int
_surface_buffers_allocate(void *eng_data, EVGL_Surface *sfc, int w, int h, Evas_GL_Context_Version version)
{
   // Create buffers
   if (sfc->color_fmt)
     {
...
{
   if (sfc->egl_image && !sfc->native_img)
     {
        _egl_image_destroy(sfc->egl_image);
        sfc->egl_image = NULL;
     }
             ...
             if ((sfc->direct_fb_opt) &&
                 ...
                 (evgl_engine->funcs->native_win_prerotation_set))
               {
                  if (!evgl_engine->funcs->native_win_prerotation_set(eng_data))
                    DBG("Pre-rotation is not supported by the driver!");
               }
...
evgl_make_current(void *eng_data, EVGL_Surface *sfc, EVGL_Context *ctx)
{
   ...
   if ((rsc->current_ctx) && (rsc->current_ctx != ctx) && (rsc->current_ctx->partial_render))
     {
   ...
   if (!ctx->extension_checked)
     {
        if (!evgl_api_get(ctx->version, EINA_TRUE))
          {
...
Evas_GL_API *
evgl_api_get(Evas_GL_Context_Version version, Eina_Bool get_ext)
{
   ...
   if (version == EVAS_GL_GLES_2_X)
     {
        _evgl_api_get(gles2_funcs, evgl_engine->api_debug_mode, get_ext);
     }
   else if (version == EVAS_GL_GLES_1_X)
     {
        _evgl_api_gles1_get(gles1_funcs, evgl_engine->api_debug_mode, get_ext);
     }
   else if (version == EVAS_GL_GLES_3_X)
     {
        // Allocate gles3 funcs here, as this is called only if GLES_3 is supported
        _evgl_api_gles3_get(gles3_funcs, evgl_engine->api_debug_mode, get_ext);
     }
...
int
evgl_surface_is_texture(void *eng_data EINA_UNUSED, EVGL_Surface *sfc)
{
   EVGL_Resource *rsc;
   int is_texture = 1;
