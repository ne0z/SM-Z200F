        ...
        if (gc->pipe[i].array.im)
          {
             if (gc->pipe[i].array.im->tex->pt->dyn.img)
               gc->pipe[i].shader.target = gc->pipe[i].array.im->tex->pt->dyn.target;
             else
               gc->pipe[i].shader.target = gc->pipe[i].array.im->native.target;
          }
        else
          gc->pipe[i].shader.target = GL_TEXTURE_2D;

        if (gc->pipe[i].shader.target == GL_TEXTURE_EXTERNAL_OES) glBindTexture(GL_TEXTURE_2D, 0);
        else glBindTexture(GL_TEXTURE_EXTERNAL_OES, 0);

        glBindTexture(gc->pipe[i].shader.target, gc->pipe[i].shader.cur_tex);
...
_pool_tex_new(Evas_Engine_GL_Context *gc, int w, int h, GLenum intformat, GLenum format)
{
   ...
   glBindTexture(gc->state.current.target, gc->state.current.cur_tex);
   GLERR(__FUNCTION__, __FILE__, __LINE__, "");
   if (!ok)
     {
...
_pool_tex_dynamic_new(Evas_Engine_GL_Context *gc, int w, int h, int intformat, int format)
{
   ...
   if (gc->shared->info.sec_tbm_surface)
     pt->dyn.target = GL_TEXTURE_2D; //Use GL_TEXTURE_2D as multitexturing GL_TEXTURE_EXTERNAL_OES with GL_TEXTURE_2D does not work on adreno;
   else
...
evas_gl_common_texture_upload(Evas_GL_Texture *tex, RGBA_Image *im, unsigned int bytes_count)
{
   ...
   if (tex->pt->texture != tex->gc->state.current.cur_tex)
     {
        glBindTexture(tex->gc->state.current.target, tex->gc->state.current.cur_tex);
        GLERR(__FUNCTION__, __FILE__, __LINE__, "");
     }
}
...
on_error:
   glBindTexture(tex->gc->state.current.target, tex->gc->state.current.cur_tex); GLERRLOG();
}
