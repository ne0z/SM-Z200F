               ...
               int msaa_bits)
{
   ...
   gw->drawable_type = info->drawable_type;
...
// EGL / GLES
   ...
   if (gw->drawable_type == EVAS_ENGINE_GL_X11_DRAWABLE_TYPE_PIXMAP)
     {
...
void
eng_window_unsurf(Outbuf *gw, Eina_Bool pre_rotation_resurf)
{
   if (!gw->surf) return;
...
int eng_outbuf_get_age(Outbuf *ob)
{
   return ob->prev_age;
}
...
eng_outbuf_flush(Outbuf *ob, Tilebuf_Rect *rects, Evas_Render_Mode render_mode)
{
   ...
   if (ob->drawable_type == EVAS_ENGINE_GL_X11_DRAWABLE_TYPE_PIXMAP)
     {
        glFlush();
     }
...
void *eng_outbuf_display_get(Outbuf *ob)
{
   return ob->disp;
}
