...
static void _evas_x_window_prop_card32_set(unsigned int win, unsigned int atom, unsigned int *val, unsigned int num);
static int _evas_x_window_prop_card32_get(Display *disp, unsigned int win, unsigned int atom, unsigned int *val, unsigned int len);
static Eina_Bool _evas_x_win_check_prerotation_support(Drawable win, unsigned int atom);
static void _evas_x_win_rotation_transform_hint_set(Drawable win, int angle, unsigned int atom);
...
#define COMPOSITOR "enlightenment"
   ...
   static Eina_Bool is_compositor = EINA_FALSE;
   char app_name[128];
...
evgl_eng_native_buffer_image_create(void *data, int tex, int target, void *native)
{
   ...
   if (glsym_eglCreateImage)
     {
        surface = glsym_eglCreateImage(eng_get_ob(re)->egl_disp,
                                       EGL_NO_CONTEXT,
...
error:
   if (surface)
      if (glsym_eglDestroyImage)
         glsym_eglDestroyImage(eng_get_ob(re)->egl_disp, surface);
#else
   //_evgl_error_set(EVAS_GL_NOT_INITIALIZED);
#endif
...
evgl_eng_native_buffer_image_destroy(void *data, void *native_image)
{
   ...
   if (glsym_eglDestroyImage)
     {
        glsym_eglDestroyImage(eng_get_ob(re)->egl_disp, native_image);
        if ((err = eglGetError()) != EGL_SUCCESS)
          {
             ERR("eglDestroyImage() failed: %x.", err);
             //_evgl_error_set(err - EGL_SUCCESS);
          }
        return 1;
     }
...
evgl_eng_native_win_prerotation_set(void *data)
{
   Render_Engine *re = data;
   if (!re) return 0;
   if (!re->is_prerotate) return 0;
   if (eng_get_ob(re)->gl_context->pre_rotated) return 0;

   _evas_x_win_rotation_transform_hint_set(eng_get_ob(re)->info->info.drawable, eng_get_ob(re)->info->info.rotation, eng_get_ob(re)->atom);
    ...
    */
   eng_window_unsurf(eng_get_ob(re), EINA_TRUE);
   eng_window_resurf(eng_get_ob(re));
   ...
   if (str)
     {
        ...
        //Tizen Only end
        // Disable partial rendering on Qualcomm chipset
        else if (vendor && strstr(vendor, "Qualcomm"))
          {
             if (!getenv("EVAS_GL_PARTIAL_ENABLE"))
               {
                  extn_have_buffer_age = 0;
                 //As Qualcomm driver decide whether to skip frame or not in eglSwapBuffersWithDamage.
                 //So, we should use this, not enough eglSwapBuffers.
                 //glsym_eglSwapBuffersWithDamage = NULL;
               }
          }
        else
...
_re_winfree(Render_Engine *re)
{
   ...
   eng_window_unsurf(eng_get_ob(re), EINA_FALSE);
}
   ...
   if (!e->engine.data.output)
     {
        ...
        if (getenv("EVAS_GL_WIN_PREROTATION"))
          {
             _evas_x_disp = info->info.display; // for pre-rotation
             re->is_prerotate = EINA_TRUE; // Set true if env is set
             // Call transform hint set with PREROTATION_QUERY to check prerotation support by the driver
             _evas_x_win_rotation_transform_hint_set(info->info.drawable, -1, info->atom);
          }
        ...
        if (re->is_prerotate)
          {
             re->is_prerotate = _evas_x_win_check_prerotation_support(info->info.drawable,
                                                                      info->atom);
             _evas_x_win_rotation_transform_hint_set(info->info.drawable, 0, info->atom);
          }
        ...
        if (!evas_render_engine_gl_generic_init(&re->generic, ob,
                                                ...
                                                eng_gl_context_use,
                                                eng_outbuf_get_age,
                                                eng_outbuf_display_get,
                                                eng_native_config_check,
                                                &evgl_funcs,
                                                e->output.w, e->output.h))
          {
             eng_window_free(ob);
             ...
             if ((info->info.display != eng_get_ob(re)->disp) ||
                 ...
                 (info->info.destination_alpha != eng_get_ob(re)->alpha))
               {
                  ...
                  ob->gl_context->references--;
                      ...
                      (eng_get_ob(re)->gl_context->pre_rotated))
               {
                  if (re->is_prerotate && eng_get_ob(re)->gl_context->pre_rotated)
                    {
  ...
  if (n->ns.type == EVAS_NATIVE_SURFACE_X11)
    {
       ...
       if (n->surface)
         {
            if ((n->recreate) && (im->native.is_compositor == EINA_TRUE))
              {
                 if (glsym_eglDestroyImage)
                   {
                      glsym_eglDestroyImage(im->native.disp, n->surface);
                      if ((err = eglGetError()) != EGL_SUCCESS)
                        ERR("eglDestroyImage() failed. (error %x)", err);
                   }
                 else
                   ERR("Try eglDestroyImage on EGL with no support");
                 if (glsym_eglCreateImage)
                   {
                      n->surface = glsym_eglCreateImage(im->native.disp,
                                                        EGL_NO_CONTEXT,
                                                        EGL_NATIVE_PIXMAP_KHR,
                                                        (void *)n->pixmap,
                                                        NULL);
                      if (!n->surface)
                        ERR("eglCreateImage() for 0x%x failed. (error %x)", (unsigned int)n->pixmap, eglGetError());
                   }
                 else
                   ERR("Try eglCreateImage on EGL with no support");
              }
...
# ifdef GLX_BIND_TO_TEXTURE_TARGETS_EXT
      ...
      if (glsym_glXBindTexImage)
        {
           glsym_glXBindTexImage(im->native.disp, (XID)n->surface,
...
# ifdef GLX_BIND_TO_TEXTURE_TARGETS_EXT
       ...
       if (glsym_glXReleaseTexImage)
         {
            glsym_glXReleaseTexImage(im->native.disp, (XID)n->surface,
                                     GLX_FRONT_LEFT_EXT);
            GLERR(__FUNCTION__, __FILE__, __LINE__, "");
         }
       else
       ...
       if (n->surface)
         {
            ...
            if (glsym_glXDestroyPixmap)
              {
                 glsym_glXDestroyPixmap(im->native.disp, n->glx_pixmap);
                 GLERR(__FUNCTION__, __FILE__, __LINE__, "");
              }
            else
...
# endif
    ...
    {
       eina_hash_del(im->native.shared->native_tbm_hash, &n->buffer, im);
#ifdef GL_GLES
       if (n->surface)
         {
            if (glsym_eglDestroyImage)
...
#if GL_GLES
        if (extn_have_y_inverted &&
            eglGetConfigAttrib(im->native.disp, n->config,
                               EGL_Y_INVERTED_NOK, &val))
          yinvert = val;
#else
        glXGetFBConfigAttrib(im->native.disp, n->config,
          ...
          if (n)
            {
              ...
              if (!eglChooseConfig(eng_get_ob(re)->egl_disp, config_attrs,
                ...
                {
                  int err = eglGetError();
                  ERR("eglChooseConfig() failed for pixmap 0x%x, num_config = %i with error %x",
              ...
              if (glsym_eglCreateImage)
                ...
                ERR("Try eglCreateImage on EGL with no support");
              if (!n->surface)
                ERR("eglCreatePixmapSurface() for 0x%x failed (error %x)", (unsigned int)pm, eglGetError());
              ...
              im->native.is_compositor = re->is_compositor;
              im->native.data        = n;
            ...
            if (n)
              {
                 ...
                 if (glsym_eglCreateImage)
                   ...
                   ERR("Try eglCreateImage on EGL with no support");
                 if (!n->surface)
                   ERR("eglCreateImage() for %p failed (error %x)", buffer, eglGetError());
                 vendor = (const char *)glGetString(GL_VENDOR);
                 if (vendor && strstr (vendor, "Qualcomm") != NULL)
                   n->recreate = 1;
                 im->native.yinvert     = 1;
