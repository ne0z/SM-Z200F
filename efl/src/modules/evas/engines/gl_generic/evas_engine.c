   ...
   if (eng_gl_image_direct_get(data, image))
     {
        ...
        re->func.x = gl_context->dc->clip.x;
        re->func.y = gl_context->dc->clip.y;
        re->func.w = gl_context->dc->clip.w;
        re->func.h = gl_context->dc->clip.h;
...
eng_gl_proc_address_get(void *data EINA_UNUSED, Evas_GL_Ext *ext, const char *name)
{
   ...
   if (evgl_engine->funcs->proc_address_get)
     return evgl_engine->funcs->proc_address_get(name);
...
eng_gl_api_get(void *data, int version)
{
   ...
   ret = evgl_api_get(version, EINA_FALSE);
...
static Eina_Bool
eng_gl_surface_direct_renderable_get(void *data, Evas_Native_Surface *ns, Eina_Bool *direct_override, void *surface)
{
   Render_Engine_GL_Generic *re = data;
...
eng_gl_ext_buffer_age_get(void *data)
{
   ...
   return re->window_age_get(re->software.ob);
}
   ...
   if (re->func.clip)
     {
        ...
        *h = re->func.h;
        ...
        DBG(" Update Region: [%d %d %d %d]", *x, *y, *w, *h);
        return 1;
     }
   else
      return 0;
...
{
   int w = 0, h = 0;
   Render_Engine_GL_Generic *re = (Render_Engine_GL_Generic *)data;
   Evas_GL_Config *cfg = (Evas_GL_Config *)config;

   EVGLINIT(data, NULL);

   // Check the validity of the target and retrive width, height
   if (!re->window_native_config_check(re->window_display_get(re->software.ob), cfg, target, native, &w, &h))
     {
                ...
                void *surface, int x, int y, Eina_Bool do_async EINA_UNUSED)
{
   ...
   glim = eng_image_data_get(data, glim, EINA_TRUE, &pixels, &load_err);
