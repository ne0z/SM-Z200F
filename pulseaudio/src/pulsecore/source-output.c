...
static void pa_source_output_write_pcm_dump(pa_source_output *o, pa_memchunk *chunk)
{
    ...
    if (o->core->pcm_dump & PA_PCM_DUMP_PA_SOURCE_OUTPUT && !o->dump_fp && o->state == PA_SOURCE_OUTPUT_RUNNING) {
        pa_gettimeofday(&now);
...
#if defined (__TIZEN__) && defined (ENABLE_PCM_DUMP)
    if (o->state == PA_SOURCE_OUTPUT_RUNNING && o->dump_fp && (o->core->pcm_dump_option & PA_PCM_DUMP_OPTION_SEPARATED)) {
