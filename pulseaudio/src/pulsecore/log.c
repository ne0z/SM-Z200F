...
#endif
                    case PA_LOG_DEBUG:
                        SECURE_SLOG(LOG_DEBUG, DLOG_TAG, "%s%s%s%s", location, timestamp, t, pa_strempty(bt));
                        break;
                    case PA_LOG_INFO:
                    case PA_LOG_NOTICE: // no notice category in dlog, use info instead.
                        SECURE_SLOG(LOG_INFO, DLOG_TAG, "%s%s%s%s", location, timestamp, t, pa_strempty(bt));
                        break;
                    case PA_LOG_WARN:
                        SECURE_SLOG(LOG_WARN, DLOG_TAG, "%s%s%s%s", location, timestamp, t, pa_strempty(bt));
                        break;
                    case PA_LOG_ERROR:
                        SECURE_SLOG(LOG_ERROR, DLOG_TAG, "%s%s%s%s", location, timestamp, t, pa_strempty(bt));
                        break;
                    default:
                        SECURE_SLOG(LOG_DEBUG, DLOG_TAG, "%s%s%s%s", location, timestamp, t, pa_strempty(bt));
                        break;
...
void pa_log_level_secure(
    ...
    va_list ap;
    va_start(ap, format);
    pa_log_levelv_secure(level, file, line, func, format, ap);
    va_end(ap);
}
