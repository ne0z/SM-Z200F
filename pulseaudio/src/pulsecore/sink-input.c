...
int pa_sink_input_new(
    ...
    pa_secure_log_info("Created input %u \"%s\" (%s) on %s with sample spec %s and channel map %s\n",
                i->index,
...
static void sink_input_free(pa_object *o) {
    ...
    pa_secure_log_info("Freeing input %u \"%s\"", i->index, pa_strnull(pa_proplist_gets(i->proplist, PA_PROP_MEDIA_NAME)));
...
/* Called from main context */
void pa_sink_input_set_smooth_volume(pa_sink_input *i, const pa_cvolume *volume, int32_t duration_msec, pa_bool_t save, pa_bool_t absolute) {
    pa_cvolume v;
    ...
    if (pa_cvolume_equal(volume, &i->volume)) {
        ...
        pa_log_info("sink-input[%d] volume is same. skip to set volume", i->index);
        return;
    }
    ...
    if (pa_sink_flat_volume_enabled(i->sink)) {
        ...
        pa_assert_se(pa_asyncmsgq_send(i->sink->asyncmsgq, PA_MSGOBJECT(i), PA_SINK_INPUT_MESSAGE_SET_SOFT_VOLUME, duration_msec, 0, NULL) == 0);
    }
...
#ifdef __TIZEN__
/* Called from main context */
void pa_sink_input_set_smooth_mute(pa_sink_input *i, pa_bool_t mute, int32_t duration_msec, pa_bool_t save) {
    pa_sink_input_assert_ref(i);
    pa_assert_ctl_context();
    pa_assert(PA_SINK_INPUT_IS_LINKED(i->state));
    ...
    pa_atomic_store(&i->thread_info.fade_onff, true);
#endif
    pa_assert_se(pa_asyncmsgq_send(i->sink->asyncmsgq, PA_MSGOBJECT(i), PA_SINK_INPUT_MESSAGE_SET_SOFT_MUTE, duration_msec, 0, NULL) == 0);
...
static int __calc_smooth_volume(pa_sink_input* i, pa_cvolume* src, pa_cvolume* dest, int32_t duration_msec)
{
    int k = 0;
    uint32_t frame_size = 0;

    if (i == NULL) {
        pa_log_debug("sink-input is null.");
        return 0;
    }

    if (src == NULL || dest == NULL) {
        pa_log_debug("sink-input[%d] reset fade info", i->index);
        i->thread_info.amount_of_current = 0;
        i->thread_info.amount_of_duration = 0;
        i->thread_info.amount_of_period = 0;
        return 0;
    ...
    frame_size = pa_frame_size(&i->sample_spec);
    i->thread_info.amount_of_current = 0;
    i->thread_info.amount_of_duration = (pa_usec_to_bytes(duration_msec == 0 ? SMOOTH_VOLUME_DURATION : duration_msec*1000, &i->sample_spec) / frame_size) * frame_size;
    ...
    pa_log_info("sink-input[%d] start_vol(%d) -> end_vol(%d), ch(%d), diff(%d) duration_msec(%d), duration_amount(%d), freq(%d), angle(%f), keep(%d)",
        i->index, src->values[0], dest->values[0], i->sample_spec.channels, dest->values[0]-src->values[0],
        duration_msec, i->thread_info.amount_of_duration, i->thread_info.amount_of_period, i->thread_info.angle[0], i->thread_info.delta_volume[0]);
...
#if defined(__TIZEN__) && defined(PA_SMOOTH_VOLUME)
        /* reset fade information*/
        __calc_smooth_volume(i, NULL, NULL, 0);
        i->thread_info.muted = i->muted;
        i->thread_info.soft_volume = i->soft_volume;
#endif
    ...
    switch (code) {
        ...
        case PA_SINK_INPUT_MESSAGE_SET_SOFT_VOLUME:
            ...
            if (!pa_cvolume_equal(&i->thread_info.soft_volume, &i->soft_volume)) {
                pa_log_info("set smooth volume. sink-input[%d], fade_onoff(%d), duration(%d)", i->index, i->thread_info.fade_onff, userdata);

                if (pa_atomic_load(&i->thread_info.fade_onff) == false) {
                    i->thread_info.soft_volume = i->soft_volume;
                } else {
                    __calc_smooth_volume(i, &i->thread_info.soft_volume, &i->soft_volume, userdata);
                }
...
#if defined(__TIZEN__) && defined(PA_SMOOTH_VOLUME)
            {
                ...
                if (i->thread_info.muted != i->muted) {
                    if (pa_atomic_load(&i->thread_info.fade_onff) == false) {
                        ...
                        if (i->muted)
                            __calc_smooth_volume(i, &i->thread_info.soft_volume, &volume, userdata);
                        else {
                            i->thread_info.muted = i->muted;
                            __calc_smooth_volume(i, &volume, &i->thread_info.soft_volume, userdata);
                        }
                    }
                    pa_sink_input_request_rewind(i, 0, TRUE, FALSE, FALSE);
                }
...
void pa_sink_input_request_rewind(
    ...
    size_t lbq;
#ifdef __TIZEN__
    const char *skip_pcm_rewind_str = NULL;
    uint32_t skip_pcm_rewind = 0;
#endif
...
#ifdef __TIZEN__
    if ((skip_pcm_rewind_str = pa_proplist_gets(i->sink->proplist, PA_PROP_MEDIA_TIZEN_SKIP_PCM_REWIND))) {
        pa_atou(skip_pcm_rewind_str, &skip_pcm_rewind);
        if (skip_pcm_rewind) {
