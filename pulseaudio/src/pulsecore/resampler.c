...
#ifdef HAVE_SEC_SRC
#include <SoundAlive_SRC192_wrapper.h>
#endif
...
pa_resampler* pa_resampler_new(
    ...
    if ((method >= PA_RESAMPLER_SPEEX_FIXED_BASE && method <= PA_RESAMPLER_SPEEX_FIXED_MAX) ||
        ...
        if (r->map_required || a->format != b->format || method == PA_RESAMPLER_PEAKS) {
            ...
            if (a->format == PA_SAMPLE_S16NE || b->format == PA_SAMPLE_S16NE
                || a->format == PA_SAMPLE_S24_32NE || b->format ==  PA_SAMPLE_S24_32NE){
                if(a->format !=  b->format){
                    r->work_format = PA_SAMPLE_S16NE;
                }else{
                    r->work_format = a->format;
    ...
    if (r->o_ss.format != r->work_format) {
        if (r->work_format == PA_SAMPLE_FLOAT32NE) {
            ...
            if (r->o_ss.format == PA_SAMPLE_S24_32NE) {
               if (!(r->from_work_format_func = pa_get_convert_to_s24_32le_function(r->work_format)))
                   goto fail;
...
#ifdef HAVE_SEC_SRC
static int _secsrc_convert_bitwidth_code(pa_sample_format_t bitwidth)
{
    switch (bitwidth){
        case PA_SAMPLE_S16LE:       return  SOUNDALIVE_BITWIDTH_16;         break;
        case PA_SAMPLE_S24_32NE:    return  SOUNDALIVE_BITWIDTH_24;         break;
        default:                    return  SOUNDALIVE_BITWIDTH_16;         break;
    }
}
...
static void secsrc_resample(pa_resampler *r, const pa_memchunk *input, unsigned in_n_frames, pa_memchunk *output, unsigned *out_n_frames) {
    int ret = 0,i=0,v=0,j=0;
    int MASK = 0xff;
    short* data_in_16bit = NULL;
    short* data_out_16bit = NULL;

    int* data_in_24bit = NULL;
    int* data_out_24bit = NULL;
    ...
    data_in = (void*)((uint8_t*) pa_memblock_acquire(input->memblock) + input->index);
    /*Decoder ouput for 24 bit format is |0|a|b|c| MSB is not sign extended
      Resampler requires signed 32 bit data.So left shit and right by 8 bit for
      sign extension*/
    if(r->work_format  == PA_SAMPLE_S24_32NE)
    {
        data_in_24bit = (int *)data_in;
        for(i=0;i<in_n_frames * r->work_channels;i++)
            *(data_in_24bit + i) = ((*(data_in_24bit + i)) << 8 ) >> 8;
    }

   data_out = (void*)((uint8_t*) pa_memblock_acquire(output->memblock) + output->index);

   ret = SoundAlive_SRC(r->secsrc.state,data_out,data_in,in_n_frames);
    *out_n_frames = ret;
...
static int _secsrc_convert_samplerate_code (int samplerate)
{
    ...
    switch(samplerate)
    {
            case 192000:        code = SOUNDALIVE_SRC_192000k;      break;
            case 176400:        code = SOUNDALIVE_SRC_176400k;      break;
            case 128000:        code = SOUNDALIVE_SRC_128000k;      break;
            case   96000:       code = SOUNDALIVE_SRC_96000k;       break;
            case   88200:       code = SOUNDALIVE_SRC_88200k;       break;
            case   64000:       code = SOUNDALIVE_SRC_64000k;       break;
            case   48000:       code = SOUNDALIVE_SRC_48000k;       break;
            case   44100:       code = SOUNDALIVE_SRC_44100k;       break;
            case   32000:       code = SOUNDALIVE_SRC_32000k;       break;
            case   24000:       code = SOUNDALIVE_SRC_24000k;       break;
            case   22050:       code = SOUNDALIVE_SRC_22050k;       break;
            case   16000:       code = SOUNDALIVE_SRC_16000k;       break;
            case   12000:       code = SOUNDALIVE_SRC_12000k;       break;
            case   11025:       code = SOUNDALIVE_SRC_11025k;       break;
            case    8000:       code = SOUNDALIVE_SRC_8000k;        break;
            default:            code = SOUNDALIVE_SRC_8000k;        break;
    }
...
static void secsrc_reset(pa_resampler *r) {
    int samplerate_in = SOUNDALIVE_SRC_8000k;
    int samplerate_out = SOUNDALIVE_SRC_192000k;

    int bitwidth_in = SOUNDALIVE_BITWIDTH_16;
    int bitwidth_out = SOUNDALIVE_BITWIDTH_16;
    ...
    bitwidth_in = _secsrc_convert_bitwidth_code(r->work_format);
    bitwidth_out = _secsrc_convert_bitwidth_code(r->o_ss.format);

    if(r->secsrc.state != NULL) {
        SoundAlive_SRC_Reset(r->secsrc.state);
        r->secsrc.state = NULL;
    }

   if ((r->secsrc.state = SoundAlive_SRC_Init(samplerate_out, samplerate_in, r->work_channels, bitwidth_in, bitwidth_in)) == NULL) {
...
static int secsrc_init(pa_resampler *r) {
    ...
    /* Todo:SECSRC is capable of converting of format conversion,
	   Currently,format conversion is happening through resampler wrapper
	   need to optimize it. input & output of resampler will be in same format[S24_32NE or S16NE] */
    if ((r->secsrc.state = SoundAlive_SRC_Init(samplerate_out, samplerate_in, r->work_channels, bitwidth_in, bitwidth_in)) == NULL) {
        return -1;
    }
