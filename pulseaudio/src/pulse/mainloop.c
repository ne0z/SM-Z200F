...
#ifdef __TIZEN__
static void mainloop_dump_context(pa_mainloop_api *a, char* buffer, int size) {
    pa_mainloop *m;
    const char* m_state[5] = { "passive", "prepared", "polling", "polled", "quit" };

    m = a->userdata;

    snprintf(buffer, size, "n_enabled_defer_events(%d), n_enabled_time_events(%d), n_io_events(%d)\nio_events_please_scan(%d), time_events_please_scan(%d), defer_events_please_scan(%d)\nprepared_timeout(%llu), state(%s)",
        m->n_enabled_defer_events, m->n_enabled_time_events, m->n_io_events,
        m->io_events_please_scan, m->time_events_please_scan, m->defer_events_please_scan,
        m->prepared_timeout, m_state[m->state]);
}
#endif
...
static const pa_mainloop_api vtable = {
    ...
    .quit = mainloop_quit,
#ifdef __TIZEN__
    .dump_context = mainloop_dump_context,
#endif
