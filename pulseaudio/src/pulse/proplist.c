...
char *pa_proplist_to_string_sep(pa_proplist *p, const char *sep) {
...
#ifdef __TIZEN__
    if ((s_device_api_str = pa_proplist_gets(p, PA_PROP_DEVICE_API))) {
        if (pa_streq(s_device_api_str, "bluez")) {
            bluez = 1;
        }
    }
#endif
    ...
    while ((key = pa_proplist_iterate(p, &state))) {
        ...
        if ((v = pa_proplist_gets(p, key))) {
            const char *t;
#ifdef __TIZEN__
            if (bluez == 1 && (strstr(key, "device.string") || strstr(key, "bluez.path"))) {
                continue; /* no not print address or user info for security */
            }
#endif
