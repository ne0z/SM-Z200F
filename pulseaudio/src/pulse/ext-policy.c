...
static void ext_policy_volume_fade_by_type_cb(pa_pdispatch *pd, uint32_t command, uint32_t tag, pa_tagstruct *t, void *userdata)
{
    pa_operation *o = userdata;
    uint32_t ret = 0;
    ...
    if (command != PA_COMMAND_REPLY) {
        ...
        while (!pa_tagstruct_eof(t)) {
            if (pa_tagstruct_getu32(t, &ret) < 0) {
                pa_context_fail(o->context, PA_ERR_PROTOCOL);
                pa_log_error("get_mute : context fail");
                goto finish;
            }
    ...
    if (o->callback) {
        ext_policy_volume_fade_by_type_cb_t cb = (ext_policy_volume_fade_by_type_cb_t) o->callback;
        cb(o->context, ret, o->userdata);
    }
...
pa_operation *pa_ext_policy_volume_fade_by_type (
        pa_context *c,
        uint32_t volume_type,
        pa_tizen_fade_type_t up_down, // 0 : fadedown, 1:fadeup
        int duration, // msecs
        ext_policy_volume_fade_by_type_cb_t cb,
        void *userdata)
{
    uint32_t tag;
    pa_operation *o = NULL;
    ...
    pa_tagstruct_putu32(t, PA_INVALID_INDEX);
    pa_tagstruct_puts(t, "module-policy");
    pa_tagstruct_putu32(t, SUBCOMMAND_VOLUME_FADE_BY_TYPE);
    pa_tagstruct_putu32(t, volume_type);
    pa_tagstruct_putu32(t, up_down);
    pa_tagstruct_putu32(t, duration);

    pa_pstream_send_tagstruct(c->pstream, t);
    pa_pdispatch_register_reply(c->pdispatch, tag, DEFAULT_TIMEOUT, ext_policy_volume_fade_by_type_cb,
