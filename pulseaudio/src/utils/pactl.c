...
static pa_bool_t pcm_dump_type_enable, pcm_dump_option_enable;
#endif
...
#if defined (__TIZEN__) && defined (ENABLE_PCM_DUMP)
                case SET_PCM_DUMP:
                    pa_operation_unref(pa_context_set_pcm_dump(c, pcm_dump_type, pcm_dump_type_enable, simple_callback, NULL));
                    break;

                case SET_PCM_DUMP_OPTION:
                    pa_operation_unref(pa_context_set_pcm_dump_option(c, pcm_dump_option, pcm_dump_option_enable, simple_callback, NULL));
                    break;
#endif
...
int main(int argc, char *argv[]) {
    ...
    if (optind < argc) {
        if (pa_streq(argv[optind], "stat")) {
            ...
            if (argc == optind+2) {
                pcm_dump_vconf |= PA_PCM_DUMP_GST_DECODER_OUT | PA_PCM_DUMP_GST_SA_SB_IN | PA_PCM_DUMP_GST_SA_SB_OUT |
                                  PA_PCM_DUMP_GST_RESAMPLER_IN | PA_PCM_DUMP_GST_RESAMPLER_OUT | PA_PCM_DUMP_GST_DHA_IN |
                                  PA_PCM_DUMP_GST_DHA_OUT | PA_PCM_DUMP_GST_AUDIO_SINK_IN | PA_PCM_DUMP_PA_STREAM_WRITE;
                pcm_dump_type |= PA_PCM_DUMP_PA_SINK_INPUT | PA_PCM_DUMP_PA_SINK;
                pcm_dump_vconf |= PA_PCM_DUMP_PA_STREAM_READ | PA_PCM_DUMP_GST_AUDIO_SRC_OUT | PA_PCM_DUMP_GST_SEC_RECORD_IN |
                                  PA_PCM_DUMP_GST_SEC_RECORD_OUT | PA_PCM_DUMP_GST_ENCODER_IN;
                ...
                for (i = optind+1; i < argc-1; i++) {
                    ...
                    } else if (pa_streq(argv[i], "sa-sb-in")) {
                        pcm_dump_vconf |= PA_PCM_DUMP_GST_SA_SB_IN;
                    } else if (pa_streq(argv[i], "sa-sb-out")) {
                        pcm_dump_vconf |= PA_PCM_DUMP_GST_SA_SB_OUT;
                    ...
                    } else if (pa_streq(argv[i], "dha-in")) {
                        pcm_dump_vconf |= PA_PCM_DUMP_GST_DHA_IN;
                    } else if (pa_streq(argv[i], "dha-out")) {
                        pcm_dump_vconf |= PA_PCM_DUMP_GST_DHA_OUT;
                    ...
                    } else if (pa_streq(argv[i], "sec-record-in")) {
                        pcm_dump_vconf |= PA_PCM_DUMP_GST_SEC_RECORD_IN;
                    } else if (pa_streq(argv[i], "sec-record-out")) {
                        pcm_dump_vconf |= PA_PCM_DUMP_GST_SEC_RECORD_OUT;
                    } else if (pa_streq(argv[i], "encoder-in")) {
                        pcm_dump_vconf |= PA_PCM_DUMP_GST_ENCODER_IN;
                    } else {
                        pa_log(_("Specify nothing, or one of: %s"), "playback, decoder-out, sa-sb-in, sa-sb-out, resampler-in, resampler-out, "
                            "dha-in, dha-out, sink-input, sink, capture, source, source-output, stream-read, gst-audio-src, sec-record-in, "
                            "sec-record-out, encoder-in");
                    }
                }
            }
