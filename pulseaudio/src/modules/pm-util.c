...
#ifdef DISPLAY_LOCK_SYNC
static int display_lock_state(const char *state, const char *flag, unsigned int timeout)
{
    DBusError err;
    DBusMessage *msg;
    char *pa[4];
...
/*Async functions are added because sync functions take about 600ms sometimes*/
static int display_lock_state_async(const char *state, const char *flag, unsigned int timeout)
{
    char *pa[4];
    char str_timeout[32];
    int ret = 0, val = 0;
...
#ifdef DISPLAY_LOCK_SYNC
static int display_unlock_state(const char *state, const char *flag)
{
    DBusError err;
    DBusMessage *msg;
    char *pa[2];
...
/*Async functions are added because sync functions take about 600ms sometimes*/
static int display_unlock_state_async(const char *state, const char *flag)
{
    char *pa[2];
    int ret = 0, val = 0;
