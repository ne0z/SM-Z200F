...
#ifdef __TIZEN__
        if (u->source->voip_device)
        {
            size_t actual_n_bytes;
            actual_n_bytes = (u->source->sample_spec.rate == VOIP_WIDE_BAND) ? u->fragment_size :VOIP_MIN_NB_PERIOD_BYTES;
            ...
            switch ((pa_source_state_t) PA_PTR_TO_UINT(data)) {
                ...
                case PA_SOURCE_SUSPENDED: {
                    ...
                    if (!PA_SOURCE_IS_OPENED(u->source->thread_info.state)) {
                        pa_log_warn("source was already closed sometimes because of close_all_devices");
                        break;
                    }
#else
