...
/*             pa_log_debug("%lu frames to write", (unsigned long) frames); */
...
#elif TIZEN_DEBUG_ENABLE
            {
                ...
                if(u->log_count == 0 || u->log_count > interval) {
                    pa_log_info("ALSA-SINK(mmap) : Wrote %lu bytes (of possible %lu bytes), total(%llu)", (unsigned long) written, (unsigned long) n_bytes, u->write_count);
...
static int unix_write(struct userdata *u, pa_usec_t *sleep_usec, pa_bool_t polled, pa_bool_t on_timeout) {
    ...
    for (;;) {
        ...
        on_timeout = FALSE;
#ifdef __TIZEN__
        if (u->sink->voip_device) {
            size_t actual_n_bytes;
            actual_n_bytes = (u->sink->sample_spec.rate == VOIP_WIDE_BAND) ? u->fragment_size :VOIP_MIN_NB_PERIOD_BYTES;
...
/*         pa_log_debug("%lu frames to write", (unsigned long) frames); */
...
#elif TIZEN_DEBUG_ENABLE
            {
                const uint32_t interval = 35;
                if(u->log_count == 0 || u->log_count > interval) {
                    pa_log_debug("ALSA-SINK(unix) : Wrote %lu bytes (of possible %lu bytes), total(%llu)", (unsigned long) written, (unsigned long) n_bytes, u->write_count);
                    if(u->log_count > interval)
                        u->log_count = 0;
                }
                u->log_count++;
...
#ifdef __TIZEN__
    if (audio_intf && audio_intf->update_audio_effect) {
        audio_return_t audio_ret = AUDIO_RET_OK;

        if (AUDIO_IS_ERROR((audio_ret = audio_intf->update_audio_effect(audio_data, (void *)u->pcm_handle,device_name ? device_name : u->device_name)))) {
            pa_log("Error updating audio effect %x", audio_ret);
        }
    }
#endif
        ...
        case PA_SINK_MESSAGE_SET_STATE:
            ...
            switch ((pa_sink_state_t) PA_PTR_TO_UINT(data)) {
                ...
                case PA_SINK_SUSPENDED: {
                    ...
                    if (!PA_SINK_IS_OPENED(u->sink->thread_info.state)) {
                        pa_log_warn("sink was already closed sometimes because of close_all_devices");
                        break;
                    }
#else
...
#ifdef __TIZEN__
            {
                ...
                if(audio_intf && audio_intf->device_state_change) {
                    const char *device_idx_str = pa_proplist_gets(u->sink->proplist, "alsa.device");
                    audio_intf->device_state_change(audio_data, device_idx_str, u->sink->thread_info.state, (pa_sink_state_t) PA_PTR_TO_UINT(data), NULL);
                }
            }
#endif
...
pa_sink *pa_alsa_sink_new(pa_module *m, pa_modargs *ma, const char*driver, pa_card *card, pa_alsa_mapping *mapping) {
    ...
    if (pa_modargs_get_value_u32(ma, "voip_device",
                                 &u->sink->voip_device) < 0) {
        pa_log("Failed to parse voip_device parameter");
    }
