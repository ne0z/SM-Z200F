...
#ifdef __TIZEN__
    pa_time_event *time_event_stable_run;
#endif
};
...
{
    int ret = -1;
    pa_strbuf *before = NULL, *after = NULL;
    char *b = NULL, *a = NULL;
...
#ifdef __TIZEN__
static void stable_run_timeout_cb(pa_mainloop_api*a, pa_time_event* e, const struct timeval *t, void *userdata)
{
    struct device_info *d = userdata;

    if (!userdata) {
        pa_log_error("userdata is not available. userdata(%x)", userdata);
        return;
    }

    void *audio_data = pa_shared_get(d->userdata->core, "tizen-audio-data");
    audio_interface_t *audio_intf = pa_shared_get(d->userdata->core, "tizen-audio-interface");

    if (d->sink) {
        if(audio_intf && audio_intf->device_state_change) {
            const char *device_idx_str = pa_proplist_gets(d->sink->proplist, "alsa.device");
            audio_intf->device_state_change(audio_data, device_idx_str, PA_SINK_INPUT_RUNNING, 3/* SINK_RUNNING_STABLE hard code in hal*/, NULL);
        }
    }
}
#endif
...
static void restart(struct device_info *d, int input_timeout) {
    ...
    if (input_timeout != USE_DEVICE_DEFAULT_TIMEOUT) {
...
#ifdef __TIZEN__
    /* Issue : pre-listening sound is played slowly. we don't want to open pcm device before because changing UCM to dual path.  */
    if  (data->flags & PA_SINK_INPUT_START_CORKED) {
        const char *all_policy = "all";
        char *policy = pa_proplist_gets(data->proplist, PA_PROP_MEDIA_POLICY);

        if (policy != NULL) {
            int min = PA_MIN(strlen(policy), strlen(all_policy));
            if (!strncmp(policy, all_policy, min)) {
                u->req_ignore_timeout = TRUE;
            }
        }

        pa_log_info("we skip to unsuspend sink because of ucm change(dual path). req_ignore_timeout(%d)", u->req_ignore_timeout);
        return PA_HOOK_OK;
    }
#endif
...
#ifdef __TIZEN__
    d->time_event_stable_run = pa_core_rttime_new(c, PA_USEC_INVALID, stable_run_timeout_cb, d);
#endif
...
#ifdef __TIZEN__
    d->userdata->core->mainloop->time_free(d->time_event_stable_run);
#endif
    pa_xfree(d);
}
...
static pa_hook_result_t device_state_changed_hook_cb(pa_core *c, pa_object *o, struct userdata *u) {
    ...
    if (pa_sink_isinstance(o)) {
...
#ifdef __TIZEN__
                if (u->req_ignore_timeout) {
                    pa_log_error("requested to ignore timeout. close devices immediately.");
                    u->req_ignore_timeout = FALSE;
                    restart(d, 0);
                } else {
                    restart(d, USE_DEVICE_DEFAULT_TIMEOUT);
                }
        ...
        if(state == PA_SINK_RUNNING) {
            ...
            pa_usec_t diff = 60ULL * PA_USEC_PER_MSEC;
            pa_core_rttime_restart(d->userdata->core, d->time_event_stable_run, now + diff);
        }
#endif
