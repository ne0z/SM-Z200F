...
//#define DASH_STREAM_DUMP_CODE

//[prasenjit.c] Need to be enabled for Dash Stream Dumping for debugging purpose
#ifdef DASH_STREAM_DUMP_CODE
#include <stdio.h>
#include <errno.h>
#include <math.h>
#include <gst/gst.h>
...
#define MAX_DOWNLOAD_ERROR_COUNT 10
#define DEFAULT_FAILED_COUNT 10
...
#ifdef GST_ADAPTIVE_OPENSOURCE_MODIFICATION
static GstFlowReturn
...
#ifdef GST_ADAPTIVE_MODIFICATION
  demux->stop_download_task_ongoing=FALSE;
  demux->wait=FALSE;
  demux->stop_request_from_reset = FALSE;
#endif
    ...
    gpointer data)
{
  ...
  if (value
    ...
    guint i = 0;
    gchar **cookies =
      (gchar **) g_malloc0 ((gst_value_array_get_size (value) + 1) * sizeof (gchar *));
...
#ifdef GST_BAD_EXTRA_DEBUG_LOG
#ifndef GST_BAD_SECURE_LOG
...
gst_adaptive_demux_reset (GstAdaptiveDemux * demux)
{
...
#ifdef GST_ADAPTIVE_MODIFICATION
  demux->stop_request_from_reset = TRUE;
#endif
  ...
  if (demux->wait == TRUE)
  {
	GST_DEBUG_OBJECT (demux, "gst_task_join end, starting pending gst_task_start");
...
#ifdef GST_BAD_EXTRA_DEBUG_LOG
  GST_WARNING_OBJECT (demux, "Adding srcpad %s:%s with caps %" GST_PTR_FORMAT,
      GST_DEBUG_PAD_NAME (pad), stream->pending_caps);
#else
  GST_DEBUG_OBJECT (demux, "Adding srcpad %s:%s with caps %" GST_PTR_FORMAT,
      GST_DEBUG_PAD_NAME (pad), stream->pending_caps);
...
#endif
#ifndef GST_ADAPTIVE_OPENSOURCE_MODIFICATION
...
#ifdef GST_ADAPTIVE_MODIFICATION
    is_task_join_success = gst_task_join (stream->download_task);
    if(is_task_join_success == FALSE)
    {
      GST_ERROR_OBJECT (demux, "gst_task_join Failed !!!");
    }
#else
...
#ifdef GST_ADAPTIVE_MODIFICATION
/* [Shweta.agg]Sometimes gst_task_start of new task is getting called before gst_task_join of
   previous task which leads to crash */
 if(demux->stop_download_task_ongoing)
 {
	GST_DEBUG_OBJECT (demux, "waiting for the success of gst_task_join");
	demux->wait=TRUE;
 }
 else
 {
...
gst_adaptive_demux_stop_tasks (GstAdaptiveDemux * demux)
{
...
#ifdef GST_ADAPTIVE_MODIFICATION
  gboolean is_task_join_success = FALSE;
  demux->stop_download_task_ongoing=TRUE;
#endif
  ...
  if (stream->download_task != NULL)
  {
	...
	if(is_task_join_success == FALSE)
	{
		GST_ERROR_OBJECT (demux, "gst_task_join failed .. calling stop instead !!!");
		gst_task_stop(stream->download_task);
	}
...
#ifdef GST_ADAPTIVE_MODIFICATION
  if (demux->stop_request_from_reset == FALSE)
  {
	  if (demux->wait == TRUE)
	  {
		GST_DEBUG_OBJECT (demux, "gst_task_join end, starting pending gst_task_start");
    ...
    GstBuffer * buffer)
{
  ...
  if (G_UNLIKELY (stream->pending_caps)) {
    ...
    if (gst_adaptive_demux_is_live (demux))
    {
	GST_DEBUG_OBJECT (stream->pad, "Live Stream case !!");
...
//[prasenjit.c] Need to be enabled for Dash Stream Dumping for debugging purpose
  ...
  if (stream->fragment.uri)
  {
	...
	index2 = (int)strlen(stream->fragment.uri);
	s1++;
	strncpy(tmp, s1, index2-index1-1);
	tmp[index2-index1-1]='\0';
	GST_DEBUG_OBJECT (demux, "Buffer Size = %d ",map.size);
	GST_DEBUG_OBJECT (demux, "Dash file name=%s",tmp);

	if (strstr(tmp,"m4s"))
	{
		tmp_ext = strstr(tmp,"_");
		sprintf(buf, "/opt/usr/media/Others/Concatenated_File%s",tmp_ext);
	}
	else
	{
		tmp_ext = strstr(tmp,".");
	...
	if (map.data != NULL)
	{
		...
		if (f != NULL)
		{
			fwrite((unsigned char*)map.data , 1 ,(gsize)map.size , f);
			fclose(f);
		}
		else
			GST_DEBUG_OBJECT (demux, "fopen failed !!!");
	}
	else
		GST_DEBUG_OBJECT (demux, "INVALID map.data !!!");
...
#ifdef GST_ADAPTIVE_MODIFICATION
/*
[prasenjit.c] For JioVOD and Hotstar AES Encrypted HLS Contents , when we launch any playback and immedietly press back key ,
KeyURI download was failing and it was causing Mutex Lock for Manifest File.
Handling the error condition by throwing Error in such cases.

Changes are part of Open Source Modification done via :
https://cgit.freedesktop.org/gstreamer/gst-plugins-bad/commit/gst-libs/gst/adaptivedemux/gstadaptivedemux.c?id=64adb1998db4ddc6b39f33f82a5fdbc370c4e772
*/
    if (klass->start_fragment) {
      if (!klass->start_fragment (demux, stream)) {
        GST_WARNING_OBJECT (stream->pad, "Starting Encrypted Fragment Failed due to Stream Cancel !!!");
        ret = GST_FLOW_ERROR;
        return ret;
      }
    }
...
#ifdef GST_BAD_EXTRA_DEBUG_LOG
      GST_ERROR_OBJECT (stream->pad, "stream stopped, reason %s",
          gst_flow_get_name (ret));
#else
...
#ifdef GST_BAD_EXTRA_DEBUG_LOG
  GST_WARNING_OBJECT (stream->pad, "Download finish: %d %s - err: %p", ret,
...
_src_event (GstPad * pad, GstObject * parent, GstEvent * event)
{
  ...
  if ((caps_str)&&(strstr(caps_str,"dash_stream"))&&(gst_adaptive_demux_is_live (stream->demux))&&(stream->demux->dash_error_count > 0))
  {
    ...
    GstAdaptiveDemuxStream * stream)
{
  ...
  while (TRUE) {
    ...
    if (gst_adaptive_demux_stream_has_next_fragment (demux, stream)) {
      ...
      ret = TRUE;
#ifdef GST_ADAPTIVE_MODIFICATION
      stream->download_current_sequence = FALSE;
#endif
      break;
    }
    ...
    gint64 end)
{
  GstFlowReturn ret = GST_FLOW_OK;
#ifdef GST_BAD_EXTRA_DEBUG_LOG
#ifdef GST_BAD_SECURE_LOG
  GST_WARNING_OBJECT (stream->pad, "Downloading uri with range:%" G_GINT64_FORMAT
      " - %" G_GINT64_FORMAT, start, end);
#else
  GST_WARNING_OBJECT (stream->pad, "Downloading uri: %s, range:%" G_GINT64_FORMAT
...
#ifdef GST_BAD_EXTRA_DEBUG_LOG
    GST_WARNING_OBJECT (stream->pad, "Fragment download result: %d %s",
...
gst_adaptive_demux_stream_download_loop (GstAdaptiveDemuxStream * stream)
{
  ...
  switch (ret) {
    ...
    case GST_FLOW_EOS:
      ...
      if (gst_adaptive_demux_is_live (demux)) {
#ifdef GST_ADAPTIVE_MODIFICATION
        stream->download_current_sequence = TRUE;
#endif
