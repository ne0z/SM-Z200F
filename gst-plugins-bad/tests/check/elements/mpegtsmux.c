...
static void
check_wfd_tsmux_pad (GstStaticPadTemplate * srctemplate,
    const gchar * src_caps_string, gint pes_id, gint pmt_id,
    const gchar * sinkname)
{
  GstElement *mux;
  ...
  for (i = 0; i < num_buffers; ++i) {
    ...
    for (; size; odata += 188, size -= 188) {
      ...
      if (pid == 0) {
        ...
        data += 2;
        el_pid = y & 0x1FFF;
        fail_unless (el_pid >= 0x1011 && el_pid < 0x1FFF);
      } else if (pid == el_pid) {
        packets++;
        /* expect to see a PES packet start */
        y = GST_READ_UINT32_BE (data);
...
GST_START_TEST (test_wifi_display)
{
  check_wfd_tsmux_pad (&video_src_template, VIDEO_CAPS_STRING, 0xE0, 0x1b,
      "sink_4113");
}
...
mpegtsmux_suite (void)
{
  ...
  tcase_add_test (tc_chain, test_wifi_display);
