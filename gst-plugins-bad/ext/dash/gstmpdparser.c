    ...
    guint indexStream, GstMediaFragmentInfo * fragment)
{
  ...
  if (stream->segments) {
    ...
    if (stream->cur_seg_template != NULL) {
      GST_DEBUG ("segment_index = %d , startnumber = %d, duration = %"GST_TIME_FORMAT,stream->segment_index,stream->cur_seg_template->MultSegBaseType->startNumber,GST_TIME_ARGS(fragment->duration));
      mediaURL =
          gst_mpdparser_build_URL_from_template (stream->
          cur_seg_template->media, stream->cur_representation->id,
