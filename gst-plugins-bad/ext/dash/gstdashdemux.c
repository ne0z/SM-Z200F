...
#ifdef GST_DASHDEMUX_OPENSOURCE_MODIFICATION
...
#ifdef GST_DASHDEMUX_OPENSOURCE_MODIFICATION
  gstadaptivedemux_class->start_fragment =
...
gst_dash_demux_setup_all_streams (GstDashDemux * demux)
{
  ...
  for (i = 0; i < gst_mpdparser_get_nb_active_stream (demux->client); i++) {
    ...
    gst_caps_set_simple (caps,"hls_streaming", G_TYPE_BOOLEAN, TRUE,"dash_stream", G_TYPE_BOOLEAN, TRUE, NULL);
...
#ifndef GST_DASHDEMUX_OPENSOURCE_MODIFICATION
...
{
  GstDashDemux *dashdemux = GST_DASH_DEMUX_CAST (demux);
  GST_DEBUG_OBJECT (demux, "Update Period=%" GST_TIME_FORMAT,GST_TIME_ARGS(dashdemux->client->mpd_node->minimumUpdatePeriod * 1000));
