...
#ifdef GST_HLSDEMUX_TEMP_MODIFICATION
...
gst_m3u8_client_free (GstM3U8Client * self)
{
  ...
  /*
  [prasenjit.c] Defensive Fix for checking the validity of the self->lock mutex.
  Changes are similar to Gstsubmux module.
  */
  if (self->lock.p)
  {
...
#ifndef GST_HLSDEMUX_OPENSOURCE_MODIFICATION
...
#ifdef GST_HLSDEMUX_OPENSOURCE_MODIFICATION
...
has_next_fragment (GstM3U8Client * client, GList * l, gboolean forward, gboolean download_current_sequence)
#else
...
#endif
{
  ...
  if (l) {
#ifdef GST_ADAPTIVE_MODIFICATION
	if (download_current_sequence)
		return TRUE;	// Return TRUE, Don't check/wait for next fragmnet sequence, to avoid delayed playback startup in LIVE case
	else
...
/*
  [prasenjit.c] For JioVOD Contents , if we continuously switch between 2 video contents very fast ,
  after pressing 20-30 times , we are observing that , when one instance is getting cleared up
  ,sometimes pending gst_m3u8_client_get_next_fragment request is still active.
  This was causing crash due to invalid File URI. Since URI is already released ,so this crash was seen.

  Currently in case of Invalid URI , we will be posting FALSE fro m3u8 , which will result in GST_FLOW_EOS getting posted from hlsdemux ,
  stopping the playback properly.
*/
  if (!file->uri) {
    GST_ERROR ("Invalid URI is returned .. EOS will be posted !!");
    GST_M3U8_CLIENT_UNLOCK (client);
    return FALSE;
  }
#endif
...
gboolean
#ifdef GST_ADAPTIVE_MODIFICATION
gst_m3u8_client_has_next_fragment (GstM3U8Client * client, gboolean forward, gboolean download_current_sequence)
#else
...
#endif
{
  ...
  if (client->current_file) {
    ...
    ret = has_next_fragment (client, client->current->files, forward, download_current_sequence);
  }
  GST_M3U8_CLIENT_UNLOCK (client);
  return ret;
}
