    ...
    GstBaseParseFrame * frame, gint * skipsize)
{
  ...
  while (TRUE) {
    ...
    if (!gst_h264_parse_process_nal (h264parse, &nalu)) {
      GST_WARNING_OBJECT (h264parse, "broken/invalid nal Type: %d %s, Size: %u will be dropped",
          nalu.type, _nal_name (nalu.type), nalu.size);
      *skipsize = nalu.size;
#ifdef GST_H264PARSE_MODIFICATION
      if (h264parse->codec_data && !GST_H264_PARSE_STATE_VALID (h264parse, GST_H264_PARSE_STATE_GOT_SPS))
        goto invalid_stream;
#endif
      goto skip;
    }
    ...
    if (!(src_caps && gst_caps_is_strictly_equal (src_caps, caps))) {
      ...
      if (buf) {
        ...
        if (!h264parse->codec_data) {
          ...
          gst_caps_set_simple (caps, "codec_data", GST_TYPE_BUFFER, h264parse->codec_data, NULL);
        }
#endif
      }
...
#ifndef GST_H264PARSE_MODIFICATION //We support this case.
...
#ifdef GST_H264PARSE_MODIFICATION
    /* handle byte-stream codec_data */
    if(size >= 4) {
      if (data[0] == 0x00 && data[1] == 0x00 &&
          (data[2] == 0x01 || (data[2] == 0x00 && data[3] == 0x01))) {
        GstMapInfo info;
        GST_WARNING_OBJECT(h264parse, "has codec_data, but byte stream format.");
        h264parse->format = GST_H264_PARSE_FORMAT_BYTE;
        h264parse->align = align;
        h264parse->packetized = FALSE;
        gst_buffer_replace(&h264parse->codec_data, codec_data);
