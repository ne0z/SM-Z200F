    ...
    packetizer, MpegTSPacketizerPacket * packet)
{
  ...
  if (afcflags & MPEGTS_AFC_PCR_FLAG) {
        ...
        && GST_CLOCK_TIME_IS_VALID (packetizer->last_in_time))
    {
	  ...
	  if (((!GST_CLOCK_TIME_IS_VALID (pcrtable->base_time))||(!GST_CLOCK_TIME_IS_VALID (pcrtable->base_pcrtime)))||((packetizer->is_live_stream)&&(!(afcflags & MPEGTS_AFC_RANDOM_ACCES_FLAGS))))
	  {
...
mpegts_packetizer_clear (MpegTSPacketizer2 * packetizer)
{
  ...
  packetizer->last_in_time = GST_CLOCK_TIME_NONE;
#ifdef GST_TSDEMUX_MODIFICATION
  if (!(packetizer->is_live_stream && packetizer->is_random_access)) {
    packetizer->ra_base_time = GST_CLOCK_TIME_NONE;
    packetizer->is_random_access = FALSE;
    packetizer->is_live_stream = FALSE;
  }
...
mpegts_packetizer_flush (MpegTSPacketizer2 * packetizer, gboolean hard)
{
  ...
  packetizer->last_in_time = GST_CLOCK_TIME_NONE;
#ifdef GST_TSDEMUX_MODIFICATION
//[prasenjit.c] When Seek is done , flush API gets called and ra_base_time was getting reset.
// This was resulting in modification of ra_base_time with new ts value , causing Seek Failure for Audio-Only Jio VOD contents.
// Example : http://music.hdi.cdn.ril.com/mod/_definst_/smil:hdindiamusic/audiofiles/8/7310/8_7310_1.smil/playlist.m3u8
  #if 0
  packetizer->ra_base_time = GST_CLOCK_TIME_NONE;
  packetizer->is_random_access = FALSE;
  #endif
  ...
  if (packet->payload_unit_start_indicator) {
    ...
    if (pointer == 0) {
      GST_LOG
          ...
          packet->pid);
#ifdef GST_TSDEMUX_OPENSOURCE_MODIFICATION
      mpegts_packetizer_clear_section (stream);
#endif
      goto section_start;
    }
    ...
    MpegTSPCR * pcr, guint64 pcrtime, GstClockTime time)
{
  ...
  if (G_UNLIKELY (!GST_CLOCK_TIME_IS_VALID (pcr->base_time))) {
    if (packetizer->is_live_stream && packetizer->is_random_access && GST_CLOCK_TIME_IS_VALID (packetizer->ra_base_time))
	pcr->base_time = gstpcrtime - packetizer->ra_base_time;
    else
	pcr->base_time = time;

    pcr->prev_out_time = GST_CLOCK_TIME_NONE;
    GST_DEBUG ("Taking new base time %" GST_TIME_FORMAT, GST_TIME_ARGS (pcr->base_time));
  }
...
*/
	GST_DEBUG ("non_pcr_base_time = %"GST_TIME_FORMAT" , last_live_pts = %"GST_TIME_FORMAT,GST_TIME_ARGS (packetizer->non_pcr_base_time),GST_TIME_ARGS (packetizer->last_live_pts));
  ...
  else if ((packetizer->is_non_pcr_live_feed)&&(GST_CLOCK_TIME_IS_VALID(packetizer->last_live_pts))&&(GST_CLOCK_TIME_IS_VALID(packetizer->last_live_pts_delta_apprx)))
  {
  ...
  GST_WARNING ("Returning timestamp %" GST_TIME_FORMAT " for pts %"
