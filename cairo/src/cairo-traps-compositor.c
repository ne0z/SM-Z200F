		       ...
		       const cairo_bool_t draw_color_glyph)
{
    ...
    if (draw_color_glyph) {
    int tx, ty;
    cairo_surface_t *temp_src = compositor->pattern_to_surface (mask,
								NULL,
								FALSE,
								NULL,
								NULL,
    ...
    if (unlikely (temp_src->status)) {
        _cairo_traps_fini (&traps);
        return temp_src->status;
	}
	status = compositor->composite_traps (mask, CAIRO_OPERATOR_IN, mask,
					  src_x - extents->x, src_y - extents->y,
					  src_x + extents->x, src_y + extents->y,
					  extents,
					  antialias, &traps);
	cairo_surface_destroy (temp_src);
	} else
