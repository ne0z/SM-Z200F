...
#ifdef CONFIG_TIZEN
        template = unit_name_template(u->id);
#endif
...
#ifdef CONFIG_TIZEN
        else if (u->manager->load_allowed_only &&
                 (u->type == UNIT_SERVICE ||
                  u->type == UNIT_SOCKET ||
                  u->type == UNIT_TARGET ||
                  u->type == UNIT_TIMER ||
                  u->type == UNIT_PATH ||
                  u->type == UNIT_SCOPE) &&
                 !hashmap_contains(u->manager->allowed_units,
                                   template)) {
                u->load_state = UNIT_DENIED;
                u->unit_file_state = UNIT_FILE_DENIED;
        }
#endif
        else {
