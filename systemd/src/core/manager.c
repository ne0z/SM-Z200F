...
int manager_new(SystemdRunningAs running_as, bool test_run, Manager **_m) {
        ...
        m->load_allowed_only = false;

        r = hashmap_ensure_allocated(&m->allowed_units, string_hash_func, string_compare_func);
        if (r < 0)
                goto fail;
#endif
...
#ifdef HAVE_SMACK
                if (m->running_as == SYSTEMD_SYSTEM) {
                        r = mac_smack_set_ip_in_fd(fd, "*");
                        if (r < 0) {
                                log_error("mac_smack_set_ip_in_fd() failed: %s", strerror(-r));
                                return -errno;
                        }

                        r = mac_smack_set_ip_out_fd(fd, "@");
                        if (r < 0) {
                                log_error("mac_smack_set_ip_out_fd() failed: %s", strerror(-r));
                                return -errno;
                        }
                }
#endif
...
void manager_free(Manager *m) {
        ...
        hashmap_free_free(m->allowed_units);
#endif
...
#ifdef CONFIG_TIZEN
        manager_set_load_unit_whitelist(m);
#endif
...
static int manager_set_load_unit_whitelist_from_path(Manager *m, const char *path) {
        _cleanup_free_ char *buf = NULL;
        char *state, *word, *name;
        size_t len;
        int r = 0;
        ...
        r = read_full_file(path, &buf, NULL);
        if (r < 0)
                return r;
        ...
        FOREACH_WORD_SEPARATOR(word, len, buf, NEWLINE, state) {
                ...
                r = hashmap_put(m->allowed_units, n, n);
                if (r < 0 && r != -EEXIST)
                        return r;

                m->load_allowed_only = true;
        }

        return 0;
}

static void manager_clear_load_unit_whitelist(Manager *m) {
        ...
        hashmap_clear_free(m->allowed_units);
        m->load_allowed_only = false;

        return;
}

int manager_set_load_unit_whitelist(Manager *m) {
        int r = 0;
        ...
        manager_clear_load_unit_whitelist(m);

        r = manager_set_load_unit_whitelist_from_path(m, "/usr/share/systemd/systemd-system-units");
        if (r < 0)
                return r;

        return manager_set_load_unit_whitelist_from_path(m, PKGSYSCONFDIR "/system.d/allowed-units");
}
#endif
