...
#ifdef CONFIG_TIZEN
static int unit_whitelist_get_from_path(Hashmap *h, const char *path) {
        _cleanup_free_ char *buf = NULL;
...
int unit_file_get_list(
        ...
        _cleanup_lookup_paths_free_ LookupPaths paths = {};
#ifdef CONFIG_TIZEN
        _cleanup_hashmap_free_free_ Hashmap *w = NULL;
#endif
        char **i;
        int r;
...
#ifdef CONFIG_TIZEN
        r = hashmap_ensure_allocated(&w, string_hash_func, string_compare_func);
        if (r < 0)
                return r;

        (void) unit_whitelist_get_from_path(w, "/usr/share/systemd/systemd-system-units");
        (void) unit_whitelist_get_from_path(w, PKGSYSCONFDIR "/system.d/allowed-units");
#endif
...
#ifdef CONFIG_TIZEN
                        template = unit_name_template(de->d_name);
                        if (!template)
                                return -ENOMEM;
#endif
...
#else
                                f->state = hashmap_get(w, template) ? UNIT_FILE_DISABLED : UNIT_FILE_DENIED;
#endif
                        else
