...
Source1002:     systemd-libs.manifest
Source1003:     libgudev1.manifest
...
# Generate systemd own unit list file
[ -e %{buildroot}%{_datarootdir}/systemd/systemd-system-units ] && rm -f %{buildroot}%{_datarootdir}/systemd/systemd-system-units
for unit in $(find %{buildroot}%{_libdir}/systemd/system %{_sysconfdir}/systemd/system -type f);
do
    echo $(basename $unit) >> %{buildroot}%{_datarootdir}/systemd/systemd-system-units
done
# add some of units manually
for unit in \
...
do
    ln -sf /dev/null %{_sysconfdir}/systemd/system/$unit
done
...
%{_bindir}/udevadm
%{_datadir}/systemd/systemd-system-units
...
%endif
%{_libdir}/libudev.so.*
%manifest systemd-libs.manifest
