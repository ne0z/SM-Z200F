...
gst_flv_demux_parse_tag_audio (GstFlvDemux * demux, GstBuffer * buffer)
{
  ...
  /* when timestamp difference between first buffer and 0:00:00 is longer than 1minute, */
  /* 1minute is just an example, it can be changed any time. */
  /* timestamp will be changed to show current position as 0:00:00 */
  /* below condition can guarantee specific case (iqiyi) */
  /*  - first buffer is comming, timestamp of first buffer is bigger than 1minute */
  gint32 second = GST_SECOND/GST_MSECOND;
  if (demux->timestamp_diff == 0 && demux->audio_offset == 0 && pts > 60 * second)
  {
    demux->timestamp_diff = pts / (60 * second);
    pts = pts - (demux->timestamp_diff * 60 * second);
    GST_WARNING_OBJECT (demux, "timestamp_diff : (init)%d", demux->timestamp_diff);
  }
  else if (demux->timestamp_diff != 0)
  {
    pts = pts - (demux->timestamp_diff * 60 * second);
  }
#endif
...
/* Prevent change of duration */
#ifndef GST_EXT_FLVDEMUX_MODIFICATION
  ...
  {
    demux->duration = GST_BUFFER_TIMESTAMP (outbuf);
    GST_DEBUG_OBJECT (demux, "duration is changed to %" GST_TIME_FORMAT,
...
#ifdef GST_EXT_FLVDEMUX_MODIFICATION
  /* when timestamp difference in minute between first buffer and 0:00:00 is longer than 1minute, */
  /* timestamp will be changed to show current position as 0:00:00 */
  gint32 second = GST_SECOND/GST_MSECOND;
  if (demux->timestamp_diff == 0 && demux->video_offset == 0 && pts > 60 * second)
  {
    demux->timestamp_diff = pts / (60 * second);
    pts = pts - (demux->timestamp_diff * 60 * second);
    GST_WARNING_OBJECT (demux, "timestamp_diff : (init)%d", demux->timestamp_diff);
...
/* seek position must be modified within changed timestamp */
#ifdef GST_EXT_FLVDEMUX_MODIFICATION
  if (demux->timestamp_diff != 0) {
    start += ((demux->timestamp_diff * 60 * 1000) * GST_MSECOND);
    GST_WARNING_OBJECT (demux, "changed seek position : %"
