...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
/* The 4 bytes allotted for the atom size field limit the maximum size of an atom to 2 GB. */
#define QTDEMUX_MAX_SAMPLE_INDEX_SIZE (2*1024*1024*1024)         // 2GB = 2*1024 MB
#else
...
struct _QtDemuxStream
{
  ...
  gboolean disabled;
#ifdef GST_EXT_QTDEMUX_MODIFICATION
  GstClockTime base_dts_time;
  GstClockTime base_pts_time;
#endif
};
...
gst_qtdemux_init (GstQTDemux * qtdemux)
{
...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
  qtdemux->is_adaptive_stream = FALSE;
  qtdemux->is_dash_stream = FALSE;
  qtdemux->is_base_pts_reset_done = FALSE;
#endif
  ...
  memset(qtdemux->spherical_caps_info, 0, GST_QTDEMUX_SPHERICAL_CAPS_COUNT * sizeof(char*));
#endif
...
#if 1
static gboolean
gst_qtdemux_src_convert (GstQTDemux * qtdemux, GstPad * pad, GstFormat src_format, gint64 src_value,
  ...
  switch (GST_QUERY_TYPE (query)) {
    ...
    case GST_QUERY_DURATION:{
      ...
      if (fmt == GST_FORMAT_TIME) {
         ...
         * [prasenjit.c] Since we are not sure of the condition of qtdemux->fragmented case for DASH/SS ,
         * So using qtdemux->is_adaptive_stream variable for differentiating the same.*/
         /*[abhijit.dey] Get duration query failing for fragmented files PLM P151230-05920.
         Updated Code from ===> if (!res && !qtdemux->is_adaptive_stream && !qtdemux->fragmented)
          */
        if (!res && !qtdemux->is_adaptive_stream) {
#else
        if (!res) {
#endif
          guint64 duration;
    ...
    gboolean use_sparse, gint64 * key_time, gint64 * key_offset)
{
  ...
  for (n = 0; n < qtdemux->n_streams; n++) {
    ...
    QtDemuxSegment *seg;
#ifdef GST_QTDEMUX_OPENSOURCE_MERGE_CHANGES
...
gst_qtdemux_setcaps (GstQTDemux * demux, GstCaps * caps)
{
...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
  /*
  [prasenjit.c]For DASH/SS Streaming cases , qtdemux was not passing on the sinkpad stream caps to subsequent elements.
  ...
  */
  if (caps)
  {
	...
	{
		GST_WARNING_OBJECT (demux, "Adaptive Streaming case : setting hls_streaming flag to TRUE !!");
		demux->is_adaptive_stream = TRUE;
	}
	if (strstr(caps_str,"dash_stream"))
	{
		GST_WARNING_OBJECT (demux, "Dash Streaming case : setting dash_stream flag to TRUE !!");
...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
static void
qtdemux_parse_360video_xml (GstQTDemux * qtdemux, GstBuffer* buffer)
{
  xmlChar *data;
  xmlChar *key;
  xmlDocPtr doc;
  ...
  data = (xmlChar *) info.data;
  /* create xml document object */
  doc = xmlParseDoc(data);
  if(doc == NULL){
    GST_ERROR_OBJECT (qtdemux, "Fail to make xml doc.");
    return;
  }

  cur = xmlDocGetRootElement(doc);
  if(cur == NULL){
    GST_ERROR_OBJECT (qtdemux, "There is no root element.");
    xmlFreeDoc(doc);
    return;
  }

  GST_INFO_OBJECT (qtdemux, "root element is %s", cur->name);

  if(xmlStrcmp(cur->name, (const xmlChar *) "SphericalVideo")){
    GST_ERROR_OBJECT (qtdemux, "The root element is not matched with SphericalVideo");
    xmlFreeDoc(doc);
    return;
  }
  ...
  while(cur != NULL){
    if ((!xmlStrcmp(cur->name, (const xmlChar *) "Spherical"))) {
      key = xmlNodeGetContent (cur);
      qtdemux->spherical_caps_info[0] = key;
      gst_tag_list_add (qtdemux->tag_list, GST_TAG_MERGE_REPLACE, GST_TAG_V360_SPHERICAL, key, NULL);
    }
    else if ((!xmlStrcmp(cur->name, (const xmlChar *) "Stitched"))) {
      key = xmlNodeGetContent (cur);
      qtdemux->spherical_caps_info[1] = key;
      gst_tag_list_add (qtdemux->tag_list, GST_TAG_MERGE_REPLACE, GST_TAG_V360_STITCHED, key, NULL);
    }
    else if ((!xmlStrcmp(cur->name, (const xmlChar *) "StitchingSoftware"))) {
      key = xmlNodeGetContent (cur);
      qtdemux->spherical_caps_info[2] = key;
      gst_tag_list_add (qtdemux->tag_list, GST_TAG_MERGE_REPLACE, GST_TAG_V360_STITCHING_SOFTWARE, key, NULL);
    }
    else if ((!xmlStrcmp(cur->name, (const xmlChar *) "ProjectionType"))) {
      key = xmlNodeGetContent (cur);
      qtdemux->spherical_caps_info[3] = key;
      gst_tag_list_add (qtdemux->tag_list, GST_TAG_MERGE_REPLACE, GST_TAG_V360_PROJECTION_TYPE, key, NULL);
    }
    cur = cur->next;
  }
  xmlFreeDoc(doc);
...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
  else if (memcmp (buffer + offset, v360_uuid, 16) == 0) {
    GstBuffer *buf;

    buf = _gst_buffer_new_wrapped ((guint8 *) buffer + offset + 16, length - offset - 16, NULL);
    qtdemux_parse_360video_xml(qtdemux, buf);
    gst_buffer_unref (buf);
  }
#endif
  else {
...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
//[prasenjit.c]Temporary change for Not updating intermediate duration
// Change kept only for Dash Case.
    if (!qtdemux->is_dash_stream)
    {
	GST_DEBUG_OBJECT (qtdemux, "updating duration for fragmented contents !");
	check_update_duration (qtdemux, sidx_parser.cumulative_pts);
    }
    else
    	GST_DEBUG_OBJECT (qtdemux, "Not updating the duration !!!");
#else
	check_update_duration (qtdemux, sidx_parser.cumulative_pts);
#endif
  }
    ...
    guint64 moof_offset, QtDemuxStream * stream)
{
  ...
  /* NOTE @stream ignored */
#ifdef GST_EXT_QTDEMUX_MODIFICATION
  if ((qtdemux->is_adaptive_stream)&&(!buffer))
  {
      GST_ELEMENT_ERROR (qtdemux, STREAM, DEMUX,
...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
       if ((qtdemux->is_adaptive_stream)&&(!stream))
       {
  ...
  switch (fourcc) {
    case FOURCC_moof:
      ...
      if (qtdemux_pull_mfro_mfra (qtdemux)) {
        ...
        GST_INFO_OBJECT (qtdemux, "Could not find mfro atom: in fragmented mp4 container");
      }
#endif
      if (qtdemux->got_moov) {
...
/*Patch for PLM P160510-05779. For certain clips segment start stop time was coming same resulting in EOS*/
#ifdef GST_EXT_QTDEMUX_MODIFICATION
  if ((start == stop) && start != GST_CLOCK_TIME_NONE)
  {
     stop = segment->media_stop = GST_CLOCK_TIME_NONE;
  }
#endif
...
/*[abhijit.dey]PLM P151230-05920 we will use trun entries in moof box to find the previous keyframe index*/
#ifndef GST_EXT_QTDEMUX_MODIFICATION
...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
//[prasenjit.c]Temporary change for Modifying Timestamp even for Local clips
  if (TRUE)//qtdemux->is_dash_stream)
  {
	  if (stream->base_dts_time == GST_CLOCK_TIME_NONE)
	  {
		stream->base_dts_time = dts;
	       GST_LOG_OBJECT (qtdemux,"Base DTS value changed !!!= %"GST_TIME_FORMAT,GST_TIME_ARGS (stream->base_dts_time));
	  }

	  if (stream->base_pts_time == GST_CLOCK_TIME_NONE)
	  {
	  	stream->base_pts_time = pts;
	       GST_LOG_OBJECT (qtdemux,"Base PTS value changed !!!= %"GST_TIME_FORMAT,GST_TIME_ARGS (stream->base_pts_time));
	  }

	  GST_LOG_OBJECT(qtdemux,
	      "[OLD] Original %" GST_TIME_FORMAT ", pts %" GST_TIME_FORMAT
	      ", duration %" GST_TIME_FORMAT " on pad %s", GST_TIME_ARGS (GST_BUFFER_DTS (buf)),
	      GST_TIME_ARGS (GST_BUFFER_PTS (buf)), GST_TIME_ARGS (duration),
	      GST_PAD_NAME (stream->pad));

	   if (stream->base_dts_time != GST_CLOCK_TIME_NONE)
		GST_BUFFER_DTS (buf) = dts - stream->base_dts_time;

          if (stream->base_pts_time != GST_CLOCK_TIME_NONE)
	   	GST_BUFFER_PTS (buf) = pts - stream->base_pts_time;
  }

   GST_DEBUG_OBJECT(qtdemux,
      "[NEW] Pushing buffer with dts %" GST_TIME_FORMAT ", pts %" GST_TIME_FORMAT
      ", duration %" GST_TIME_FORMAT " on pad %s", GST_TIME_ARGS (GST_BUFFER_DTS (buf)),
      GST_TIME_ARGS (GST_BUFFER_PTS (buf)), GST_TIME_ARGS (duration),
      GST_PAD_NAME (stream->pad));
#else
...
/*[abhijit.dey]PLM P151230-05920 make sure trun entries table is not deleted incase tfra entried not found in file
As we will use trun entries in moof box for seek*/
#ifdef GST_EXT_QTDEMUX_MODIFICATION
    if (stream->ra_entries == NULL){
      GST_WARNING_OBJECT (stream->pad,"Cound not find ra entry for stream %d",stream->track_id);
      continue;
    }
#endif
...
gst_qtdemux_configure_stream (GstQTDemux * qtdemux, QtDemuxStream * stream)
{
  ...
  */
  if (stream->caps)
  {
	...
	{
		stream->caps = gst_caps_make_writable (stream->caps);
		gst_caps_set_simple (stream->caps,"dash_stream", G_TYPE_BOOLEAN, TRUE, NULL);
		GST_DEBUG_OBJECT (qtdemux,"DASH/SS Demuxer case !! appending dash_stream flag in caps = %"GST_PTR_FORMAT,stream->caps);
	}
  }
#endif
  if (stream->subtype == FOURCC_vide) {
...
#ifndef GST_QTDEMUX_OPENSOURCE_MERGE_CHANGES
      segment->duration = stime - segment->time;
#endif
...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
//[prasenjit.c] Base PTS/DTS value set to be done only once !!
// Since prepare_streams gets called even during seek , so value reset should not happen again.
    if (qtdemux->is_base_pts_reset_done == FALSE)
    {
	qtdemux->is_base_pts_reset_done = TRUE;
	stream->base_dts_time = GST_CLOCK_TIME_NONE;
    	stream->base_pts_time = GST_CLOCK_TIME_NONE;
    }
#endif
...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
      } else if (FOURCC_covr == add_funcs[i].fourcc) {
        GST_DEBUG_OBJECT (qtdemux, "ignore tag atom %" GST_FOURCC_FORMAT,
            GST_FOURCC_ARGS (add_funcs[i].fourcc));
#endif
      } else {
  ...
  if (data_ptr) {
    ...
    if(stream->fourcc == FOURCC_mp4v) {
      ...
      if(err) {
        ...
        gst_caps_set_simple (stream->caps, "variant", G_TYPE_STRING, "itu", NULL);
...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
  if (qtdemux->spherical_caps_info[0]) {
    if (!strcmp(qtdemux->spherical_caps_info[0], "true"))
      gst_caps_set_simple (caps, "spherical", G_TYPE_BOOLEAN, TRUE, NULL);
    else
      gst_caps_set_simple (caps, "spherical", G_TYPE_BOOLEAN, FALSE, NULL);
  }
  if (qtdemux->spherical_caps_info[1]) {
    if (!strcmp(qtdemux->spherical_caps_info[1], "true"))
      gst_caps_set_simple (caps, "stitched", G_TYPE_BOOLEAN, TRUE, NULL);
    else
      gst_caps_set_simple (caps, "stitched", G_TYPE_BOOLEAN, FALSE, NULL);
  }
  if (qtdemux->spherical_caps_info[2]) {
    gst_caps_set_simple (caps, "stitching-software", G_TYPE_STRING, qtdemux->spherical_caps_info[2], NULL);
  }
  if (qtdemux->spherical_caps_info[3]) {
    gst_caps_set_simple (caps, "projection-type", G_TYPE_STRING, qtdemux->spherical_caps_info[3], NULL);
  }
#endif
