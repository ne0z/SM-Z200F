...
*/
static gboolean
gst_mpeg_audio_parse_do_push_seek (GstBaseParse * parse, GstEvent * event)
{
    ...
    case GST_EVENT_SEEK:
      {
        ...
        if (pad_mode != GST_PAD_MODE_PULL && mp3parse->encoded_file_size) {
          gboolean ret = FALSE;
          GST_INFO_OBJECT (mp3parse, "mp3 parser is PUSH MODE.");
          ret = gst_mpeg_audio_parse_do_push_seek(parse, event);
          return ret;
        } else if (!mp3parse->encoded_file_size) {
          GST_INFO_OBJECT (mp3parse, "Seek will be handled by demuxer");
          break;
        }
