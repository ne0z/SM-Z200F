...
#ifdef GST_EXT_AMRPARSER_MODIFICATION /* added to include amrnb header size */
...
static gboolean gst_amr_parse_src_eventfunc (GstBaseParse * parse,
    GstEvent * event);
#define gst_amr_parse_parent_class parent_class
#endif
...
gst_amr_parse_class_init (GstAmrParseClass * klass)
{
  ...
  parse_class->pre_push_frame = GST_DEBUG_FUNCPTR (gst_amr_parse_pre_push_frame);
#ifdef GST_EXT_AMRPARSER_MODIFICATION /* make full amr index table when seek */
  parse_class->src_event = GST_DEBUG_FUNCPTR (gst_amr_parse_src_eventfunc);
#endif
}
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_SEEK:
    {
      ...
      if (pad_mode != GST_PAD_MODE_PULL) {
        GST_INFO_OBJECT (amrparse, "amr parser is not pull mode. amr parser can not make index table.");
        return FALSE;
      }
      ...
      while (total_file_size - base_offset >= AMR_MAX_PULL_RANGE_BUF) {
        ...
        res = gst_pad_pull_range (parse->sinkpad, base_offset, base_offset + AMR_MAX_PULL_RANGE_BUF, &buffer);
        if (res != GST_FLOW_OK) {
