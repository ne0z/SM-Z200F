...
#ifdef GST_EXT_SOUP_MODIFICATION
  g_object_class_install_property (gobject_class, PROP_IS_SEEKABLE,
      g_param_spec_boolean ("is-seekable", "is-seekable", "Is the content seekable",
          TRUE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#endif
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_IS_SEEKABLE:
      ...
      GST_INFO_OBJECT (src, " setting src->seekable = %d",src->seekable);
      break;
#endif
    case PROP_USER_ID:
      if (src->user_id)
...
#ifdef GST_EXT_SOUP_MODIFICATION
    case PROP_IS_SEEKABLE:
      GST_WARNING_OBJECT (src, "src->seekable = %d",src->seekable);
...
gst_soup_http_src_headers_foreach (const gchar * name, const gchar * val,
    gpointer src)
{
#if defined(GST_SOUP_SECURE_LOG)
  if (!g_strcmp0(name,"X-DM-BackNode") || !g_strcmp0(name,"X-DM-LB-IP") || !g_strcmp0(name,"Referer") || !g_strcmp0(name,"Location")) {
    GST_WARNING_OBJECT (src, " %s:", name);
  }
  else {
    GST_WARNING_OBJECT (src, " %s: %s", name, val);
  }
#elif defined(GST_GOOD_EXTRA_DEBUG_LOG)
  GST_WARNING_OBJECT (src, " %s: %s", name, val);
#else
  ...
  else if (g_ascii_strcasecmp (name, "Dash-Oldest-Segment") == 0)
  {
    if (val)
    {
      ...
      GST_WARNING_OBJECT (src, "Dash-Oldest-Segment set as %s ", tmp->dash_oldest_segment);
    }
  }
  else if (g_ascii_strcasecmp (name, "Dash-Newest-Segment") == 0)
  {
    if (val)
    {
      ...
      GST_WARNING_OBJECT (src, "Dash-Newest-Segment set as %s ", tmp->dash_newest_segment);
    }
  }
#endif
}
...
gst_soup_http_src_got_headers_cb (SoupMessage * msg, GstSoupHTTPSrc * src)
{
  ...
  if ((accept_ranges =
    ...
    {
      GST_WARNING_OBJECT (src, "server is not seekable");
      src->seekable = FALSE;
    }
  }
...
#endif
    src->seekable = FALSE;
#ifdef GST_SOUP_SECURE_LOG
    GST_ELEMENT_ERROR (src, RESOURCE, SEEK,
        (_("Server does not support seeking.")),
        ("Server does not accept Range HTTP header, URL: , Redirect to: "));
#else
    GST_ELEMENT_ERROR (src, RESOURCE, SEEK,
        (_("Server does not support seeking.")),
...
#ifdef GST_GOOD_EXTRA_DEBUG_LOG
  GST_WARNING_OBJECT (src, "finished, io status: %d", src->session_io_status);
#else
...
#define SOUP_HTTP_SRC_ERROR(src,soup_msg,cat,code,error_message)     \
  GST_ELEMENT_ERROR ((src), cat, code, ("%s", error_message),        \
      ("%s (%d), URL: , Redirect to: ", (soup_msg)->reason_phrase,                \
...
gst_soup_http_src_parse_status (SoupMessage * msg, GstSoupHTTPSrc * src)
{
  if (msg->method == SOUP_METHOD_HEAD) {
	...
	GST_ELEMENT_ERROR (src, RESOURCE, NOT_FOUND,
             ("%s", msg->reason_phrase),
             ("%s (%d), URL: , Redirect to: , Dash-Oldest-Segment:%s,Dash-Newest-Segment:%s,", msg->reason_phrase,
             msg->status_code, GST_STR_NULL (src->dash_oldest_segment),GST_STR_NULL (src->dash_newest_segment)));
#elif defined(GST_EXT_SOUP_MODIFICATION)
	GST_ELEMENT_ERROR (src, RESOURCE, NOT_FOUND,
             ("%s", msg->reason_phrase),
             ("%s (%d), URL: %s, Redirect to: %s , Dash-Oldest-Segment:%s,Dash-Newest-Segment:%s,", msg->reason_phrase,
             msg->status_code, src->location,
             GST_STR_NULL (src->redirection_uri),GST_STR_NULL (src->dash_oldest_segment),GST_STR_NULL (src->dash_newest_segment)));
#else
      GST_ELEMENT_ERROR (src, RESOURCE, NOT_FOUND,
          ("%s", msg->reason_phrase),
      ...
      GST_ELEMENT_ERROR (src, RESOURCE, NOT_AUTHORIZED, ("%s",
              msg->reason_phrase), ("%s (%d), URL: , Redirect to: ",
      ...
      GST_ELEMENT_ERROR (src, RESOURCE, OPEN_READ,
          ("%s", msg->reason_phrase),
          ("%s (%d), URL: , Redirect to: ", msg->reason_phrase,
              msg->status_code));
#else
      GST_ELEMENT_ERROR (src, RESOURCE, OPEN_READ,
          ("%s", msg->reason_phrase),
...
#if defined(GST_SOUP_SECURE_LOG)
  GST_WARNING_OBJECT (src, "start");
#elif defined(GST_GOOD_EXTRA_DEBUG_LOG)
  GST_WARNING_OBJECT (src, "start(\"%s\")", src->location);
#else
