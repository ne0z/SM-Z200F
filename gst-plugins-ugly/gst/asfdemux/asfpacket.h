...
typedef struct
{
  int                pl_cnt;                     // No. of payloads in current frame
  int                pl_size[SWMFD_PL_MAX_CNT];  // Each payload size in current frame
  int                rep_data_size;              /*[SWMFD_PL_MAX_CNT];*/    // replicated_data size of first payload..
  unsigned char      rep_data[18];               /* CHECK-ME: 2304 = 18 * SWMFD_PL_MAX_CNT */ // replicated data of first payload in a frame.
...
typedef struct {
  ...
  gboolean      rff;
#if defined(ASFDEMUX_MODIFICATION) && defined(DRM_ENABLE) && defined(FEATURE_SECURE_PLAYBACK_TZ)
  SWMFDS_INFO_PL  hardware_payload_info;
#endif
} AsfPayload;
