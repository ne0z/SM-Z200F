...
drm_trusted_result_e drm_util_pr_init (DRM_DECRYPT_HANDLE *phandle, char* file_path, gboolean *is_drm_file, drm_file_type_e file_type)
{
  ...
  if (phandle == NULL || file_path == NULL || is_drm_file == NULL || strlen(file_path) <= 0) {
    GST_ERROR ("Invalid input parameters !!!");
    return DRM_TRUSTED_RETURN_INTERNAL_ERROR;
  }
  ...
  *is_drm_file = FALSE;
  ...
  if (file_type != DRM_TYPE_PLAYREADY && file_type != DRM_TYPE_PLAYREADY_ENVELOPE) {
    GST_INFO ("This is not PlayReady DRM!!!!");
    *is_drm_file = FALSE;
    return DRM_TRUSTED_RETURN_SUCCESS;
  }

  GST_INFO ("input file is DRM file...");
...
drm_trusted_result_e drm_util_pr_decrypt_payload (DRM_DECRYPT_HANDLE handle, AsfPayload *payload, GstBuffer *outbuf)
{
  drm_trusted_result_e drm_trusted_result;
  ...
  GST_DEBUG ("Enter [%s] payload=%p, handle=%p", __func__, payload, handle);
  ...
  payload_info.media_offset = 0;

  if (outbuf) {
    GST_DEBUG("outbuf present");
    payload_info.payload_data_output = GST_BUFFER_DATA(outbuf); /* secure_mode */
    GST_DEBUG("outbuf address %p", outbuf);
#ifdef FEATURE_SECURE_PLAYBACK_TZ
    payload_info.media_type = 1;
#endif
  } else {
    GST_DEBUG("outbuf absent");
    payload_info.payload_data_output = GST_BUFFER_DATA(payload->buf); /* non-secure mode */
#ifdef FEATURE_SECURE_PLAYBACK_TZ
    payload_info.media_type = 0;
#endif
  }

  drm_trusted_result = drm_trusted_read_decrypt_session (handle, &payload_info, &decrypt_resp);
  if(drm_trusted_result != DRM_TRUSTED_RETURN_SUCCESS) {
    GST_ERROR ("failed to decrypt buffer. handle=%p", handle);
    return drm_trusted_result;
  }

  GST_DEBUG ("Leave [%s], drm_trusted_read_decrypt_session() success", __func__);

  return drm_trusted_result;
}

drm_trusted_result_e drm_util_pr_finalize (DRM_DECRYPT_HANDLE *phandle)
{
  drm_trusted_result_e drm_trusted_result;
