    ...
    AsfStream * stream)
{
  ...
  if (G_UNLIKELY (GST_CLOCK_TIME_IS_VALID (payload->ts) &&
          payload->ts < demux->segment.start && payload->keyframe) && !demux->secure_mode) {
#else
  if (G_UNLIKELY (GST_CLOCK_TIME_IS_VALID (payload->ts) &&
      ...
      *p_size -= payload_len;
      ...
      if (payload.mo_offset != 0) {
        ...
        if ((prev = asf_payload_find_previous_fragment (&payload, stream))) {
          ...
          GST_DEBUG_OBJECT(demux, "Merging different payload of same frame");
          prev->hardware_payload_info.pl_size[prev->hardware_payload_info.pl_cnt] = gst_buffer_get_size(payload.buf);
          prev->hardware_payload_info.pl_cnt++;
#endif
...
#if defined(ASFDEMUX_MODIFICATION) && defined(DRM_ENABLE) && defined(FEATURE_SECURE_PLAYBACK_TZ)
        payload.hardware_payload_info.pl_size[payload.hardware_payload_info.pl_cnt] = gst_buffer_get_size(payload.buf);
        payload.hardware_payload_info.pl_cnt++;
        payload.hardware_payload_info.rep_data_size = payload.rep_data_len;
        for(int i = 0; i < 18; i++)
          payload.hardware_payload_info.rep_data[i] = (unsigned char*)payload.rep_data[i];
        payload.hardware_payload_info.total_frame_size = payload.mo_size;
#endif
