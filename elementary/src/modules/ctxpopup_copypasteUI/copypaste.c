   ...
   if (ext_mod->have_selection)
     {
        if ((ext_mod->viewport_rect->x != -1) || (ext_mod->viewport_rect->y != -1) ||
            (ext_mod->viewport_rect->w != -1) || (ext_mod->viewport_rect->h != -1))
          {
             Evas_Coord ex, ey;
             Eina_Rectangle sel;
             /* No need to check the intersection when any handler is visible, think about it */
             if (evas_object_visible_get(ext_mod->end_handler) || evas_object_visible_get(ext_mod->start_handler))
               return EINA_TRUE;
             ...
             if (sy < vy)
               {
                  ...
                  if (ehy + ehh < vy + vh && ey >= vy) //end handler is showing *->* end handler can be above outside VP in -ve direction
                    {
                       y = ehy + ehh;
   ...
   if(!ext_mod)
     {
        ...
        ext_mod->mouse_up = EINA_FALSE;
        /* Entry mouse down is missed when popup first created causing issue.
         * So lets keep it true */
        ext_mod->mouse_down = EINA_TRUE;
        ext_mod->entry_move = EINA_FALSE;
     }
...
#ifdef ELM_FEATURE_QUICK_MEMO_ENABLE
#ifdef HAVE_ELEMENTARY_APPSVC
                        if (ext_mod->quick_memo_enabled && _check_memo_package())
                          {
