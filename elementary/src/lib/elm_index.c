...
//COMMERCIAL_ONLY: For rotation effect of pagecontrol/effect style.
static int
_find_sequence(Elm_Object_Item *eo_it, Elm_Index_Data *sd)
{
   Eina_List *l;
   Elm_Object_Item *eo_item;
   int count = 0;
   ...
   EINA_LIST_FOREACH(sd->items, l, eo_item)
     {
        ELM_INDEX_ITEM_DATA_GET(eo_item, it);
        if ((eo_it == it) && (it->level == sd->level)) break;
        count++;
     }
   return count;
                  ...
                  if (it)
                    {
                       ...
                       if (it_closest)
                         {
                            msg->val[0] = _find_sequence(it, sd);
                            msg->val[1] = _find_sequence(it_closest, sd);
                            edje_object_message_send(VIEW(it), EDJE_MESSAGE_INT_SET, 1, msg);
                         }
                       //
               ...
               void *event_info)
{
   ...
   //COMMERCIAL_ONLY: For rotation effect of pagecontrol/effect style
   if (!strcmp(elm_object_style_get(wd->obj), "pagecontrol/effect")) return;
   //
                 ...
                 void *event_info EINA_UNUSED)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "_index_resize_cb: %p", obj);
...
elm_index_add(Evas_Object *parent)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "elm_index_add: %p", parent);
   ...
   if (selected)
     {
        ...
        if (eo_it_last)
          {
             ...
             msg->val[0] = _find_sequence(it_inactive, sd);
             //
          }
        ...
        //COMMERCIAL_ONLY: For rotation effect of pagecontrol/effect style
        msg->val[1] = _find_sequence(it_active, sd);
        if (eo_it_last)
          {
             edje_object_message_send(VIEW(it_inactive), EDJE_MESSAGE_INT_SET, 1, msg);
             edje_object_signal_emit(VIEW(it_inactive),
                                     "elm,state,inactive", "elm");
             edje_object_message_signal_process(VIEW(it_inactive));
          }
        edje_object_message_send(VIEW(it_active), EDJE_MESSAGE_INT_SET, 1, msg);
        //
...
_elm_index_level_go(Eo *obj, Elm_Index_Data *sd, int level)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "_elm_index_level_go: obj(%p) level(%d)", obj, level);
