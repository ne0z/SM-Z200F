...
_elm_image_async_open_cancel(void *data, Ecore_Thread *thread EINA_UNUSED)
{
   ...
   eina_spinlock_free(&sd->async.lck);
   ...
   if (sd->async.th)
     {
        if (!ecore_thread_cancel(sd->async.th) )
          {
             ERR("Thread is running , cancellation of thread has been pending.");
             // move other data free time to 'cancel function' for avoiding crashes (this leaks)
...
elm_image_add(Evas_Object *parent)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "elm_image_add: %p", parent);
