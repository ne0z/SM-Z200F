...
static void _create_content_bg_image(Eo* eo_item);
                ...
                const char *item_style)
{
   ...
   _create_content_bg_image(EO_OBJ(it));
...
_remove_bg_image(Eo* eo_item)
{
   ...
   if (edje_object_part_exists(elm_layout_edje_get(VIEW(it)), "mesh_bg_changeable"))
     {
        Evas_Object* bg_image = elm_object_part_content_get(VIEW(it), "mesh_bg_changeable");
        if (bg_image)
          {
...
static void
_create_shared_image_buffer(Eo* eo_item, Evas_Object *bg)
{
    ELM_NAVIFRAME_ITEM_DATA_GET(eo_item, it);
   int iw = 0, ih = 0;
   ...
   if (fixed)
   ...
   else
     {
        ...
        if (handler) it->handler = handler;
        if (tbm_surface) it->tbm_surface = tbm_surface;
     }
...
static void
_create_content_bg_image(Eo* eo_item)
{
    ELM_NAVIFRAME_ITEM_DATA_GET(eo_item, it);

    int changeable = (int)ecore_evas_data_get(ecore_evas_ecore_evas_get(evas_object_evas_get(VIEW(it))), "changeable_ui");
    if (!changeable)
      {
    ...
    if (changeable)
      {
         if (edje_object_part_exists(elm_layout_edje_get(VIEW(it)), "mesh_bg_changeable"))
           {
              int iw = 0, ih = 0;
              Evas_Object *bg = elm_object_part_content_get(VIEW(it), "mesh_bg_changeable");
              if (bg)
                {
                   if (it->tbm_surface)
                     {
                        iw = tbm_surface_get_width(it->tbm_surface);
                        ih = tbm_surface_get_height(it->tbm_surface);
                     }
                   else if (ecore_evas_data_get(ecore_evas_ecore_evas_get(evas_object_evas_get(WIDGET(it))), "fixed_theme"))
                     {
                        ecore_x_window_geometry_get(ecore_x_window_root_first_get(), NULL, NULL, &iw, &ih);
                     }
                   evas_object_image_data_update_add(bg, 0, 0, iw, ih);
                ...
                {
                   bg = evas_object_image_filled_add(evas_object_evas_get(WIDGET(it)));
                   _create_shared_image_buffer(eo_item, bg);
                }

              elm_object_part_content_set(VIEW(it), "mesh_bg_changeable", bg);
           }
   ...
   if (top_item)
     {
        ...
        evas_object_smart_callback_call(WIDGET(top_item), "item,hide,internal", VIEW(top_item));
...
elm_naviframe_add(Evas_Object *parent)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "%p", parent);
...
_elm_naviframe_item_pop_to(Eo *eo_it, Elm_Naviframe_Item_Data *it)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "%p", eo_it);
...
_elm_naviframe_elm_interface_atspi_component_accessible_at_point_get(Eo *obj, Elm_Naviframe_Data *_pd EINA_UNUSED, Eina_Bool screen_coords, int x, int y)
{
   ...
   eo_item = eo_do_ret(obj, eo_item, elm_obj_naviframe_top_item_get());
   if (!eo_item) return NULL;
