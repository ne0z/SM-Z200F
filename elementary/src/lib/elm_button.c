...
_elm_button_elm_widget_theme_apply(Eo *obj, Elm_Button_Data *_pd EINA_UNUSED)
{
   ...
   // Forcely send elm,event,pass,disabled signal for other style
   edje_object_signal_emit(wd->resize_obj, "elm,event,pass,disabled", "elm" );
...
_elm_button_evas_object_smart_add(Eo *obj, Elm_Button_Data *_pd EINA_UNUSED)
{
   ...
   //TIZEN_ONLY(20151029): avoid to apply the default theme for uxt_button_add(),
   //but _elm_button_elm_widget_theme_apply will be called()
   ...
   style = evas_object_data_get(parent, "UXT_BUTTON_ADD");
   if (!style)
     style = elm_widget_style_get(obj);

   if (!elm_layout_theme_set(obj, "button", "base", style))
     ERR("Failed to set layout!");

   // Forcely send elm,event,pass,disabled signal for default style
   edje_object_signal_emit(wd->resize_obj, "elm,event,pass,disabled", "elm" );
}
