...
// TIZEN_ONLY(20160223): to support custom calendar.
   ...
   for (idx = 0; idx < ELM_DATETIME_TYPE_COUNT; idx++)
     {
        ...
        // TIZEN_ONLY(20151030): check whether a object is available or not for uxt_date_time_add()
        //if (field->fmt_exist && field->visible)
        if (field->item_obj && field->fmt_exist && field->visible)
        //
   ...
   if (!sd->user_format)
     {
        ...
        if (!strcmp(locale, "km_KH"))
          {
             char km_KH_fmt[] = "%A ថ្ងៃ %e ខែ %B ឆ្នាំ  %Y, %H ម៉ោង %M នាទី %S វិនាទី​";
...
//TIZEN_ONLY(20151030): support to set the other styles for uxt_date_time_add()
static void
_field_set(Evas_Object *obj, Elm_Datetime_Field_Type field_type, Evas_Object *field_obj)
{
   Datetime_Field *field;
   ...
   for (idx = ELM_DATETIME_MONTH; idx < ELM_DATETIME_TYPE_COUNT - 1; idx++)
     {
        ...
        if (date_limit && idx == ELM_DATETIME_DATE)
        //
                 ...
                 int *range_max)
{
   ...
   if (sd->date_limit && field_type == ELM_DATETIME_DATE)
   //
     {
             ...
             if (priv->mod_data)
               {
                  ...
                  //
                  //TIZEN_ONLY(20151030): support to set the other styles for uxt_date_time_add()
                  priv->mod_data->field_set = _field_set;
                  //
               }
          }
...
_elm_datetime_value_set(Eo *obj, Elm_Datetime_Data *sd, const struct tm *newtime)
{
   ...
   // TIZEN_ONLY(20160223): to support custom calendar.
   _apply_range_restrictions(&sd->curr_time, sd->date_limit);
   //
...
_elm_datetime_value_min_set(Eo *obj, Elm_Datetime_Data *sd, const struct tm *mintime)
{
   ...
   // TIZEN_ONLY(20160223): to support custom calendar.
   _apply_range_restrictions(&sd->min_limit, sd->date_limit);
   //
...
_elm_datetime_value_max_set(Eo *obj, Elm_Datetime_Data *sd, const struct tm *maxtime)
{
   ...
   // TIZEN_ONLY(20160223): to support custom calendar.
   _apply_range_restrictions(&sd->max_limit, sd->date_limit);
   //
