...
// TIZEN_ONLY(20151030): Reduce theme searching cost when the given path is absolute.
#ifdef _WIN32
static Eina_Bool
...
_resizing_eval_item(Elm_Toolbar_Item_Data *it)
{
   ...
   evas_object_geometry_get(obj, &x, &y, NULL, &h);
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "%p h(%d)", it, h);
   evas_object_move(sd->more, x, y + h);
               ...
               const char *icon)
{
   ...
   // TIZEN_ONLY(20151030): Reduce theme searching cost when the given path is absolute.
   if (_path_is_absolute(icon)) goto end;
   //
...
_item_select(Elm_Toolbar_Item_Data *it)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "%p", it);
...
_elm_toolbar_elm_widget_theme_apply(Eo *obj, Elm_Toolbar_Data *sd)
{
   ...
   if (evas_object_data_get(obj, "UXT_TOOLBAR_ADD"))
     evas_object_data_del(obj, "UXT_TOOLBAR_ADD");
          ...
          const void *data)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "(%p) icon(%s) label(%s)", obj, icon, label);
...
_elm_toolbar_evas_object_smart_add(Eo *obj, Elm_Toolbar_Data *priv)
{
   ...
   // TIZEN_ONLY(20151030): Use only theme for Icon widget.
   //elm_toolbar_icon_order_lookup_set(obj, ELM_ICON_LOOKUP_THEME_FDO);
   elm_toolbar_icon_order_lookup_set(obj, ELM_ICON_LOOKUP_THEME);
   //
...
_elm_toolbar_shrink_mode_set(Eo *obj, Elm_Toolbar_Data *sd, Elm_Toolbar_Shrink_Mode shrink_mode)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "(%p) mode(%d)", obj, shrink_mode);
