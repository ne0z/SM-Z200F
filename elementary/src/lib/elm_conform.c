...
// TIZEN_ONLY(20151204): Improve launching performance.
   ...
   if ((part_type & ELM_CONFORMANT_VIRTUAL_KEYPAD_PART) && sd->virtualkeypad)
     {
       ...
       Evas_Coord_Rectangle rect;
        ...
        if (!sd->keypad_disable)
          ...
          {
             DBG("[KEYPAD]: auto resize off - size(%d,%d, %dx%d)", 0, 0, 0, 0);
   ...
   if (part_type & ELM_CONFORMANT_CLIPBOARD_PART)
     {
        ...
        if (!sd->clipboard_disable)
          {
             DBG("[CLIPBOARD]: size(%d,%d, %dx%d).", sx, sy, sw, sh);
             _conformant_part_size_hints_set(obj, sd->clipboard, sx, sy, sw, sh);
          }
        else
          {
             DBG("[CLIPBOARD]: auto resize off - size(%d,%d, %dx%d).", 0, 0, 0, 0);
                        ...
                        void *event_info EINA_UNUSED)
{
   ...
   _indicator_hide_effect(conform);
}
             ...
             const char *source EINA_UNUSED)
{
  ...
  DBG("Auto resizing by keypad is disabled");
             ...
             const char *source EINA_UNUSED)
{
   ...
   DBG("Auto resizing by keypad is enabled");
             ...
             const char *source EINA_UNUSED)
{
  ...
  DBG("Auto resizing by clipboard is disabled");
             ...
             const char *source EINA_UNUSED)
{
  ...
  DBG("Auto resizing by clipboard is enabled");
...
_elm_conformant_evas_object_smart_del(Eo *obj, Elm_Conformant_Data *sd)
{
   ...
   evas_object_event_callback_del
     (obj, EVAS_CALLBACK_RESIZE, _move_resize_cb);

   evas_object_event_callback_del
     (obj, EVAS_CALLBACK_MOVE, _move_resize_cb);
   ...
   edje_object_signal_callback_del
     (wd->resize_obj, "elm,state,virtualkeypad,disable", "",
     _on_keypad_off);
   edje_object_signal_callback_del
     (wd->resize_obj, "elm,state,virtualkeypad,enable", "",
     _on_keypad_on);
   edje_object_signal_callback_del
...
//
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "elm_conformant_add: %p", parent);
...
_elm_conformant_eo_base_constructor(Eo *obj, Elm_Conformant_Data *sd)
{
   ...
   edje_object_signal_callback_add
     (wd->resize_obj, "elm,state,virtualkeypad,disable", "", _on_keypad_off, obj);
   edje_object_signal_callback_add
     (wd->resize_obj, "elm,state,virtualkeypad,enable", "", _on_keypad_on, obj);

   edje_object_signal_callback_add
     (wd->resize_obj, "elm,state,clipboard,disable", "", _on_clipboard_off, obj);
   edje_object_signal_callback_add
     (wd->resize_obj, "elm,state,clipboard,enable", "", _on_clipboard_on, obj);
