...
EOLIAN static void
_elm_interface_scrollable_animation_stop(Eo *obj EINA_UNUSED, Elm_Scrollable_Smart_Interface_Data *sid)
{
   ...
   if (sid->down.onhold_tlast > 0.0)
     {
        ...
        vx = sid->down.onhold_vx * td *
          (double)_elm_config->thumbscroll_hold_threshold * 4.0;
                                ...
                                void *event_info)
{
   ...
   if ((ev->timestamp / 1000.0) < sid->down.history[0].timestamp)
      return;
   ...
   if ((!sid->hold) && (!sid->freeze))
     {
        if ((sid->down.dragged) ||
		  ...
		  (((_elm_scroll_can_scroll(sid, LEFT) || _elm_scroll_can_scroll(sid, RIGHT)) && sid->down.dir_x) ||
             ...
             else if ((!sid->loop_v && !sid->loop_h) &&
		          ((sid->down.dir_x && sid->down.hdir == LEFT && _elm_scroll_can_scroll(sid, LEFT)) ||
                   (sid->down.dir_x && sid->down.hdir == RIGHT && _elm_scroll_can_scroll(sid, RIGHT)) ||
                   (sid->down.dir_y && sid->down.vdir == UP && _elm_scroll_can_scroll(sid, UP)) ||
                   (sid->down.dir_y && sid->down.vdir == DOWN && _elm_scroll_can_scroll(sid, DOWN))) && 
                  !sid->down.dragged_began)
               {
                  ev->event_flags |= EVAS_EVENT_FLAG_ON_HOLD;
                  sid->down.dragged_began = EINA_TRUE;
