...
{
   UPF_TRACE_BEGIN(UPF_TRACE_TAG_ELEMENTARY);
   int size = 0;
   Ecore_X_Atom atom;
   char buf[512];
...
{
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "%p", data)
   char *s;
...
#define D _config_edd
   ...
   //TIZEN_ONLY (20151021): Use config value
...
{
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "%f", scale);
   if (scale < 0.0) return;
...
_elm_config_profile_set(const char *profile)
{
   ...
   UPF_TRACE_BEGIN_F(UPF_TRACE_TAG_ELEMENTARY, "%s", profile);
   if (_elm_profile)
     {
