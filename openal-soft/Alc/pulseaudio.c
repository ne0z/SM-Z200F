...
typedef enum {
    CORK_CAUSE_NONE,
    CORK_CAUSE_ASM       = 0x0001,
    CORK_CAUSE_STATE    = 0x0002,
    CORK_CAUSE_TIMEOUT   = 0x0004,
} CorkCause;
#endif
...
#ifdef __TIZEN__
    int cork_cause;
    struct timeval cork_cause_time;
#ifdef USE_ASM_IN_PULSEAUDIO
    int asm_handle;
...
static void stream_success_cb (pa_stream *s, int success, void *userdata) {
    ...
    AL_DEBUG("stream_success_cb(success:%d)\n", success);
    ppa_threaded_mainloop_signal(data->loop, 0);
}
#endif
...
static ALuint PulseProc(ALvoid *param)
{
    ...
    do {
...
#ifdef SUPPORT_CORK_BY_STATE
        ActiveSourceCount = GetActiveSourceCount(Device);
        if(ActiveSourceCount == 0 && !(data->cork_cause & CORK_CAUSE_STATE))
        {
            data->cork_cause |= CORK_CAUSE_STATE;
            AL_PRINT("set cork_cause due to source deactivate");
            gettimeofday(&data->cork_cause_time, NULL);
        }
        else if(ActiveSourceCount > 0 && (data->cork_cause & CORK_CAUSE_STATE))
        {
            data->cork_cause &= ~CORK_CAUSE_STATE;
            AL_PRINT("unset cork_cause due to source activate");
        }
#endif

        if(ppa_stream_is_corked(data->stream) && data->cork_cause == CORK_CAUSE_NONE)
        {
            AL_PRINT("uncork");
            if(!(o = ppa_stream_cork(data->stream, 0, stream_success_cb, data)))
                AL_PRINT("failed to pa_stream_cork(0), error(%s)\n", ppa_strerror(ppa_context_errno(data->context)));
        }
        else if(!ppa_stream_is_corked(data->stream) && data->cork_cause != CORK_CAUSE_NONE)
        {
            unsigned long long diff_msec = 0;

            gettimeofday(&current_time, NULL);
            if(data->cork_cause_time.tv_sec < current_time.tv_sec)
                diff_msec = (current_time.tv_sec - data->cork_cause_time.tv_sec) * 1000;
            diff_msec += (current_time.tv_usec - data->cork_cause_time.tv_usec) / 1000;
            if (diff_msec >= CORK_TIMEOUT_MSEC)
            {
                AL_PRINT("cork cause:0x%x", data->cork_cause);
        ...
        if(!ppa_stream_is_corked(data->stream)) {
    ...
    pa_channel_map *chanmap)
{
   ...
   data->cork_cause = CORK_CAUSE_NONE;
#endif
...
// OpenAL {{{
#if defined (__TIZEN__) && defined (USE_ASM_IN_PULSEAUDIO)
ASM_cb_result_t asm_callback(int handle, ASM_event_sources_t event_src, ASM_sound_commands_t command, unsigned int sound_status, void* cb_data)
{
    ...
    case ASM_COMMAND_RESUME:
        ...
        /* unmute */
        data->cork_cause &= ~CORK_CAUSE_ASM;
        AL_PRINT("unset cork_cause due to ASM resume[event_src:%d]", event_src);
        break;
    case ASM_COMMAND_PAUSE:
    case ASM_COMMAND_STOP:
        /* mute */
        data->cork_cause |= CORK_CAUSE_ASM;
        AL_PRINT("set cork_cause due to ASM pause[event_src:%d]", event_src);
        if(!(o = ppa_stream_cork(data->stream, 1, stream_success_cb, data)))
            AL_PRINT("failed to pa_stream_cork(1), error(%s)\n", ppa_strerror(ppa_context_errno(data->context)));
        if(o)
        {
...
static int _get_asm_information(ASM_sound_events_t *type, int *options)
{
    ...
    if(_mm_session_util_read_information(-1, &cur_session, &session_options) < 0) {
        ...
        session_options = MM_SESSION_OPTION_UNINTERRUPTIBLE;
        ret = mm_session_update_option(MM_SESSION_UPDATE_TYPE_ADD, session_options);
        if (ret) {
            AL_PRINT("mm_session_update_option() failed\n");
            return MM_ERROR_SOUND_INTERNAL;
        }
    }
