...
#if defined (__TIZEN__) && defined (SUPPORT_CORK_BY_STATE)
ALsizei GetActiveSourceCount(ALCdevice *device)
{
    ALCcontext **ctx, **ctx_end;
    ALsource **src, **src_end;
    ALsizei ActiveSourceCount = 0;
    ...
    while(ctx != ctx_end)
    {
        ...
        ActiveSourceCount += (*ctx)->ActiveSourceCount;
    ...
    if (device->ActiveSourceCount != ActiveSourceCount)
    {
        device->ActiveSourceCount = ActiveSourceCount;
        AL_DEBUG("active sources:%d", ActiveSourceCount);
    }
...
#ifdef USE_DLOG
void al_print(unsigned int level, const char *fname, unsigned int line, const char *func, const char *fmt, ...)
#else
...
#endif
{
...
#ifdef USE_DLOG
    i = snprintf(str, sizeof(str), "%s: %s(%d) > ", fn, func, line);
#else
    ...
    str[sizeof(str)-1] = 0;
#ifdef USE_DLOG
    print_system_log(level, "MM_OPENAL", "%s", str);
#else
...
ALC_API ALCcontext* ALC_APIENTRY alcCreateContext(ALCdevice *device, const ALCint *attrList)
{
    ...
    AL_PRINT("create context:%p", ALContext);
...
ALC_API ALCvoid ALC_APIENTRY alcDestroyContext(ALCcontext *context)
{
    ...
    AL_PRINT("destroy context:%p (current active:%d)", context, context->ActiveSourceCount);
