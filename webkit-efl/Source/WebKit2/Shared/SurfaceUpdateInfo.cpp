...
void SurfaceUpdateInfo::encode(CoreIPC::ArgumentEncoder* encoder) const
{
    ...
    encoder->encode(platformSurfaceID);
    encoder->encode(platformSurfaceSize);
    encoder->encode(partialUpdate);
    encoder->encode(isLowScaleTile);
#endif
}
...
#if ENABLE(TIZEN_WEBKIT2_TILED_AC_SHARED_PLATFORM_SURFACE)
    if (!decoder->decode(result.platformSurfaceID))
        return false;
    if (!decoder->decode(result.platformSurfaceSize))
        return false;
    if (!decoder->decode(result.partialUpdate))
        return false;
    if (!decoder->decode(result.isLowScaleTile))
        return false;
#endif
