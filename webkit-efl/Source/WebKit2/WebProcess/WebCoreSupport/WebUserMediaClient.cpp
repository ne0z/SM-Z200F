...
void WebUserMediaClient::cancelUserMediaRequest(UserMediaRequest* request)
{
    return;
}
...
void WebUserMediaClient::startUserMedia()
{
    ...
    if (m_userMediaRequest.get()->video()) {
        ...
        if (m_userMediaRequest->rearcamera())
            source = MediaStreamSource::create(createCanonicalUUIDString(), MediaStreamSource::TypeVideo, "Back camera");
        else
...
{
    m_page->userMediaPermissionRequestManager()->requestPermission(context->securityOrigin(), this);
}
