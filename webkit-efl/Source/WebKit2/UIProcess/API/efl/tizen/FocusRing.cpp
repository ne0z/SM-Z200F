...
void FocusRing::show(const Vector<IntRect>& rects)
{
    ...
    for (size_t i = 0; i < rects.size(); ++i)
        sceneRects.append(m_viewImpl->transformToScene().mapRect(rects[i]));
    if (m_state == FocusRingShowState && m_rects == sceneRects)
        return;

    m_rects = sceneRects;

    if (!m_viewImpl->pageClient->pageDidRendered()) {
        if (m_focusRingObject)
    ...
    if (!m_isImageWithNoLink)
        evas_object_show(m_focusRingObject);

    if (m_state == FocusRingWaitRectWithContextMenuState)
        m_viewImpl->gestureClient->showContextMenu(centerPointInScreen());

#if ENABLE(TIZEN_LINK_MAGNIFIER)
    LinkMagnifierProxy::linkMagnifier().setClickedExactly(true);
#endif
...
void FocusRing::showWithContextMenu(const IntPoint& position, const Color& color, const Vector<IntRect>& rects)
{
    if (m_showTimer) {
        ecore_timer_del(m_showTimer);
        m_showTimer = 0;
    ...
    if (InputMethodContextEfl::isSystemKeypadShow()) {
        m_viewImpl->inputMethodContext()->hideIMFContext();
        m_state = FocusRingWaitHideKeypadState;
    } else {
        m_viewImpl->gestureClient->showContextMenu(m_position);
        show(rects);
    }
}

void FocusRing::didChangeVisibleContentRect(const IntPoint& previousScrollPosition, float previousScale)
{
    if (m_state == FocusRingWaitHideKeypadState) {
        m_state = FocusRingWaitRectWithContextMenuState;
        m_viewImpl->page()->recalcFocusedRects();
    } else if (m_state == FocusRingShowState && !m_rects.isEmpty()
        && (m_viewImpl->scrollPosition() != previousScrollPosition || m_viewImpl->scaleFactor() != previousScale)) {
...
#if ENABLE(TIZEN_WEBKIT2_TILED_BACKING_STORE)
    double scaleFactor = m_viewImpl->pageClient->scaleFactor();
    if (scaleFactor - 1 > numeric_limits<float>::epsilon())
        fingerSize.scale(1 / scaleFactor, 1 / scaleFactor);
#endif
    WebHitTestResult::Data hitTestResultData = m_viewImpl->page()->hitTestResultAtPoint(contentsPosition, WebHitTestResult::HitTestModeDefault | WebHitTestResult::HitTestModeSetFocus, fingerSize);
#else
    WebHitTestResult::Data hitTestResultData = m_viewImpl->page()->hitTestResultAtPoint(contentsPosition, WebHitTestResult::HitTestModeDefault | WebHitTestResult::HitTestModeSetFocus);
#endif

    if (hitTestResultData.focusedRects.isEmpty())
        return;

    if (!hitTestResultData.absoluteImageURL.isEmpty() && hitTestResultData.absoluteLinkURL.isEmpty() && equalIgnoringCase(hitTestResultData.nodeData.tagName, "img")) {
        m_isImageWithNoLink = true;
        return;
    }
...
bool FocusRing::ensureFocusRingObject()
{
    ...
    m_focusRingObject = evas_object_image_filled_add(evas_object_evas_get(m_viewImpl->view()));
    ...
    if (m_imagePath.isNull()) {
        ...
        if (m_focusRingColor.isValid())
            ...
            if (EflAssistHandle) {
                ...
                int r, g, b;
                (*webkit_ea_theme_color_get)("B041P", &r, &g, &b, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);

                Color focusRingColor(r, g, b, 80);
                focusRingColor.getRGBA(red, green, blue, alpha);
            }
        }
...
void FocusRing::requestToHide(bool immediately)
{
    ...
    if (m_hideTimer)
        ecore_timer_del(m_hideTimer);
    m_hideTimer = ecore_timer_add(static_cast<double>(s_hideTimerTime) / 1000, hideTimerCallback, this);
}
...
void FocusRing::hide(bool onlyColorDrawing)
{
    ...
    m_rects.clear();
    m_state = FocusRingHideState;
    m_viewImpl->page()->clearFocusedNode();
}

bool FocusRing::rectsChanged(const Vector<IntRect>& newRects)
{
     return (m_state == FocusRingShowState && unionRect(m_rects) != m_viewImpl->transformToScene().mapRect(unionRect(newRects)));
}
...
IntPoint FocusRing::centerPointInScreen()
{
    ...
    evas_object_geometry_get(m_viewImpl->view(), &x, &y, &width, &height);
