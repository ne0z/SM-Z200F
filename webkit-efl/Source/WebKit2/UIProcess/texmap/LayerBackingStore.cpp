...
void LayerBackingStore::createTile(int id, float scale, bool)
{
    createTile(id, scale);
}
...
{
#if ENABLE(TIZEN_WEBKIT2_TILED_AC)
    // backing store could be cleared by SuspendPainting.
...
bool LayerBackingStore::isEmpty() const
{
    return static_cast<size_t>(m_tiles.size()) == m_tilesToRemove.size();
}
...
#elif ENABLE(TIZEN_WEBKIT2_DEBUG_BORDERS)
...
void LayerBackingStore::paintToTextureMapper(TextureMapper* textureMapper, const FloatRect& targetRect, const TransformationMatrix& transform, float opacity)
{
...
#if ENABLE(TIZEN_WEBKIT2_TILED_AC)
    LOG(AcceleratedCompositing, "[UI ] layer paint %d tiles (of %d tiles) @%s\n",
        tilesToPaint.size(), m_tiles.size(), "LayerBackingStore::paintToTextureMapper");
#endif
    ...
    for (size_t i = 0; i < tilesToPaint.size(); ++i) {
        ...
        tile->paint(textureMapper, transform, opacity, edgesExposed);
        static bool shouldDebug = shouldShowTileDebugVisuals();
        if (!shouldDebug)
            continue;

        textureMapper->drawBorder(Color(0xFF, 0, 0), 2, tile->rect(), transform);
        textureMapper->drawNumber(static_cast<LayerBackingStoreTile*>(tile)->repaintCount(), 8, tilesToPaint[i]->rect().location(), transform);
    }
}
