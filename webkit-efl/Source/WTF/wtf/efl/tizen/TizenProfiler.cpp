...
#if PROFILE_EXCLUDE_NESTED_CALLS
// When A calls B, the elapsed time for A will be A - B then show all nested calls
bool TizenProfiler::s_excludeNested = getenv("TIZEN_PROFILE_EXCLUDE_NESTED") ? true : false;
#endif
...
void TizenProfiler::printByFunctions(FILE* fp)
{
...
#if PROFILE_EXCLUDE_NESTED_CALLS
            if (s_excludeNested) {
                size_t startItemIndex = i;

                while (--startItemIndex >= 0) {
                    if (item->m_startItem == s_vectorItems[startItemIndex]) {
                        if (startItemIndex < i - 2)
                            data->time -= USEC(s_vectorItems[i - 1]->m_time.tv_sec - s_vectorItems[startItemIndex + 1]->m_time.tv_sec,
                                            s_vectorItems[i - 1]->m_time.tv_usec - s_vectorItems[startItemIndex + 1]->m_time.tv_usec);
                        break;
                    }
                }
            }
    ...
    for (HashMap<const char*, FunctionProfileItem*>::iterator it = m_countFunctionMap.begin(); it != end; ++it) {
        fprintf(fp, "%s\t%u\t%u\n", it->first, it->second->count, it->second->time);
        profileFunctionCount += it->second->count;
        profileFunctionTotalTime += it->second->time;
    }
