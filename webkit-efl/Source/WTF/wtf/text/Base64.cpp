...
#if ENABLE(TIZEN_BASE64)
    unsigned sawEqualsSign = 0;
#else
...
#if ENABLE(TIZEN_BASE64)
    if (policy == Base64FailOnInvalidCharacter) {
        // If the length of input is divisible by 4, and input ends with one or two = characters, remove the trailing = characters.
        if (sawEqualsSign <= 2 && !((outLength+sawEqualsSign) % 4))
            sawEqualsSign = 0;
