...
#endif
#if ENABLE(TIZEN_TEXT_CODEC_MEMORY_REDUCTION)
        BufferExternal
#endif
...
public:
    ...
    WTF_EXPORT_PRIVATE static PassRefPtr<StringImpl> create8BitIfPossible(const UChar*, unsigned length);
...
#if ENABLE(TIZEN_TEXT_CODEC_MEMORY_REDUCTION)
    WTF_EXPORT_PRIVATE static PassRefPtr<StringImpl> adopt(StringBufferConst<LChar>&);
#endif
...
public:
    ...
    WTF_EXPORT_PRIVATE PassRefPtr<StringImpl> replace(UChar, const LChar*, unsigned replacementLength);
