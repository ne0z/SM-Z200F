...
#if ENABLE(TIZEN_FULLSCREEN_API)
static StyleSheetContents* fullscreenHorizontalStyleSheet;
static StyleSheetContents* fullscreenVerticalStyleSheet;
#endif
...
void StyleResolver::addKeyframeStyle(const Vector<StyleRuleKeyframes*>& rules)
{
    size_t size = rules.size();
    for (size_t i = 0; i < size; ++i) {
        AtomicString s(rules[i]->name());
        m_keyframesRuleMap.set(s.impl(), rules[i]);
    }
}
#endif
...
static void ensureDefaultStyleSheetsForElement(Element* element)
{
...
#if ENABLE(TIZEN_FULLSCREEN_API)
        String fullscreenRules = String(fullscreenUserAgentStyleSheet, sizeof(fullscreenUserAgentStyleSheet));
#else
    ...
    if (page && page->mainFrame() && page->mainFrame()->view() && element->document()->webkitIsFullScreen() && element->isMediaElement()) {
        IntRect visibleContentRect = page->mainFrame()->view()->visibleContentRect();
        if (!fullscreenHorizontalStyleSheet && visibleContentRect.width() > visibleContentRect.height()) {
        ...
        } else if (!fullscreenVerticalStyleSheet && visibleContentRect.width() < visibleContentRect.height()) {
            ...
            fullscreenHorizontalStyleSheet = 0;
        }
    }
#endif
...
void StyleResolver::collectMatchingRulesForRegion(RuleSet* rules, int& firstRuleIndex, int& lastRuleIndex, const MatchOptions& options)
{
    ...
    for (unsigned i = 0; i < size; ++i) {
        const CSSSelector* regionSelector = rules->m_regionSelectorsAndRuleSets.at(i).selector;
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
    addKeyframeStyle(defaultStyle->keyframeRules());
#endif
...
#if PLATFORM(TIZEN)
    m_currentMatchResult = &matchResult;
#endif
...
{
    return primitiveValue ? primitiveValue->convertToLength<FixedIntegerConversion | PercentConversion | FractionConversion | CalculatedConversion | ViewportPercentageConversion>(style, rootStyle, multiplier) : Length(Undefined);
}
...
{
    return primitiveValue ? primitiveValue->convertToLength<FixedFloatConversion | PercentConversion | FractionConversion | CalculatedConversion | ViewportPercentageConversion>(style, rootStyle, multiplier) : Length(Undefined);
}
...
#if ENABLE(TIZEN_FULLSCREEN_API)
        collectCSSOMWrappers(m_styleRuleToCSSOMWrapperMap, m_styleSheetCSSOMWrapperSet, fullscreenHorizontalStyleSheet);
        collectCSSOMWrappers(m_styleRuleToCSSOMWrapperMap, m_styleSheetCSSOMWrapperSet, fullscreenVerticalStyleSheet);
#endif
...
#if PLATFORM(TIZEN)
bool StyleResolver::currentMatchResultHasAuthorStylesForProperty(RenderStyle* style, CSSPropertyID propertyID) const
{
    if (!m_currentMatchResult || style != m_style || m_currentMatchResult->ranges.firstAuthorRule < 0)
        return false;

    for (int i = m_currentMatchResult->ranges.firstAuthorRule; i <= m_currentMatchResult->ranges.lastAuthorRule; ++i) {
        const MatchedProperties& matchedProperties = m_currentMatchResult->matchedProperties[i];
        if (matchedProperties.properties->getPropertyCSSValue(propertyID))
            return true;
    }
