...
public:
    ...
    bool checkSelector(const CSSSelector*, Element*, bool isFastCheckableSelector = false) const;
...
private:
    ...
    bool checkScrollbarPseudoClass(const CSSSelector*, PseudoId& dynamicPseudo) const;
...
inline bool SelectorChecker::tagMatches(const Element* element, const CSSSelector* selector)
{
    ...
    if (localName != starAtom && localName != element->localNameInlined())
        return false;
