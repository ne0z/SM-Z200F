...
CSSParserContext::CSSParserContext(CSSParserMode mode, const KURL& baseURL)
    ...
    , isWebkitTextSizeAdjustEnabled(true)
#endif
...
#endif
#if ENABLE(TIZEN_DISABLE_WEBKIT_TEXT_SIZE_ADJUST)
    , isWebkitTextSizeAdjustEnabled(document->settings() ? document->settings()->webkitTextSizeAdjustEnabled() : true)
#endif
...
#endif
#if ENABLE(TIZEN_DISABLE_WEBKIT_TEXT_SIZE_ADJUST)
        && a.isWebkitTextSizeAdjustEnabled == b.isWebkitTextSizeAdjustEnabled
#endif
...
#if ENABLE(TIZEN_DISABLE_WEBKIT_TEXT_SIZE_ADJUST)
        if (!parserContext.isWebkitTextSizeAdjustEnabled)
            return false;
#endif
...
bool CSSParser::validUnit(CSSParserValue* value, Units unitflags, CSSParserMode cssParserMode)
{
    ...
    case CSSPrimitiveValue::CSS_VMIN:
        b = (unitflags & FLength);
        break;
#if ENABLE(TIZEN_MEDIA_QUERY)
    case CSSPrimitiveValue::CSS_DPCM:
        b = (unitflags & FLength);
        value->unit = CSSPrimitiveValue::CSS_DPCM;
        break;
#endif
...
bool CSSParser::parseFillShorthand(CSSPropertyID propId, const CSSPropertyID* properties, int numProperties, bool important)
{
    ...
    for (i = 0; i < numProperties; i++) {
        ...
        if (properties[i] == CSSPropertyBackgroundPosition) {
            ...
            continue;
#if ENABLE(TIZEN_NOT_INITIAIZE_BACKGROUND_SIZE_IN_SHORTHAND_WORKAROUND)
        else if (properties[i] == CSSPropertyBackgroundSize && !parsedProperty[i])
            continue;
#endif
        else
...
PassRefPtr<CSSValue> CSSParser::parseColumnWidth()
{
    ...
    if (value->id != CSSValueAuto && !validUnit(value, FLength | FNonNeg, CSSStrictMode))
        return nullptr;
...
PassRefPtr<CSSValue> CSSParser::parseColumnCount()
{
    ...
    if (value->id != CSSValueAuto && !validUnit(value, FPositiveInteger, CSSQuirksMode))
        return nullptr;
...
bool CSSParser::parseColumnsShorthand(bool important)
{
    ...
    if (columnWidth)
        ...
        addProperty(CSSPropertyWebkitColumnWidth, cssValuePool().createIdentifierValue(CSSValueAuto), important, !hasPendingExplicitAuto /* implicit */);
    ...
    if (columnCount)
        ...
        addProperty(CSSPropertyWebkitColumnCount, cssValuePool().createIdentifierValue(CSSValueAuto), important, !hasPendingExplicitAuto /* implicit */);
    ...
    case CharacterIdentifierStart:
        ...
        if (UNLIKELY(*m_currentCharacter == '(')) {
            ...
            if (m_parsingMode == SupportsMode && !hasEscape) {
                detectSupportsToken(result - m_tokenStart);
                if (m_token != IDENT)
                    break;
            }
#endif
...
#if ENABLE(TIZEN_MEDIA_QUERY)
    //FIXME : Upversion - Must reviewd by Gyuyoung Kim
    case DPIS:
    case DPCMS:
        break;
#endif
