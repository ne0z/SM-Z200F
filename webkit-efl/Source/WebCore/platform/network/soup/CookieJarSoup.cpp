...
static SoupCookieJar* cookieJarForDocument(const Document* document)
{
    ...
    return SOUP_COOKIE_JAR(soup_session_get_feature(context ? context->soupSession() : WebCore::ResourceHandle::defaultSession(), SOUP_TYPE_COOKIE_JAR));
#else
    if (!context)
        return 0;
...
// FIXME : libsoup should be upgraded!!
// soup_cookie_jar_get_cookie_list() and soup_cookie_jar_add_cookie_with_first_party() need libsoup2.4_2.39.
...
{
#if ENABLE(TIZEN_CONSIDER_COOKIE_DISABLED_IF_SOUP_COOKIE_JAR_ACCEPT_NEVER_IS_SET)
    SoupCookieJar* jar = cookieJarForDocument(document);
    if (!jar)
        return false;

    SoupCookieJarAcceptPolicy policy = soup_cookie_jar_get_accept_policy(jar);
    // When we enable/disable "Accept cookies" in browser settings, it sets SOUP_COOKIE_JAR_ACCEPT_ALWAYS/SOUP_COOKIE_JAR_ACCEPT_NEVER
    // value to SoupCookieJarAcceptPolicy. So returning false in case of SOUP_COOKIE_JAR_ACCEPT_NEVER otherwise returning true.
    if (policy == SOUP_COOKIE_JAR_ACCEPT_NEVER)
        return false;

    return true;
#else
    !!cookieJarForDocument(document);
#endif
}
