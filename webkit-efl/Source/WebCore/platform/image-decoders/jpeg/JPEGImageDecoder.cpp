...
#if ENABLE(TIZEN_JPEG_IMAGE_SCALE_DECODING)
            scaleValue = m_decoder->calculateScale();
            m_info.scale_denom *= scaleValue;
#endif
...
bool JPEGImageDecoder::outputScanlines(ImageFrame& buffer)
{
    ...
    if (bufferStride && width != bufferStride / static_cast<int>(sizeof(unsigned))) {
        jumpLast = true;
...
#if !ENABLE(TIZEN_JPEG_IMAGE_SCALE_DECODING)
#if ENABLE(TIZEN_USE_XPIXMAP_DECODED_IMAGESOURCE)
        if (!buffer.setSize(scaledSize().width(), scaledSize().height(), m_isGLTargetSurface, m_decodeInCpuMemoryForcely))
#else
        if (!buffer.setSize(scaledSize().width(), scaledSize().height()))
#endif
#else
#if ENABLE(TIZEN_USE_XPIXMAP_DECODED_IMAGESOURCE)
        if (!buffer.setSize(info->output_width, info->output_height, m_isGLTargetSurface, m_decodeInCpuMemoryForcely))
#else
...
#if ENABLE(TIZEN_JPEG_IMAGE_SCALE_DECODING)
unsigned int JPEGImageDecoder::calculateScale() {
    unsigned int scaleDenom = 1;
    jpeg_decompress_struct* info = m_reader->info();
    int imageSize = info->image_width*info->image_height;
    int maxSize = maxNumPixels();

    // Adapt 1/2 scaling value for preventing image quality degradation due to scaled jpeg decoding
    if (imageSize > 0 && maxSize > 0 && imageSize > maxSize ) {
        scaleDenom = 2;
    }
