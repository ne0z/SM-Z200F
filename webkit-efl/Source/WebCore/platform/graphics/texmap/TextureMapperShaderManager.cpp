...
static const char* fragmentShaderSourceSimpleUV =
    FRAGMENT_SHADER(
        uniform sampler2D s_source;
        uniform lowp vec2 u_textureSize;
        uniform lowp float u_opacity;
        ...
        void main(void)
        {
            lowp vec4 color = texture2D(s_source, u_textureSize * v_sourceTexCoord);
            gl_FragColor = vec4(color.rgb * u_opacity, color.a * u_opacity);
        }
    );

static const char* fragmentShaderSourceOpacityAndMaskUV =
    FRAGMENT_SHADER(
        uniform sampler2D s_source;
        uniform sampler2D s_mask;
        uniform lowp vec2 u_textureSize;
        ...
        {
            lowp vec4 color = texture2D(s_source, u_textureSize * v_sourceTexCoord);
            lowp vec4 maskColor = texture2D(s_mask, u_textureSize * v_maskTexCoord);
            lowp float fragmentAlpha = u_opacity * maskColor.a;
            gl_FragColor = vec4(color.rgb * fragmentAlpha, color.a * fragmentAlpha);
        }
    );
...
#if ENABLE(TIZEN_UV_MAPPING)
    case SimpleUV:
        program = TextureMapperShaderProgramSimpleUV::create();
        break;
    case OpacityAndMaskUV:
        program = TextureMapperShaderProgramOpacityAndMaskUV::create();
        break;
#endif
    case Invalid:
        ASSERT_NOT_REACHED();
...
#if ENABLE(TIZEN_UV_MAPPING)
TextureMapperShaderProgramSimpleUV::TextureMapperShaderProgramSimpleUV()
    : TextureMapperShaderProgram(vertexShaderSourceSimple, fragmentShaderSourceSimpleUV)
{
    initializeProgram();
    getUniformLocation(m_flipLocation, "u_flip");
    getUniformLocation(m_matrixLocation, "u_matrix");
...
TextureMapperShaderProgramOpacityAndMaskUV::TextureMapperShaderProgramOpacityAndMaskUV()
    : TextureMapperShaderProgram(vertexShaderSourceOpacityAndMask, fragmentShaderSourceOpacityAndMaskUV)
{
    initializeProgram();
    getUniformLocation(m_matrixLocation, "u_matrix");
    getUniformLocation(m_flipLocation, "u_flip");
