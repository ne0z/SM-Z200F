...
{
#if ENABLE(TIZEN_WEBKIT2_TILED_AC)
    if (graphicsSurfaceToken != m_graphicsSurfaceToken || m_graphicsSurfaceSize != surfaceSize) {
#else
...
#if ENABLE(TIZEN_WEBKIT2_TILED_AC)
        setSurface(GraphicsSurface::create(surfaceSize, surfaceFlags, graphicsSurfaceToken, flags));
#else
    ...
    if (surface && surface->frontBuffer() != frontBuffer)
        ...
        surface->swapBuffers(frontBuffer);
#else
        surface->swapBuffers();
#endif
}
