...
#if ENABLE(TIZEN_CSS_FILTER_OPTIMIZATION)
    unsigned char* data = pixelArray->data();
    for (unsigned pixelOffset = 0; pixelOffset < pixelArrayLength; pixelOffset += 4) {
        for (unsigned channel = 0; channel < 4; ++channel) {
            // In this case, we can avoid calling of set function
            // because the type of tables is unsigned char.
            data[pixelOffset + channel] = tables[channel][data[pixelOffset + channel]];
        }
    }
#else
    for (unsigned pixelOffset = 0; pixelOffset < pixelArrayLength; pixelOffset += 4) {
