...
#if ENABLE(TIZEN_FIX_WRONG_NORMALIZED_ANIMATION)
    double normalized = 0;
    if (loopCount == iterationCount) {
        if (direction != Animation::AnimationDirectionAlternate)
            normalized = 1.0;
    } else
        normalized = remainder / duration;
#else
