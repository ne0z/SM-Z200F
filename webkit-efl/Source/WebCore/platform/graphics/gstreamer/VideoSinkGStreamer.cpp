...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
static gboolean
webkit_video_sink_event(GstBaseSink* base_sink, GstEvent* event)
{
    gboolean ret = TRUE;

    WebKitVideoSinkPrivate* priv = WEBKIT_VIDEO_SINK(base_sink)->priv;
    WebKitVideoSink* sink = WEBKIT_VIDEO_SINK(base_sink);
    ...
    case GST_EVENT_CUSTOM_DOWNSTREAM_OOB:
        if (structure) {
            ...
            guint addrSize = sizeof(addr);

#ifdef GST_API_VERSION_1
            GstBuffer* buf = gst_buffer_new_allocate(NULL, addrSize, NULL);
            gst_buffer_fill (buf, 0, (guint8*) addr, addrSize);
#else
            GstBuffer *buf = gst_buffer_new();
            GST_BUFFER_MALLOCDATA(buf) = (guint8*) g_malloc(addrSize);
            GST_BUFFER_DATA(buf) = GST_BUFFER_MALLOCDATA(buf);
            gst_buffer_set_data(buf, (guint8*) addr, addrSize);
#endif
    ...
    default:
        ...
        if (GST_BASE_SINK_CLASS(parent_class)->event) {
            ...
            ret = GST_BASE_SINK_CLASS(parent_class)->event(base_sink, event);
        }
    break;
    }
    return ret;
...
static void webkit_video_sink_class_init(WebKitVideoSinkClass* klass)
{
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
    baseSinkClass->event = webkit_video_sink_event;
#endif
#ifdef GST_API_VERSION_1
    ...
    webkitVideoSinkSignals[REPAINT_REQUESTED] = g_signal_new("repaint-requested",
            ...
            GST_TYPE_BUFFER);
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
    webkitVideoSinkSignals[IGNORE_REQUESTED] = g_signal_new("ignore-requested",
            G_TYPE_FROM_CLASS(klass),
            static_cast<GSignalFlags>(G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION),
            0, // Class offset
            0, // Accumulator
