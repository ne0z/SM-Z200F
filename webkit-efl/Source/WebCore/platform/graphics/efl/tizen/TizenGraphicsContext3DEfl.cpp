...
bool GraphicsContext3D::makeContextCurrent()
{
    ...
    return m_internal->makeContextCurrent();
}
...
{
    m_internal->prepareTexture(m_currentWidth, m_currentHeight);
}
...
{
    m_internal->releaseShaderCompiler();
}
...
{
    m_internal->readRenderingResults(pixels, pixelsSize, m_currentWidth, m_currentHeight);
}

#if ENABLE(TIZEN_WEBGL_ANIMATION_SNAPSHOT_FIX)
void GraphicsContext3D::paintRenderingResultsToCanvas(ImageBuffer* imageBuffer, DrawingBuffer*, bool isForcePaint)
{
    m_internal->paintRenderingResultsToCanvas(imageBuffer, m_currentWidth, m_currentHeight, isForcePaint);
}
#else
void GraphicsContext3D::paintRenderingResultsToCanvas(ImageBuffer* imageBuffer, DrawingBuffer*)
{
    m_internal->paintRenderingResultsToCanvas(imageBuffer, m_currentWidth, m_currentHeight);
}
#endif
...
{
    return m_internal->paintRenderingResultsToImageData(m_currentWidth, m_currentHeight);
}
...
bool GraphicsContext3D::getImageData(Image* image, GC3Denum format, GC3Denum type, bool premultiplyAlpha, bool ignoreGammaAndColorProfile, Vector<uint8_t>& outputVector)
{
    ...
    if (image->filenameExtension() == "jpg" && static_cast<unsigned>(decoder.size().width() * decoder.size().height()) > decoder.maxPixelsPerDecodedImage()) {
        float scaleX = static_cast<float>(decoder.size().width()) / static_cast<float>(width);
        float scaleY = static_cast<float>(decoder.size().height()) / static_cast<float>(height);

        RefPtr<cairo_surface_t> expand_image = adoptRef(cairo_image_surface_create(cairo_image_surface_get_format(imageSurface.get()),
                                                  decoder.size().width(), decoder.size().height()));
        RefPtr<cairo_t> expand_cr = adoptRef(cairo_create(expand_image.get()));
        cairo_scale(expand_cr.get(), scaleX, scaleY);
        cairo_set_source_surface(expand_cr.get(), imageSurface.get(), 0, 0);
        cairo_set_operator(expand_cr.get(), CAIRO_OPERATOR_SOURCE);
        cairo_paint(expand_cr.get());

        imageSurface = expand_image;
        width = cairo_image_surface_get_width(imageSurface.get());
        height = cairo_image_surface_get_height(imageSurface.get());
    }
#endif
