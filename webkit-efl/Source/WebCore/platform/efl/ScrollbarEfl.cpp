...
PassRefPtr<Scrollbar> Scrollbar::createNativeScrollbar(ScrollableArea* scrollableArea, ScrollbarOrientation orientation, ScrollbarControlSize size, bool isFrame)
#else
...
#if !ENABLE(TIZEN_FIX_DRAWING_ADDITIONAL_SCROLLBAR)
    frameRectsChanged();
#endif
}
...
#if ENABLE(TIZEN_DISPLAY_SCROLLBAR_JUST_FOR_SCROLLVIEW)
    /* The scrollbar just for ScrollView is disaplyed.
       The scrollbar for other object is not displayed. (ex. DIV scrollbar)
       If the scrollbar is for ScrollView, scrollableArea() and parent() is same. */
    if (scrollableArea() != parent())
        return;
#endif
