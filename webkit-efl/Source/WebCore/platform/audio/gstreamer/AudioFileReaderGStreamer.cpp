...
public:
    ...
    PassOwnPtr<AudioBus> createBus(float sampleRate, bool mixToMono, bool async);
#else
...
gboolean AudioFileReader::handleMessage(GstMessage* message)
{
    ...
    case GST_MESSAGE_WARNING:
        ...
        TIZEN_LOGE("GST_MESSAGE_WARNING : %d, %s. Debug output: %s", error->code, error->message, debug.get());
#endif
...
#if ENABLE(TIZEN_DLOG_SUPPORT)
        TIZEN_LOGE("GST_MESSAGE_ERROR : %d, %s. Debug output: %s", error->code, error->message, debug.get());
#endif
...
#if ENABLE(TIZEN_WEB_AUDIO)
PassOwnPtr<AudioBus> AudioFileReader::createBus(float sampleRate, bool mixToMono, bool async)
#else
...
{
#if ENABLE(TIZEN_WEB_AUDIO)
    return AudioFileReader(filePath).createBus(sampleRate, mixToMono, false);
#else
...
#if ENABLE(TIZEN_WEB_AUDIO)
PassOwnPtr<AudioBus> createBusFromInMemoryAudioFile(const void* data, size_t dataSize, bool mixToMono, float sampleRate, bool async)
{
    return AudioFileReader(data, dataSize).createBus(sampleRate, mixToMono, async);
}
#else
