...
#if ENABLE(TIZEN_WEB_AUDIO)
    flags = static_cast<GParamFlags>(G_PARAM_READWRITE);
    g_object_class_install_property(objectClass,
                                    PROP_STOP,
                                    g_param_spec_boolean("stop", "stop",
                                                      "task stop",
                                                      false, flags));

    g_object_class_install_property(objectClass,
                                    PROP_START,
                                    g_param_spec_boolean("start", "start",
                                                      "task start",
                                                      false, flags));
#endif
...
static void webKitWebAudioSrcSetProperty(GObject* object, guint propertyId, const GValue* value, GParamSpec* pspec)
{
    ...
    case PROP_STOP:
        if (g_value_get_boolean(value)) {
            priv->skipPadChain = true;
            gst_task_stop(priv->task.get());
            GST_DEBUG_OBJECT(src, "webkitwebaudiosrc's task is stopped.");
        }
        break;
    case PROP_START:
        if (g_value_get_boolean(value)) {
            priv->skipPadChain = false;
            gst_task_start(priv->task.get());
            GST_DEBUG_OBJECT(src, "webkitwebaudiosrc's task is started.");
        }
        break;
#endif
    default:
