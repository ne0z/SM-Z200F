...
void XMLHttpRequest::didFail(const ResourceError& error)
{
    ...
    TIZEN_SECURE_LOGI("[Loader] this[%p], url [%s], domain[%s], errorCode[%d]", this, url().string().utf8().data(), error.domain().utf8().data(), error.errorCode());
#endif
...
#if ENABLE(TIZEN_UPDATE_CHARSET_WHEN_REUSING_XHR_OBJECT)
    if (m_responseEncoding.isEmpty() || m_mimeTypeOverride.isEmpty())
#else
