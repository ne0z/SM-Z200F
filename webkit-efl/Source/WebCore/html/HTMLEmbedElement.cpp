...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
    , m_isChangedToIframeElement(false)
#endif
{
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
bool HTMLEmbedElement::isPlugInYoutube()
{
    if (m_isChangedToIframeElement)
        return false;

    if (m_serviceType.isEmpty() || m_url.isEmpty())
        return false;

    return m_serviceType.contains("application/x-shockwave-flash", false) && m_url.contains("youtube.com", false);
}

void HTMLEmbedElement::changeToIframeElement()
{
    const String delimiters[2] = {"watch?v=", "/v/"};
    int delimiterCount = sizeof(delimiters) / sizeof(String);

    Node* parentNode = NULL;
    Node* currentNode = this;

    for (Node* node = this->parentOrHostNode(); node; node = node->parentOrHostNode()) {
    ...
    for (int i = 0; i < delimiterCount; ++i) {
        size_t videoIdDelimiterPos = m_url.find(delimiters[i], false);
        if (videoIdDelimiterPos == notFound)
            continue;

        StringBuilder embedUrl;
        embedUrl.append("http://www.youtube.com/embed/");
        embedUrl.append(m_url.substring(videoIdDelimiterPos + delimiters[i].length()));

        RefPtr<HTMLIFrameElement> iframeElement = HTMLIFrameElement::create(iframeTag, document());
        iframeElement->cloneDataFromElement(*this);
        iframeElement->setAttribute(srcAttr, embedUrl.toString());

        ExceptionCode ec;
        parentNode->replaceChild(iframeElement.release(), currentNode, ec);
    }
}
#endif
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
    if (isPlugInYoutube()) {
        m_isChangedToIframeElement = true;
        RunLoop::main()->dispatch(WTF::bind(&HTMLEmbedElement::changeToIframeElement, this));
        return;
    }
#endif
