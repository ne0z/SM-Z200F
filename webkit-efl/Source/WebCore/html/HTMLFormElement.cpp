...
#if ENABLE(TIZEN_PASSWORD_MANAGER)
bool HTMLFormElement::PersonalData::isEmpty() {
    if (!username || !password || username->value().isEmpty() || password->value().isEmpty())
        return true;
    else
        return false;
}

HTMLFormElement::PersonalData HTMLFormElement::collectPersonalData(const Vector<FormAssociatedElement*>& associatedElements)
{
    PersonalData personalData;
    //int maxPasswordsToRemember = 2;
    //int firstPasswordIndex = 0;
    int passwordIndex = 0;

    // First, we are looking for the password fields
    for (unsigned i = 0; i < associatedElements.size(); i++)
    {
        if (!associatedElements[i])
            continue;

        HTMLElement* htmlElement = toHTMLElement(associatedElements[i]);
        if (!htmlElement || !htmlElement->hasTagName(inputTag))
            continue;

        HTMLInputElement* inputElement = static_cast<HTMLInputElement *> (htmlElement);
        if (!inputElement || !inputElement->isEnabledFormControl())
            continue;

        // Some of web pages have more than one password input field     
        /*if ((personalData.passwords.size() < maxPasswordsToRemember) && (inputElement->inputType() == HTMLInputElement::PASSWORD))
        {
           if (personalData.passwords.isEmpty())
               firstPasswordIndex = i;

           personalData.passwords.append(inputElement);
        }*/
	
        if (inputElement->isPasswordField())
        {
            passwordIndex = i;
            personalData.password = inputElement;
            break;
        }
    }

    if (personalData.password)
    // Then, we are looking for the username field
    //if (!personalData.passwords.isEmpty())
    {
    	for (int i = passwordIndex - 1; i >= 0; i--)
        {
            ...
            {
                personalData.username = inputElement;
                break;
            }
        }
    }
