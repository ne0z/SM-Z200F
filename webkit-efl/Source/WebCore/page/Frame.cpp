...
{
    TIZEN_LOGI(" orientation: [%d]", m_orientation);

    if (Document* doc = document())
        doc->enqueueWindowEvent(Event::create(eventNames().orientationchangeEvent, false, false));

#if ENABLE(SCREEN_ORIENTATION_SUPPORT) && ENABLE(TIZEN_SCREEN_ORIENTATION_SUPPORT)
    if (Document* doc = document()) {
        if (doc->domWindow() && doc->domWindow()->screen()) {
            // FIXME: consider adding this event to the queue (similarly to window's
            //        orientationchangeand and resize).
            //        This event is often fired after ewk_view's reisze which causes
            //        content change and requires to do relayout which has not been done yet.
            doc->domWindow()->screen()->sendOrientationChangeEvent(m_orientation);
        }
    }
#endif
}
...
float Frame::contentsScaleFactor() const
{
    if (!m_page)
        return 0;
    return m_page->chrome()->client()->contentsScaleFactor();
}
#endif
