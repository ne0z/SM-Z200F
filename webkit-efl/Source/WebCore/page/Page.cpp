...
#endif // #if ENABLE(TIZEN_BROWSER_DASH_MODE)

namespace WebCore {

#if ENABLE(TIZEN_WEBKIT2_ABOUT_MEMORY) || ENABLE(TIZEN_NATIVE_MEMORY_SNAPSHOT)
...
#if ENABLE(TIZEN_BROWSER_DASH_MODE)
    , m_dashModeCheckTimer(this, &Page::dashModeCheckTimerFired)
    , m_dashModeRetries(0)
#endif // #if ENABLE(TIZEN_BROWSER_DASH_MODE)
#if ENABLE(TIZEN_CSP)
    , m_cspHeaderType (ContentSecurityPolicy::ReportOnly)
#endif
{
...
bool Page::goBack()
{
    ...
    TIZEN_LOGI("[Loader] goBack item = %p", item);
#endif
...
#if ENABLE(TIZEN_DLOG_SUPPORT)
    TIZEN_LOGI("[Loader] goForward item = %p", item);
#endif
...
{
#if ENABLE(TIZEN_DLOG_SUPPORT)
    TIZEN_LOGI("[Loader] goBackOrForward distance = %d", distance);
#endif
...
#if ENABLE(TIZEN_PREVENT_SAME_DOCEMENT_STOP_LOADING_FOR_YOUTUBE)
    if (m_mainFrame->loader()->history()->shouldStopLoadingForHistoryItem(item)) {
        if (!(equalIgnoringCase(item->url().host(), "m.youtube.com") && item->url().hasFragmentIdentifier() && equalIgnoringFragmentIdentifier(m_mainFrame->document()->url(), item->url())))
            m_mainFrame->loader()->stopAllLoaders();
         else
            TIZEN_LOGI("no call stopAllLoaders because of the redirect problem of youtube with fragmentIdentifier url");
    }
#else
    if (m_mainFrame->loader()->history()->shouldStopLoadingForHistoryItem(item))
        m_mainFrame->loader()->stopAllLoaders();
...
unsigned Page::pageCount() const
{
    ...
    if (!frameView) {
#if ENABLE(TIZEN_DLOG_SUPPORT)
        TIZEN_LOGI("FrameView :: %08x, MainFrame :: %08x",frameView, mainFrame());
#endif
        return 0;
    }
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
        MediaResourceControllerGStreamerTizen::mediaResourceController().updateVisibilityState(visibilityState == PageVisibilityStateVisible);
#endif
    }
}
...
#if ENABLE(TIZEN_BROWSER_DASH_MODE)
void Page::setDashModeIfNeeded(int retries)
{
    m_dashModeRetries = retries;
    m_dashModeCheckTimer.startOneShot(2.9);
}

void Page::dashModeCheckTimerFired(Timer<Page>*)
{
    if (m_dashModeRetries > 0) {
...
#if ENABLE(TIZEN_CSP)
void Page::setContentPolicy(const String& policy, ContentSecurityPolicy::HeaderType headerType)
{
    if(policy.length())
        m_contentPolicyString = policy;
    m_cspHeaderType = headerType;
#if ENABLE(TIZEN_DLOG_SUPPORT)
    if(policy.length()){
        TIZEN_LOGI("[CSP_POLICY] policy for widget : %s", policy.ascii().data());
    }
#endif
}

const String& Page::contentPolicyString() const
{
    return m_contentPolicyString;
}

ContentSecurityPolicy::HeaderType Page::headerType() const
{
    return m_cspHeaderType;
}
#endif //TIZEN_CSP
