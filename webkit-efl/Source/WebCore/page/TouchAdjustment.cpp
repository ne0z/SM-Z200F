...
void compileSubtargetList(const NodeList& intersectedNodes, SubtargetGeometryList& subtargets, NodeFilter nodeFilter, AppendSubtargetsForNode appendSubtargetsForNode)
{
    ...
    for (unsigned i = 0; i < length; ++i) {
        ...
        // PLM Bug[P121206-0475]: We can not click the button to see the next contents list in the cafe.daum.net.
        // FIXME: The TouchAdjustment algorithm finds the node which is not clickable but its ancestor is clickable,
        // because the all nodes are added to the candidates if its ancestor satisfies the nodeFilter.
        // So we can not click the correct clickable button if it has the non-clickable child.
        // It is strange that the nodes are added to the candidates if its ancestor satisfies the nodeFilter.
        // I think the node can be added to the candidates only if the "node" satisfies the nodeFilter (not ancestor).
        // The Bug[P121206-0475] is fixed with this patch and this patch should be submitted to the webkit opensource.
        // I will apply this patch to the Tizen to resolve the defect firstly,
        // and then upload to the opensource after investgating original codes more.
        if (nodeFilter(node))
