...
#if ENABLE(TIZEN_BROWSER_DASH_MODE)
        void setDashModeIfNeeded(int retries);
#endif // #if ENABLE(TIZEN_BROWSER_DASH_MODE)
...
#if ENABLE(TIZEN_BROWSER_DASH_MODE)
        int m_dashModeRetries;
        Timer<Page> m_dashModeCheckTimer;
        void dashModeCheckTimerFired (Timer<Page>*);
#endif // #if ENABLE(TIZEN_BROWSER_DASH_MODE)

#if ENABLE(TIZEN_CSP)
        String m_contentPolicyString;
        ContentSecurityPolicy::HeaderType m_cspHeaderType;
    public:
        void setContentPolicy(const String& policy, ContentSecurityPolicy::HeaderType headerType);
        const String& contentPolicyString() const;
        ContentSecurityPolicy::HeaderType headerType() const;
#endif //TIZEN_CSP
    };
