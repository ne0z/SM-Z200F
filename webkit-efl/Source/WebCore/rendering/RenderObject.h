...
#if ENABLE(TIZEN_ENLARGE_CARET_WIDTH)
const static char* caretEnvironmentVariable = getenv("TIZEN_WEBKIT_CARET_SET");
const int caretWidth = (caretEnvironmentVariable && (strncmp(caretEnvironmentVariable, "EMAIL", 5))) ? 3 : 2;
#else
...
public:
    ...
    FloatQuad localToContainerQuad(const FloatQuad&, RenderBoxModelObject* repaintContainer, MapCoordinatesFlags mode = 0, bool* wasFixed = 0) const;
    FloatPoint localToContainerPoint(const FloatPoint&, RenderBoxModelObject* repaintContainer, MapCoordinatesFlags mode = 0, bool* wasFixed = 0) const;
    ...
    virtual void mapLocalToContainer(RenderBoxModelObject* repaintContainer, TransformState&, MapCoordinatesFlags mode = ApplyContainerFlip, bool* wasFixed = 0) const;
    ...
    virtual bool checkIntersect(const IntRect& rect);
#endif
