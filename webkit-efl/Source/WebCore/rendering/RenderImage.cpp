...
RenderImage::RenderImage(Element* element, const float imageDevicePixelRatio)
...
void RenderImage::imageDimensionsChanged(bool imageSizeChanged, const IntRect* rect)
{
    ...
    if (intrinsicSizeChanged) {
        ...
        if (imageSizeChanged || width() != oldwidth || height() != oldheight || containingBlockNeedsToRecomputePreferredSize) {
            shouldRepaint = false;
            if (!selfNeedsLayout())
                setNeedsLayout(true);
        }
...
void RenderImage::paintReplaced(PaintInfo& paintInfo, const LayoutPoint& paintOffset)
{
    ...
    if (!loadsImagesAutomatically || !m_imageResource->hasImage() || m_imageResource->errorOccurred()) {
#else
    if (!m_imageResource->hasImage() || m_imageResource->errorOccurred()) {
        ...
        if (cWidth > 2 && cHeight > 2) {
...
#if ENABLE(TIZEN_DISPLAY_MISSING_IMAGES)
#if ENABLE(TIZEN_DISPLAY_IMAGE_ALT_TEXT)
            ...
            RefPtr<Image> image = m_imageResource->image(0, 0, loadsImagesAutomatically);

            if ((!loadsImagesAutomatically || m_imageResource->errorOccurred()) && (m_altText.isEmpty() || noTextDraw) && !image->isNull() && usableWidth >= image->width() && usableHeight >= image->height()) {
#else
            RefPtr<Image> image = m_imageResource->image(0, 0, loadsImagesAutomatically);

            if ((!loadsImagesAutomatically || m_imageResource->errorOccurred()) && !image->isNull() && usableWidth >= image->width() && usableHeight >= image->height()) {
#endif // ENABLE(TIZEN_DISPLAY_IMAGE_ALT_TEXT)
#else
            ...
            if (m_imageResource->errorOccurred() && !image->isNull() && usableWidth >= image->width() && usableHeight >= image->height()) {
                ...
                context->drawImage(image.get(), style()->colorSpace(), snapRectToDevicePixels(LayoutRect(paintOffset + imageOffset, imageSize), deviceScaleFactor), CompositeSourceOver, shouldRespectImageOrientation());
...
void RenderImage::paint(PaintInfo& paintInfo, const LayoutPoint& paintOffset)
{
    ...
    if (!isVideo())
#endif
    // Return the paint to prevent incremental image rendering to get a performance gain
    // Return case : foreground phase, has no decorations (border or background), has foreground image and is loading
    if (paintInfo.phase == PaintPhaseForeground && !hasBoxDecorations()
        && m_imageResource.get() && m_imageResource->cachedImage() && m_imageResource->cachedImage()->isLoading())
        return;
#endif
