...
bool KeyframeValue::hasRotateAnimationOnly() const
{
    for (HashSet<CSSPropertyID>::const_iterator it = properties().begin(); it != properties().end(); ++it) {
        if (CSSPropertyWebkitTransform != *it)
            return false;
    }
    return m_style->transform().hasRotateOnly();
}
#endif
