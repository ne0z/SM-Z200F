...
HitTestResult::HitTestResult(const HitTestResult& other)
    ...
    , m_innerChildImageNonSharedNode(other.innerChildImageNonSharedNode())
#endif
...
#if ENABLE(TIZEN_CHILD_NODE_IMAGE_URL)
Image* HitTestResult::childNodeImage() const
{
    if (!m_innerChildImageNonSharedNode)
        return 0;

    RenderObject* renderer = m_innerChildImageNonSharedNode->renderer();
    if (renderer && renderer->isImage()) {
        RenderImage* image = static_cast<WebCore::RenderImage*>(renderer);
        if (image->cachedImage() && !image->cachedImage()->errorOccurred())
            return image->cachedImage()->imageForRenderer(image);
...
String HitTestResult::altChildDisplayString() const
{
    if (!m_innerChildImageNonSharedNode)
        return String();

    if (m_innerChildImageNonSharedNode->hasTagName(imgTag)) {
        HTMLImageElement* image = static_cast<HTMLImageElement*>(m_innerChildImageNonSharedNode.get());
        return displayString(image->getAttribute(altAttr), m_innerChildImageNonSharedNode.get());
    }

    if (m_innerChildImageNonSharedNode->hasTagName(inputTag)) {
        HTMLInputElement* input = static_cast<HTMLInputElement*>(m_innerChildImageNonSharedNode.get());
        return displayString(input->alt(), m_innerChildImageNonSharedNode.get());
    }

    return String();
}

KURL HitTestResult::childImageNonSharedImageURL() const
{
    if (!m_innerChildImageNonSharedNode)
        return KURL();

    AtomicString urlString;
    Element* element = static_cast<Element*>(m_innerChildImageNonSharedNode.get());
    urlString = element->getAttribute(element->imageSourceAttributeName());
    return m_innerChildImageNonSharedNode->document()->completeURL(stripLeadingAndTrailingHTMLSpaces(urlString));
}
#endif
...
#if ENABLE(TIZEN_CHILD_NODE_IMAGE_URL)
KURL HitTestResult::findChildAbsoluteImageURL()
{
    if (!(m_innerNonSharedNode && m_innerNonSharedNode->document()))
        return KURL();

    if (!(m_innerNonSharedNode->renderer()))
        return KURL();
    ...
    if (m_innerChildImageNonSharedNode)
        ...
        for(Node* innerNonSharedNodeImage = m_innerNonSharedNode->firstChild(); innerNonSharedNodeImage; innerNonSharedNodeImage = innerNonSharedNodeImage->nextSibling()) {
            if (innerNonSharedNodeImage->hasTagName(embedTag)
                || innerNonSharedNodeImage->hasTagName(imgTag)
                || innerNonSharedNodeImage->hasTagName(inputTag)
                || innerNonSharedNodeImage->hasTagName(objectTag)
#if ENABLE(SVG)
                || innerNonSharedNodeImage->hasTagName(SVGNames::imageTag)
#endif
            ) {
                m_innerChildImageNonSharedNode = innerNonSharedNodeImage;
                foundUrl = true;
                break;
            }
        }
    ...
    if (foundUrl)
        return childImageNonSharedImageURL();
    else
        return KURL();
}
#endif
...
#if ENABLE(TIZEN_DRAG_SUPPORT)
bool HitTestResult::isDragSupport() const
{
    if (!m_innerNode)
        return false;
    ...
    DragState state;
    page->dragController()->delegateDragSourceAction(view->contentsToRootView(roundedPoint()));

    if (!page->dragController()->draggableNode(frame, innerNode(), roundedPoint(), state))
        return false;
