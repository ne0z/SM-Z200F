...
void RenderMarquee::updateMarqueePosition()
{
    ...
    if (activate) {
        ...
        if (!m_stopped)
            start();
#if ENABLE(TIZEN_WEBKIT2_TEXT_SELECTION)
    Frame *frame = m_layer->renderer()->document()->frame();
    ...
    IntRect marqueeRect = toRenderBox(m_layer->renderer())->absoluteContentBox();
      if (rects[0].intersects(marqueeRect) || rects[rects.size()-1].intersects(marqueeRect))
#if ENABLE(TIZEN_WEBKIT2_FORM_DATABASE) && ENABLE(TIZEN_WEBKIT2_AUTOFILL_PROFILE_FORM)
            frame->editor()->client()->respondToChangedSelection(frame, true, false);
#else
            frame->editor()->client()->respondToChangedSelection(frame, true);
#endif
#endif
    }
}
