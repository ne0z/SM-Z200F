...
{
#if ENABLE(TIZEN_RESTART_HIGH_QUALITY_TIMER_WHEN_NOT_ACTIVE)
    if (!m_timer.isActive())
#endif
...
{
    m_destRect.move(max(xOffset, LayoutUnit(0)), 0);
    m_phase.setX(-min(xOffset, LayoutUnit(0)));
    m_destRect.setWidth(m_tileSize.width() + min(xOffset, LayoutUnit(0)));
}
void RenderBoxModelObject::BackgroundImageGeometry::setNoRepeatY(LayoutUnit yOffset)
{
    m_destRect.move(0, max(yOffset, LayoutUnit(0)));
    m_phase.setY(-min(yOffset, LayoutUnit(0)));
    m_destRect.setHeight(m_tileSize.height() + min(yOffset, LayoutUnit(0)));
}
...
void RenderBoxModelObject::BackgroundImageGeometry::useFixedAttachment(const LayoutPoint& attachmentPoint)
{
    ...
    m_phase.move(max(alignedPoint.x() - m_destRect.x(), LayoutUnit(0)), max(alignedPoint.y() - m_destRect.y(), LayoutUnit(0)));
}
                                                            ...
                                                            BackgroundImageGeometry& geometry)
{
    ...
    if (backgroundRepeatX == RepeatFill)
        geometry.setPhaseX(geometry.tileSize().width() ? geometry.tileSize().width() - fmodf(xPosition + left, geometry.tileSize().width()) : 0);
    else
    ...
    if (backgroundRepeatY == RepeatFill)
        geometry.setPhaseY(geometry.tileSize().height() ? geometry.tileSize().height() - fmodf(yPosition + top, geometry.tileSize().height()) : 0);
    else 
                                       ...
                                       BackgroundBleedAvoidance bleedAvoidance, bool includeLogicalLeftEdge, bool includeLogicalRightEdge)
{
    ...
    //In Cairo port, this check must be removed because a rounded rect border can look different with a rect border because a complex clip handles different with a simple clip
#if !ENABLE(TIZEN_NOT_CHECK_ROUNDED_BORDER_ALL_CLIPPED_OUT)
                                                 ...
                                                 BoxSide side, bool firstEdgeMatches, bool secondEdgeMatches)
{
    ...
    // The antialias setting for rounded rects border must be ON because rounded rects painted with antialias on
#if ENABLE(TIZEN_ROUNDED_BORDER_CLIP_ANTIALIAS_ON)
    if (outerBorder.isRounded()) {
        graphicsContext->clipConvexPolygon(4, quad, true);
        return;
    }
#endif
