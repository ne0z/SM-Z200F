...
float TextAutosizer::clusterMultiplier(WritingMode writingMode, const TextAutosizingWindowInfo& windowInfo, float textWidth) const
{
    ...
    // 0.5f doesn't work for all the devices. It works for devices with scale factor of 2
    // but in devices with scale factor of 3 the font size is big
    float scaleValue = 1.0 / m_document->page()->deviceScaleFactor();
