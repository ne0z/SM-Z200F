...
RenderObject* RenderObject::createObject(Element* element, RenderStyle* style)
{
    ...
    if (doc->cssRegionsEnabled() && style->isDisplayRegionType() && !style->regionThread().isEmpty() && doc->renderView())
        return new (arena) RenderRegion(element, doc->renderView()->flowThreadController()->ensureRenderFlowThreadWithName(style->regionThread()));
...
bool RenderObject::checkIntersect(const IntRect& rect)
{
...
void RenderObject::setStyle(PassRefPtr<RenderStyle> style)
{
    ...
    if (RenderStyle* oldFirstLineStyle = oldStyle ? oldStyle->getCachedPseudoStyle(FIRST_LINE) : 0)
        updateFillImages(oldFirstLineStyle->backgroundLayers(), 0);
#endif
...
void RenderObject::mapLocalToContainer(RenderBoxModelObject* repaintContainer, TransformState& transformState, MapCoordinatesFlags mode, bool* wasFixed) const
{
    if (repaintContainer == this)
        return;
...
FloatQuad RenderObject::localToContainerQuad(const FloatQuad& localQuad, RenderBoxModelObject* repaintContainer, MapCoordinatesFlags mode, bool* wasFixed) const
{
...
FloatPoint RenderObject::localToContainerPoint(const FloatPoint& localPoint, RenderBoxModelObject* repaintContainer, MapCoordinatesFlags mode, bool* wasFixed) const
{
    TransformState transformState(TransformState::ApplyTransformDirection, localPoint);
    mapLocalToContainer(repaintContainer, transformState, mode | ApplyContainerFlip | UseTransforms, wasFixed);
    transformState.flatten();
...
#if ENABLE(TIZEN_FIRST_LINE_IMAGE_PAINTING)
        if (RenderStyle* firstLineStyle = m_style->getCachedPseudoStyle(FIRST_LINE))
            updateFillImages(firstLineStyle->backgroundLayers(), 0);
#endif
    }
...
#if ENABLE(TIZEN_FIRST_LINE_IMAGE_PAINTING)
    {
        RenderStyle* newPseudoStyle = style()->addCachedPseudoStyle(result.release());
        if (newPseudoStyle->styleType() == FIRST_LINE)
            const_cast<RenderObject*>(this)->updateFillImages(0, newPseudoStyle->backgroundLayers());
        return newPseudoStyle;
    }
#else
