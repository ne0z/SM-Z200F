...
void RenderReplaced::paint(PaintInfo& paintInfo, const LayoutPoint& paintOffset)
{
    ...
    if (drawSelectionTint) {
        ...
        paintInfo.context->fillRect(pixelSnappedIntRect(selectionPaintingRect), selectionBackgroundColor().blendWithWhite(), style()->colorSpace());
#else
...
LayoutUnit RenderReplaced::computeReplacedLogicalWidth(ShouldComputePreferred shouldComputePreferred) const
{
    ...
    if (style()->logicalWidth().isAuto()) {
        ...
        if (hasIntrinsicWidth)
            return computeReplacedLogicalWidthRespectingMinMaxWidth(roundToInt(constrainedSize.width()), shouldComputePreferred);
