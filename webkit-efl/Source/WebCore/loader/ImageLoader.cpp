...
#if ENABLE(TIZEN_REMOVE_IMG_SRC_ATTRIBUTE)
    if (!attr.isNull() && attr == m_failedLoadURL)
        return;

    if (attr.isNull() && m_image.get()) {
        if (RenderImageResource* imageResource = renderImageResource())
            imageResource->setCachedImage(0);

        setImage(0);

        RenderObject* renderer = client()->sourceElement()->renderer();
        if (renderer && renderer->isImage() && toRenderImage(renderer)->setImageSizeForAltText())
