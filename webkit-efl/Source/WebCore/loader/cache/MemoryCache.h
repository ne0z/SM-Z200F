...
#include "IntRect.h"
#include "tizen/AsyncResourceLoaderTizen.h"
#endif
...
public:
    ...
    void setCapacity(unsigned totalBytes) { m_capacity = totalBytes; }
    unsigned getMaxAsyncDecodeTotalBytes() const { return m_asyncImagedeCodeCapacity; }
#endif
...
#if ENABLE(TIZEN_BACKGROUND_DISK_CACHE)
    void restoreResourcesFromDisk();
    void restoreFinished(bool isfinished);
    void markVisibleContents(const IntRect& visibleRect);
    Vector<LRUList, 32> resources(void) { return m_allResources; }
    void setRestoredSize(unsigned size) { m_restoredSize = size; }
    unsigned restoredSize() { return m_restoredSize; }
#endif
    ...
    void removeAllDeadResources();
    void removeAllLiveDecodedResources();
    void removeAllLiveEncodedResources();
#endif

#if ENABLE(TIZEN_WEBKIT2_ABOUT_MEMORY)
    String dumpCachedResourceLists();
#endif
...
#if ENABLE(TIZEN_BACKGROUND_DISK_CACHE)
    unsigned m_restoredSize;
    bool m_restoreFinished;
...
#if ENABLE(TIZEN_JPEGIMAGE_DECODING_THREAD)
    unsigned m_asyncImagedeCodeCapacity;
#endif
};
