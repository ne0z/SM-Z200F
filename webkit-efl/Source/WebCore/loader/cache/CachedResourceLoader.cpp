...
bool CachedResourceLoader::canRequest(CachedResource::Type type, const KURL& url, bool forPreload)
{
    if (!document()->securityOrigin()->canDisplay(url)) {
        ...
        TIZEN_LOGI("[Loader] URL was not allowed by SecurityOrigin::canDisplay return 0!!");
#endif
        return 0;
    }
...
#if ENABLE(TIZEN_DLOG_SUPPORT)
            TIZEN_LOGI("[Loader] XSLStyleSheet m_document->securityOrigin()->canRequest");
#endif
            return false;
        }
#endif
...
#if ENABLE(TIZEN_DLOG_SUPPORT)
            TIZEN_LOGI("[Loader] XSLStyleSheet Script !allowScriptFromSource return false");
#endif
            return false;
        }
...
#if ENABLE(TIZEN_DLOG_SUPPORT)
                TIZEN_LOGI("[Loader] !allowScriptFromSource called didNotAllowScript() return false");
#endif
                return false;
            }
        }
...
#if ENABLE(TIZEN_DLOG_SUPPORT)
            TIZEN_LOGI("[Loader] CSSStyleSheet !allowStyleFromSource() return false");
#endif
            return false;
        }
        break;
...
#if ENABLE(TIZEN_DLOG_SUPPORT)
            TIZEN_LOGI("[Loader] ImageResource !allowImageFromSource() return false");
#endif
            return false;
        }
...
#if ENABLE(TIZEN_DLOG_SUPPORT)
                TIZEN_LOGI("[Loader] ImageResource !allowImage() return false");
#endif
                return false;
            }
        }
...
#if ENABLE(TIZEN_DLOG_SUPPORT)
            TIZEN_LOGI("[Loader] RawResource !allowMediaFromSource() return false");
#endif
            return false;
        }
        break;
...
CachedResourceHandle<CachedResource> CachedResourceLoader::requestResource(CachedResource::Type type, ResourceRequest& request, const String& charset, const ResourceLoaderOptions& options, ResourceLoadPriority priority, bool forPreload)
{
    ...
    if (!url.isValid()) {
#if ENABLE(TIZEN_DLOG_SUPPORT)
        TIZEN_LOGI("[Loader] !url.isValid() return 0");
#endif
        return 0;
    }
...
#if ENABLE(TIZEN_DLOG_SUPPORT)
        TIZEN_LOGI("[Loader] !canRequest return 0");
#endif
        return 0;
    }
    ...
    if (!resource) {
#if ENABLE(TIZEN_DLOG_SUPPORT)
        TIZEN_LOGI("[Loader] !resource return 0");
#endif
        return 0;
    }

    ASSERT(resource->url() == url.string());
    LOG(ResourceLoading, "Adding CachedResource %p for '%s' to hashmap.", resource.get(), request.url().string().latin1().data());
#if ENABLE(TIZEN_DLOG_SUPPORT)
    //TIZEN_LOGI("Adding CachedResource %p for '%s' to hashmap.", resource.get(), request.url().string().latin1().data());
#endif
...
CachedResourceHandle<CachedResource> CachedResourceLoader::loadResource(CachedResource::Type type, ResourceRequest& request, const String& charset, ResourceLoadPriority priority, const ResourceLoaderOptions& options)
{
    ...
    bool inCache = true;
    if (request.url().isLocalFile() && document()) {
        String cacheControl = document()->getCacheControl();
        if (equalIgnoringCase(cacheControl, "no-cache"))
            inCache = false;
    }
    ...
    if (resource->errorOccurred()) {
        if (inCache)
            memoryCache()->remove(resource.get());
#if ENABLE(TIZEN_DLOG_SUPPORT)
        TIZEN_LOGI("[Loader] resource->errorOccurred() return 0!!");
#endif
        return 0;
    }
...
#endif
#if ENABLE(TIZEN_CACHE_RESOURCE_DELETE_WITH_ITERATOR)
    //TIZEN_LOGI("Removing the resource[%p: URL:%s] with iterator instead of URL.", resource, resource->url().string().ascii().data());
    //TIZEN_LOGI("No of hashmap entries ::before delete : %d ",m_documentResources.size());
    DocumentResourceMap::iterator it = m_documentResources.find(resource->url());
    while (it != m_documentResources.end()){
        if(it->second.get() == resource){
            //TIZEN_LOGI("Found match:: itr:%p matches res:%p", it->second.get(), resource);
            m_documentResources.remove(it);
            break;
        }
        ++it;
    }
    //TIZEN_LOGI("No of hashmap entries ::after delete : %d ",m_documentResources.size());
#else
...
#if ENABLE(TIZEN_LOAD_REMOTE_IMAGES)
bool CachedResourceLoader::loadRemoteImages(const KURL& resourceUrl)
{
    if (!m_loadRemoteImages && !resourceUrl.protocolIsData()  && !resourceUrl.isLocalFile())
        return false;
    else
        return true;
}
void CachedResourceLoader::setLoadRemoteImages(bool loadRemoteImages)
{
    if (loadRemoteImages == m_loadRemoteImages)
        return;

    m_loadRemoteImages = loadRemoteImages;

    if (!m_loadRemoteImages || !m_autoLoadImages)
        return;
