...
#include "image-decoders/AsyncImageDecoder.h"
#endif
...
public:
    ...
    Image* imageForRenderer(const RenderObject*, bool autoLoadImages = true); // Returns the nullImage() if the image is not available yet.
#else
...
#if ENABLE(TIZEN_JPEGIMAGE_DECODING_THREAD)
    AsyncImageDecoder* getAsyncImageDecoder() { return s_asyncImageDecoder; }
    static void setAsyncImageDecoder(AsyncImageDecoder* decoder) { s_asyncImageDecoder = decoder; }
#endif
...
private:
    ...
    typedef pair<LayoutSize, float> SizeAndZoom;
...
#endif
#if ENABLE(TIZEN_JPEGIMAGE_DECODING_THREAD)
    static AsyncImageDecoder* s_asyncImageDecoder;
#endif
};
...
public:
    ...
    virtual bool checkIntersect(const IntRect& rect) { return false; }
#endif
};
