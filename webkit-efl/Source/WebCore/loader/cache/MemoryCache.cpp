...
static const unsigned cAsyncImageDecodeCapacity = 32 * 1024 * 1024; // This optimized value can be modified according to your system memory.
#endif
...
MemoryCache::MemoryCache()
    ...
    , m_restoredSize(0)
    , m_asyncLoader(0)
    , m_restoreFinished(false)
#endif
{
#if ENABLE(TIZEN_JPEGIMAGE_DECODING_THREAD)
    m_asyncImagedeCodeCapacity = cAsyncImageDecodeCapacity;
#endif
}
...
#else
            if (elapsedTime < cMinDelayBeforeLiveDecodedPrune && targetSize != 0)
#endif
                return;
...
#if !ENABLE(TIZEN_CLEAR_MEMORY_CACHE_BUG_FIX)
    setDisabled(true);
    setDisabled(false);
#else
    m_disabled = true;
...
#if ENABLE(TIZEN_BACKGROUND_DISK_CACHE)
void MemoryCache::restoreResourcesFromDisk()
{
    bufferDatabase().ensureOpen();

    if (!m_asyncLoader)
        m_asyncLoader = new AsyncResourceLoader(this);

    m_asyncLoader->startRestoreThread();

    while (OwnPtr<AsyncResourceLoader::DecodingTask> decodingTask = m_asyncLoader->m_queue.waitForMessage()) {
        if (decodingTask.get()->isfinished())
            break;
    }
}

void MemoryCache::restoreFinished(bool isfinished)
{
    m_restoreFinished = isfinished;
    return;
}

void MemoryCache::markVisibleContents(const IntRect& visibleRect)
{
    if (m_inPruneResources)
        return;
    ...
    for (int i = size - 1; i >= 0; i--) {
        ...
        while (current) {
            CachedResource* prev = current->m_prevInAllResourcesList;
            ASSERT(current->hasClients());
            if (current->isLoaded() && current->encodedSize()) {
                current->setLocatedInViewport(false);
                CachedResourceClientWalker<CachedResourceClient> w(current->clients());
                while (CachedResourceClient* c = w.next()) {
                    if (c->resourceClientType() == CachedResourceClient::ImageType && c->checkIntersect(visibleRect)) {
                        current->setLocatedInViewport(true);
                        continue;
                    }
                }
            }
...
#if ENABLE(TIZEN_JPEGIMAGE_DECODING_THREAD) || ENABLE(TIZEN_BACKGROUND_DISK_CACHE)
bool MemoryCache::isSizeAvailable()
{

    if (m_liveSize + m_deadSize <= m_capacity)
        return true;
...
#if ENABLE(TIZEN_CACHE_MEMORY_OPTIMIZATION)
void MemoryCache::removeAllDeadResources()
{
    if (!m_pruneEnabled)
        return;

    pruneDeadResourcesToSize(0);
}

void MemoryCache::removeAllLiveDecodedResources()
{
    pruneLiveResourcesToSize(0);
}

void MemoryCache::removeAllLiveEncodedResources()
{
#if ENABLE(TIZEN_BACKGROUND_DISK_CACHE)
    if (m_inPruneResources)
        return;
    ...
    for (int i = size - 1; i >= 0; i--) {
        // Destroy any encoded data in live objects that we can.
        // Remove from the tail, since this is the least frequently accessed of the objects.
        CachedResource* current = m_allResources[i].m_tail;
        while (current) {
            CachedResource* prev = current->m_prevInAllResourcesList;
            ASSERT(current->hasClients());
            if (current->isLoaded() && current->encodedSize()) {
                success = current->data()->evictToCache();
            }
            if (!success)
                break;
            current = prev;
...
#if ENABLE(TIZEN_WEBKIT2_ABOUT_MEMORY)
String MemoryCache::dumpCachedResourceLists()
{
    CachedResourceMap::iterator e = m_resources.end();
    StringBuilder urlList;
    for (CachedResourceMap::iterator it = m_resources.begin(); it != e; ++it) {
        CachedResource* resource = it->second;
        urlList.append(resource->url().string() + '\n');
    }
