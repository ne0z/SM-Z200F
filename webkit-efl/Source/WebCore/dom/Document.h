...
public:
    ...
    String getCacheControl() const { return m_cacheControl; }
    void setCacheControl(const String&);
#endif
    ...
    */
...
#if ENABLE(TIZEN_CSP)
    void copyContentSecurityPolicy(Document*);
#endif
    ...
    void setDashModeIfNeeded(double threshold);
    void setDashModeForDynamicCreateIfNeeded();
#endif
...
#if ENABLE(TIZEN_BROWSER_DASH_MODE)
    Timer<Document> m_dashModeCheckTimer;
    void dashModeCheckTimerFired (Timer<Document>*) {}
    double m_lastTimeForDynamicCreate;
    int m_countParseHtmlForDynamicCreate;
    Timer<Document> m_dashModeTimerForDynamicCreate;
    int m_dashModeRetriesForDynamicCreate;
    void dashModeTimerForDynamicCreateFired(Timer<Document>*);
#endif
...
inline Node::Node(Document* document, ConstructionType type)
    ...
    , m_next(0)
    , m_renderer(0)
#if ENABLE(TIZEN_ELEMENT_CREATED_BY_PARSER_INFO_STORE)
    , m_createdByParser(true)
#endif
{
    if (document)
        document->guardRef();
