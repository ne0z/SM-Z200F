...
PassRefPtr<Node> Element::cloneNode(bool deep)
{
    ...
    RefPtr<Element> element = deep ? cloneElementWithChildren() : cloneElementWithoutChildren();
    element->setCreatedByParser(isCreatedByParser());
        ...
        if (*newPseudoStyle != *pseudoStyleCache->at(i)) {
            ...
            if (pseudoId == FIRST_LINE || pseudoId == FIRST_LINE_INHERITED) {
                ...
                if (pseudoId == FIRST_LINE)
                    renderer()->updateFillImages(0, newPseudoStyle->backgroundLayers());
#endif
            }
            return true;
        }
