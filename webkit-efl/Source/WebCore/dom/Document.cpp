...
#include "wtf/efl/tizen/TizenSystemUtilities.h"
#endif
...
#if ENABLE(TIZEN_BROWSER_DASH_MODE)
    , m_dashModeCheckTimer(this, &Document::dashModeCheckTimerFired)
    , m_lastTimeForDynamicCreate(0)
    , m_countParseHtmlForDynamicCreate(0)
    , m_dashModeTimerForDynamicCreate(this, &Document::dashModeTimerForDynamicCreateFired)
#endif
#ifndef NDEBUG
...
#if ENABLE(TIZEN_BROWSER_DASH_MODE)
    if (m_dashModeCheckTimer.isActive())
        m_dashModeCheckTimer.stop();
    if (m_dashModeTimerForDynamicCreate.isActive())
        m_dashModeTimerForDynamicCreate.stop();
#endif
...
#if ENABLE(TIZEN_CACHE_LAST_RESULT_OF_CSS_SELECTOR_QUERY)
...
#if ENABLE(TIZEN_DISABLE_MEMORY_CACHE_FOR_FILE_SCHEME)
void Document::setCacheControl(const String& cacheControl)
{
    if (m_cacheControl == cacheControl)
        return;
...
{
    m_bParsing = b;
#if ENABLE(TIZEN_FORCE_LAYOUT_FOR_SETTINGS_PAGE)
    if (!m_bParsing && view()){
        if(!view()->needsLayout()){
            TIZEN_LOGI("--Marking needsLayout for FrameView--");
            // parsing has changed, so we just force one more layout
            // as this happens before the final progress complete, we can
            // afford one extra layout
            view()->setNeedsLayout();
        }
...
static bool isSeparator(UChar c)
{
    ...
    // Some websites are using seprator of viewport meta tag with ';'.
    // For example, facebook and youtube.
    // However, the ';' is not correct sperator in viewport specification.
    // I report this problem to webkit. WebKit guy said that facebook should
    // fix this wrong usage. For now, our browser needs to support ';' until facebook fixes this problem. - Gyuyoung Kim
    return c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '=' || c == ',' || c == '\0' || c == ';';
#else
    return c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '=' || c == ',' || c == '\0';
#endif
...
void Document::finishedParsing()
{
    ...
    if (RefPtr<Frame> f = frame()) {
        ...
        TIZEN_LOGI("[Loader] domContentLoadedEventFired");
#endif
...
Color Document::themeColor()
{
    ...
    for (unsigned int i = 0; i < length; ++i) {
        Node* child = children->item(i);
        if (!child->hasTagName(metaTag))
            continue;
        HTMLMetaElement* metaElement = static_cast<HTMLMetaElement*>(child);
        RGBA32 rgb;
        if (equalIgnoringCase(metaElement->name(), "theme-color") && CSSParser::fastParseColor(rgb, metaElement->content().stripWhiteSpace(), true))
            return Color(rgb);
    }
    return Color();
}
...
#if ENABLE(TIZEN_CSP)
void Document::copyContentSecurityPolicy(Document* other)
{
    contentSecurityPolicy()->copyStateFrom(other->contentSecurityPolicy());
}
#endif
...
#if ENABLE(TIZEN_EXTENSIBLE_API)
        if (!TizenExtensibleAPI::extensibleAPI().fullScreen())
...
#if ENABLE(TIZEN_BROWSER_DASH_MODE)
void Document::setDashModeIfNeeded(double threshold)
{
    if (WTF::dashModeSetTime() && !m_dashModeCheckTimer.isActive()) {
        if (currentTime() - WTF::dashModeSetTime() < threshold) {
            WTF::setWebkitDashMode(WTF::DashModeBrowserJavaScript, 3000);
            m_dashModeCheckTimer.startOneShot(2.9);
            if (page())
                page()->setDashModeIfNeeded(4);
        }
    }
}

#define MAX_DOM_DYNCREATE_COUNT 250
#define INTERVAL_DOM_DYNCREATE  0.050  // 50ms

void Document::setDashModeForDynamicCreateIfNeeded()
{
    if (m_countParseHtmlForDynamicCreate++ == 0)
        m_lastTimeForDynamicCreate = currentTime();
    if (m_countParseHtmlForDynamicCreate >= MAX_DOM_DYNCREATE_COUNT) {
        m_countParseHtmlForDynamicCreate = 0;
        if (!m_dashModeTimerForDynamicCreate.isActive()
            && ((currentTime() - m_lastTimeForDynamicCreate) < INTERVAL_DOM_DYNCREATE)) {
            m_dashModeRetriesForDynamicCreate = 1;
            WTF::setWebkitDashMode(WTF::DashModeBrowserJavaScript, 3000);
            m_dashModeTimerForDynamicCreate.startOneShot(2.9);
        }
    }
}

void Document::dashModeTimerForDynamicCreateFired(Timer<Document>*)
{
    // Dynamic create takes at least 6.5 ~ 7s. Single fire is not enough.
    if (m_dashModeRetriesForDynamicCreate--) {
        WTF::setWebkitDashMode(WTF::DashModeBrowserJavaScript, 3000);
        m_dashModeTimerForDynamicCreate.startOneShot(2.9);
    }
}
