...
void DynamicNodeListCacheBase::invalidateCache() const
{
    ...
    if (type() == NodeListCollectionType) {
        if (static_cast<const DynamicNodeList*>(this)->isTagNodeList())
            static_cast<const TagNodeList*>(this)->clearSnapshot();
        return;
    }
#else // ENABLE(TIZEN_CONSTRUCT_TAG_NODE_LIST_WITH_SNAPSHOT)
    if (type() == NodeListCollectionType)
        return;
#endif // ENABLE(TIZEN_CONSTRUCT_TAG_NODE_LIST_WITH_SNAPSHOT)
...
#if ENABLE(TIZEN_CONSTRUCT_TAG_NODE_LIST_WITH_SNAPSHOT)
    if (isTagNodeList()) {
        const TagNodeList* tagNodeList = static_cast<const TagNodeList*>(this);
        if (tagNodeList->hasSnapshot())
            return tagNodeList->snapshot()->length();
    }
#endif // ENABLE(TIZEN_CONSTRUCT_TAG_NODE_LIST_WITH_SNAPSHOT)
    return lengthCommon();
}
...
Node* DynamicNodeList::item(unsigned offset) const
{
    ...
    if (isTagNodeList()) {
        const TagNodeList* tagNodeList = static_cast<const TagNodeList*>(this);
        if (tagNodeList->hasSnapshot())
            return tagNodeList->snapshot()->item(offset);
    }
#endif // ENABLE(TIZEN_CONSTRUCT_TAG_NODE_LIST_WITH_SNAPSHOT)
