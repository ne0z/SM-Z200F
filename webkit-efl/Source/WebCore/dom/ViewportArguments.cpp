...
ViewportAttributes computeViewportAttributes(ViewportArguments args, int desktopWidth, int deviceWidth, int deviceHeight, float devicePixelRatio, IntSize visibleViewport)
{
    ...
    case ViewportArguments::ValueAuto:
        if (args.type == ViewportArguments::ViewportMeta && !args.userScalable)
            args.width = deviceWidth;
        break;
#endif
    }
    ...
    if (args.maximumScale == ViewportArguments::ValueAuto) {
#if PLATFORM(TIZEN)
        // from Android S browser's policy
        result.maximumScale = float(4.0);
        result.minimumScale = min(float(4.0), result.minimumScale);
#else
