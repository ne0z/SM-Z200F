...
#if ENABLE(TIZEN_JSC_SEED_GC_ACTIVITY_CALLBACK)
    , m_lastGCLength(0.01)
#else
    , m_lastGCLength(0)
#endif
...
void Heap::markRoots(bool fullGC)
{
...

    {
        ...
        {
#if ENABLE(TIZEN_JSC_PARALLEL_GC_THREAD_CREATION_CHECK)
            if (UNLIKELY(m_sharedData.markingThreadsCount() == 0))
                m_sharedData.init();
#endif
            GCPHASE(Convergence);
...
void Heap::collect(SweepToggle sweepToggle)
{
    ...
    static const double timeThreshold = 1.0;
    static const unsigned short queueSize = 5;
    static const unsigned short maxKeeper = 4;
    static double timeStampQueue[queueSize] = {0,};
    static double setTime = 0;
    static unsigned short currentIndex = 0;
    static unsigned short numOfKeepers = 0;
    unsigned short previousIndex = 0;
    double curTime = WTF::currentTime();

    // Check if enough time has passed to set the state of JS dashmode to false.
    if (m_globalData->needFasterOperation() && (curTime - setTime > 3.0))
        m_globalData->setNeedFasterOperation(false);

    timeStampQueue[currentIndex] = curTime;
    previousIndex = (currentIndex + 1) % queueSize;
    if (timeStampQueue[currentIndex] - timeStampQueue[previousIndex] < timeThreshold) {
        m_globalData->setNeedFasterOperation(true);
        numOfKeepers = maxKeeper;
    }
    currentIndex = previousIndex;
    // Prolong the period of JS dashmode.
    if (numOfKeepers && (sweepToggle > DoNotSweep || curTime - setTime > 1.0)) {
#if ENABLE(TIZEN_BROWSER_DASH_MODE)
    ...
    static size_t curMaxHeapSize = 0;
    // Conditions to skip the current GC request.
    if ((currentIndex != (queueSize - 1)) && (sweepToggle == DoNotSweep) && (curMaxHeapSize < 230 * MB))
        return;

    // GC activity callback proceeds only when the heap is not too big.
    if (sweepToggle == DoSweepByCallback) {
        if (curMaxHeapSize > 64 * MB)
            return;
        else
            sweepToggle = DoSweep;
    }
    ...
    if (sweepToggle == DoSweep)
        m_globalData->codeProfileCache()->clear();
#endif
