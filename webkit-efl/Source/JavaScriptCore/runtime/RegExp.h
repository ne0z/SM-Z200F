...
namespace JSC {
...
#if ENABLE(TIZEN_JSC_REGEXP_TEST_CACHE)
    typedef HashMap<JSString*, MatchResult> RegExpTestCacheMap;
#endif
    ...
    public:
        ...
        inline KnownPattern knownPatternNum() { return m_knownPatternNum; }
#endif

#if ENABLE(TIZEN_JSC_REGEXP_TEST_CACHE)
        bool lookupTestCache(JSString* string, MatchResult& result)
        {
            RegExpTestCacheMap::iterator iter = m_testCacheMap.find(string);
            if (iter == m_testCacheMap.end())
                return false;
        ...
        void createTestCache(JSString* key, MatchResult val)
        {
            m_testCacheMap.add(key, val);
        }
...
#if ENABLE(TIZEN_FASTPATH_FOR_KNOWN_REGEXP)
        KnownPattern m_knownPatternNum;
#endif

#if ENABLE(TIZEN_JSC_REGEXP_TEST_CACHE)
        RegExpTestCacheMap m_testCacheMap;
#endif
    };
