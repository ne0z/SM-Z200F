...
{
#if !ENABLE(TIZEN_FASTPATH_FOR_KNOWN_REGEXP)
...
#if ENABLE(TIZEN_FASTPATH_FOR_KNOWN_REGEXP)
    if(global && regExp->knownPatternNum() == RegExp::WhitespaceReplacement) {
        unsigned  i = 0;
        // FIXME: If source is not a JSRopeString, the replacement can be done on here (not just a early return).
        if (source.is8Bit()) {
            const LChar* sourceCharacters = source.characters8();
            for (; i < sourceLen; i++, sourceCharacters++)
                if (*sourceCharacters == '\n' || *sourceCharacters == '\t' || *sourceCharacters == '\r')
                    break;
        } else {
            const UChar* sourceCharacters = source.characters16();
            for (; i < sourceLen; i++, sourceCharacters++)
                if (*sourceCharacters == '\n' || *sourceCharacters == '\t' || *sourceCharacters == '\r')
                    break;
        }
...
EncodedJSValue JSC_HOST_CALL stringProtoFuncReplace(ExecState* exec)
{
    ...
    if (exec->argument(1) == JSValue(JSValue::JSUndefined)) {
        StringFindCache* stringFindCache = (&exec->globalData())->stringFindCache();
        JSString* result;

        StringFindKey key((exec->argument(0)).toString(exec), string->value(exec));

        result = stringFindCache->lookup(key);

        if (result == 0) {
            result = asString(replaceUsingStringSearch(exec, string, searchValue));
            stringFindCache->create(key, result);
        }
        return JSValue::encode(result);
    }
    else {
        return JSValue::encode(replaceUsingStringSearch(exec, string, searchValue));
    }
#else
...
EncodedJSValue JSC_HOST_CALL stringProtoFuncFontcolor(ExecState* exec)
{
    ...
    UString s = thisValue.toString(exec)->value(exec);
    JSValue a0 = exec->argument(0);
    String color(a0.toString(exec)->value(exec).impl());
...
EncodedJSValue JSC_HOST_CALL stringProtoFuncFontsize(ExecState* exec)
{
    ...
    String fontSize(a0.toString(exec)->value(exec).impl());
...
EncodedJSValue JSC_HOST_CALL stringProtoFuncAnchor(ExecState* exec)
{
    ...
    UString s = thisValue.toString(exec)->value(exec);
    JSValue a0 = exec->argument(0);
    String anchor(a0.toString(exec)->value(exec).impl());
...
EncodedJSValue JSC_HOST_CALL stringProtoFuncLink(ExecState* exec)
{
    ...
    UString s = thisValue.toString(exec)->value(exec);
    JSValue a0 = exec->argument(0);
    String linkText(a0.toString(exec)->value(exec).impl());
