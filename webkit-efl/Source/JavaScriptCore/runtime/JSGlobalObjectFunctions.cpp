...
static JSValue encode(ExecState* exec,const Bitmap<256>& doNotEscape)
{
    ...
    for (size_t k = 0; k < cstr.length(); k++, p++) {
        char c = *p;
        if (doNotEscape.get(static_cast<char>(c)))
            builder.append(c);
        else {
            char tmp[4];
...
static JSValue decode(ExecState* exec, const CharType* characters, int length, const Bitmap<256>& doNotUnescape, bool strict)
{
    ...
    while (k < length) {
        ...
        if (c == '%') {
            ...
            if (charLen && (u >= 128 || !doNotUnescape.get(static_cast<char>(u)))) {
                if (u < 256)
...
EncodedJSValue JSC_HOST_CALL globalFuncEscape(ExecState* exec)
{
    ...
    if (str.is8Bit()) {
        ...
        for (unsigned k = 0; k < str.length(); k++, c++) {
            int u = c[0];
            if (doNotEscape.get(static_cast<char>(u)))
                builder.append(c, 1);
            else {
                char tmp[4];
    ...
    for (unsigned k = 0; k < str.length(); k++, c++) {
        ...
        if (u > 255) {
            ...
            builder.append(tmp);
        } else if (doNotEscape.get(static_cast<char>(u)))
            builder.append(c, 1);
        else {
            char tmp[4];
            snprintf(tmp, sizeof(tmp), "%%%02X", u);
