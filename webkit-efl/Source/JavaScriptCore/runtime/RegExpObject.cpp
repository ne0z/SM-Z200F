...
bool RegExpObject::test(ExecState* exec, JSString* string)
{
    if (regExp()->pattern().length() > 12) {
        return match(exec, string);
    }
    else {
        RegExp* currentRegExp = regExp();
        if(currentRegExp->global())
            return match(exec, string);

        MatchResult result;
        if (currentRegExp->lookupTestCache(string, result)) {
            if (result)
                exec->lexicalGlobalObject()->regExpConstructor()->recordCachedResult(exec->globalData(), currentRegExp, string, result);
            return result;
        }
        result = match(exec, string);
        currentRegExp->createTestCache(string, result);
        return result;
    }
}
#endif
