...
RegExp::RegExp(JSGlobalData& globalData, const UString& patternString, RegExpFlags flags)
    ...
    , m_knownPatternNum(UnKnown)
#endif
{
#if ENABLE(TIZEN_FASTPATH_FOR_KNOWN_REGEXP)
    if (patternString == "[\\n\\t\\r]")
        m_knownPatternNum = WhitespaceReplacement;
#endif
}
...
void RegExp::visitChildren(JSCell* cell, SlotVisitor& visitor)
{
    RegExp* thisObject = jsCast<RegExp*>(cell);
    Base::visitChildren(thisObject, visitor);

    thisObject->m_testCacheMap.clear();
}
#endif
