...
PlatformWebView::PlatformWebView(WKContextRef context, WKPageGroupRef pageGroup)
{
    ...
    m_view = toImpl(WKViewCreate(ecore_evas_get(m_window), context, pageGroup));
    ewk_view_mouse_events_enabled_set(m_view, true);

    evas_object_name_set(m_view, "m_view");
    evas_object_move(m_view, 0, 0);
...
{
    m_windowIsKey = isKey;
    evas_object_focus_set(m_view, isKey);
}

void PlatformWebView::setMainFrameIsFirstResponder(bool isFirstResponder)
{
}
