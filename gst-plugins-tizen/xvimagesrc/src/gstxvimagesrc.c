...
#ifdef COMMERCIAL_FEATURE
static int g_error_type = NO_VIDEO_ACTIVITY;
static gboolean g_video_available = TRUE;
#endif
...
gst_xv_image_src_class_init (GstXVImageSrcClass * klass)
{
  ...
  gst_xv_image_src_signals[SIGNAL_SELECTION_NOTIFY] =
      ...
      NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);
#ifdef COMMERCIAL_FEATURE
  gst_xv_image_src_signals[SIGNAL_VIDEO_ACTIVITY] =
      g_signal_new ("video-activity-change", G_TYPE_FROM_CLASS (klass),
      G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET (GstXVImageSrcClass, video_activity_change),
...
#if defined(COMMERCIAL_FEATURE) && defined(TIZEN_PROFILE_LITE)
  src ->phy_buf = NULL;
#endif
  drm_init (src);
...
gst_xv_image_src_start (GstBaseSrc * bsrc)
{
  ...
  src->ion_fd = open ("/dev/ion", open_flags);
  if (src->ion_fd >= 0) {
    GST_DEBUG ("[TBM_MEM] ion_fd open device success");
  } else {
    GST_ERROR ("[TBM_MEM] ion_fd open device failed");
  }
#endif
  if (src->format_id)
    xvimagesrc_thread_start (src);
...
#ifdef COMMERCIAL_FEATURE
static gboolean
signal_video_activity_change (void *asrc)
{
  GstXVImageSrc *src = (GstXVImageSrc *) asrc;

  g_signal_emit (src, gst_xv_image_src_signals[SIGNAL_VIDEO_ACTIVITY], 0, g_video_available);
...
#ifdef COMMERCIAL_FEATURE
  if (g_strrstr(error_msg, "BadRequest")) {
    g_error_type = NO_VIDEO_ACTIVITY;
  } else {
    g_error_type = OUT_OF_BUFFERS;
  }
#endif
  error_caught = TRUE;
  return 0;
...
#ifdef COMMERCIAL_FEATURE
  GST_ERROR ("HW Buf finalize gem_name: %d fd_name: %d bo: %p", buffer->YBuf, buffer->fd_name, buffer->bo[0]);
  if (buffer->fd_name) {
    close(buffer->fd_name);
  }
...
gst_xv_image_src_update_thread (void *asrc)
{
  ...
  while (!src->thread_return) {
    ...
    if (error_caught) {
#ifdef COMMERCIAL_FEATURE
      if(g_error_type == NO_VIDEO_ACTIVITY) {
        GST_ERROR("gst_xv_image_src_update_thread NO VIDEO ACTIVITY");
        if(g_video_available) {
          g_video_available = FALSE;
          g_timeout_add (1, signal_video_activity_change, src);
        }
      }
      else {
...
#ifdef COMMERCIAL_FEATURE
    if(!g_video_available) {
      g_video_available = TRUE;
      GST_ERROR("gst_xv_image_src_update_thread VIDEO ACTIVITY STARTED");
      g_timeout_add (1, signal_video_activity_change, src);
    }
#endif
    XSetErrorHandler (handler);
...
#endif
	  {
        ...
        if (data->BufType == XV_BUF_TYPE_LEGACY) {
          ...
          if (src->buf_share_method == BUF_SHARE_METHOD_FD) {
            ...
            tbm_bo_handle bo_handle_vptr;
#ifdef TIZEN_PROFILE_LITE
            struct ion_phys_data phys_data;
            struct ion_custom_data custom_data;
#endif
            int phy_size = 0;
            GST_DEBUG ("BUF_SHARE_METHOD_TIZEN_BUFFER");
#endif
            mm_video_buf->handle.bo[0] = drm_convert_gem_to_bo (src->bufmgr, data->YBuf);
#ifdef COMMERCIAL_FEATURE
            bo_handle_fd = tbm_bo_get_handle (mm_video_buf->handle.bo[0], TBM_DEVICE_MM);
            if(!bo_handle_fd.u32)
            {
              GST_ERROR ("[Error] : cannot get Ion FD");
              continue;
            }
            mm_video_buf->handle.dmabuf_fd[0] = bo_handle_fd.u32;
            src->framesize = tbm_bo_size(mm_video_buf->handle.bo[0]);
            bo_handle_vptr = tbm_bo_get_handle (mm_video_buf->handle.bo[0], TBM_DEVICE_CPU);
            if(!bo_handle_vptr.ptr)
            {
              GST_ERROR ("[Error] : cannot get Ion FD");
              continue;
            }
            mm_video_buf->data[0] = bo_handle_vptr.ptr;
#ifdef TIZEN_PROFILE_LITE
            phys_data.fd_buffer = bo_handle_fd.u32;
            custom_data.cmd = 0;
            custom_data.arg = (unsigned long)&phys_data;
            if (src->ion_fd > 0){
              if(ioctl(src->ion_fd, ION_IOC_CUSTOM, &custom_data) < 0) {
                GST_ERROR ("[TBM_MEM] ION_IOC_CUSTOM fails %d %s",errno,strerror(errno));
                mm_video_buf->handle.paddr[0] = src->phy_buf;
              } else {
                mm_video_buf->handle.paddr[0] = phys_data.phys;
                src->phy_buf = phys_data.phys;
                GST_ERROR("bo = %p, fd = %d, va = %p, pa = %p",
                    mm_video_buf->handle.bo[0], mm_video_buf->handle.dmabuf_fd[0], mm_video_buf->data[0], mm_video_buf->handle.paddr[0]);
              }
            } else
#endif
            {
              mm_video_buf->handle.paddr[0] = NULL;
            }
#endif
