...
gssize
soup_cache_input_stream_push_ongoing (GInputStream  *stream,
					void          *buffer,
					gssize nread)
{
	SoupCacheInputStream *istream = SOUP_CACHE_INPUT_STREAM (stream);
...
void
soup_cache_input_stream_push_cancel (GInputStream *stream)
{
	GError *err = NULL;
	soup_cache_input_stream_close_fn (stream, NULL, &err);
}
