...
#if ENABLE(TIZEN_CERTIFICATE_FILE_SET)
			if (!soup_message_is_from_session_restore (item->msg))
...
void soup_session_set_certificate_file(SoupSession *session, SoupURI *uri)
{
	...
	if (soup_uri_is_https (uri, priv->https_aliases) && (!priv->tlsdb)) {
		...
		if(error){
			TIZEN_LOGE ("Error!! error->code: [%d], error->domain : [%d] , error->message :[%s]", error->code, error->domain, error->message);
		}
...
#if ENABLE(TIZEN_VALIDATE_CACHE_OBJECT_ON_SOUP_GET_FEATURE)
	if (cache) {
