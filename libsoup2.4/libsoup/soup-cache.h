...
typedef enum {
	SOUP_CACHE_RESPONSE_UC_FRESH = SOUP_CACHE_RESPONSE_FRESH,
	SOUP_CACHE_RESPONSE_UC_NEEDS_VALIDATION = SOUP_CACHE_RESPONSE_NEEDS_VALIDATION,
	SOUP_CACHE_RESPONSE_UC_STALE = SOUP_CACHE_RESPONSE_STALE,
	SOUP_CACHE_RESPONSE_UC_NONE = 3
} SoupCacheResponseUC;
...
//START ENABLE_TIZEN_COMPRESSION_PROXY
	...
	/* methods */
	SoupCacheResponseUC (*has_response) (SoupCache   *cache,
							SoupMessage* msg);

	gpointer (*push_start)(SoupCache   *cache, SoupMessage *msg);
	void (*push_ongoing)(gpointer handle, const char* data, int length);
	void (*push_cancel)(gpointer handle);

	/* signals */
	void     (*removed_cache_resource) (const char *url);
	void     (*cache_cleared) (void);
//END ENABLE_TIZEN_COMPRESSION_PROXY
} SoupCacheClass;
