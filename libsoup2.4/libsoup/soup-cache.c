...
soup_cache_entry_new (SoupCache *cache, SoupMessage *msg, time_t request_time, time_t response_time)
{
	...
	date = soup_message_headers_get_one (entry->headers, "Date");
#if ENABLE (TIZEN_SET_DATE_AS_RESPONSE_TIME)
	if (!date) {
		SoupDate *tmpDate = soup_date_new_from_time_t (entry->response_time);
		date = soup_date_to_string (tmpDate, SOUP_DATE_HTTP);
		soup_date_free (tmpDate);
	}
#endif
