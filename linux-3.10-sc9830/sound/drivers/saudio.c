	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] !=  saudio->kcon_apptype){
		...
		result = saudio_send_common_cmd_ex(saudio->dst, saudio->channel,
			 SAUDIO_CMD_SET_APPTYPE, SAUDIO_CMD_SET_APPTYPE, app_type, 0, 0);
		if (result) {
			ETRACE("snd_pcm_APPType_control_put: RESUME, send_common_cmd result is %d!\n",result);
			if(result != (-ERESTARTSYS))
			saudio_snd_card_free(saudio);
			return result;
		}
