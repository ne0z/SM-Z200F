...
struct _GstSingleQueue
{
  ...
  gboolean is_audio_q, is_video_q;
#endif
...
#ifdef GST_MQ_MODIFICATION
/* For HLS Case Only, default limits, we try to keep up to 2 seconds of data and if there is not
...
#define DEFAULT_VIDEO_MAX_SIZE_BYTES 10 * 1024 * 1024 /* 10 MB */
#define DEFAULT_VIDEO_MAX_SIZE_BUFFERS 5
#define DEFAULT_VIDEO_MAX_SIZE_TIME 2 * GST_SECOND

/* For HLS Case Only, default limits, we try to keep up to 0 seconds of data and if there is not
 * time, up to 1 MB. The number of buffers is dynamically scaled to make sure
 * there is data in the queues. Normally, the byte and time limits are not hit
 * in theses conditions. */
#define DEFAULT_AUDIO_MAX_SIZE_BYTES 1* 1024 * 1024 /* 1 MB */
#define DEFAULT_AUDIO_MAX_SIZE_BUFFERS 5
#define DEFAULT_AUDIO_MAX_SIZE_TIME 0 * GST_SECOND
#endif
...
enum
{
  ...
  PROP_AUDIO_ONLY_BUFFERING_PERCENT,
  PROP_IS_AUDIO_ONLY_HLS_MQ,
#endif
  PROP_USE_BUFFERING,
  PROP_LOW_PERCENT,
  PROP_HIGH_PERCENT,
...
gst_multi_queue_class_init (GstMultiQueueClass * klass)
{
...
#ifdef GST_MQ_MODIFICATION
  /**
   * GstMultiQueue::lowbuffering:
   * @multiqueue: the multiqueue instance
   *
   * Reports that the audio-queue for HLS is almost empty ( less than 10% data remaining )
   * Before using this signal , is-audio-only-hls-mq property must be set in the MQ.
   *
   * This can be used only for Audio-Only HLS streams
   */
  gst_multi_queue_signals[SIGNAL_LOWBUFFERING] =
      g_signal_new ("lowbuffering", G_TYPE_FROM_CLASS (klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET (GstMultiQueueClass, lowbuffering), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
#endif
  /* PROPERTIES */
  ...
  g_object_class_install_property (gobject_class, PROP_AUDIO_ONLY_BUFFERING_PERCENT,
      g_param_spec_int ("audio-only-buffering-percent", "Audio queue Buffering Percent",
          "Audio queue Buffering Percent Value",
          0, G_MAXINT, 0,
          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
   g_object_class_install_property (gobject_class, PROP_IS_AUDIO_ONLY_HLS_MQ,
      g_param_spec_boolean ("is-audio-only-hls-mq", "Is Audio Only HLS for MQ ",
          "To know if Multiqueue is is for a Audio-Only HLS Content",
...
#ifdef GST_MQ_MODIFICATION
  mqueue->max_video_size.bytes = DEFAULT_VIDEO_MAX_SIZE_BYTES;
  mqueue->max_video_size.visible = DEFAULT_VIDEO_MAX_SIZE_BUFFERS;
  mqueue->max_video_size.time = DEFAULT_VIDEO_MAX_SIZE_TIME;

  mqueue->max_audio_size.bytes = DEFAULT_AUDIO_MAX_SIZE_BYTES;
  mqueue->max_audio_size.visible = DEFAULT_AUDIO_MAX_SIZE_BUFFERS;
  mqueue->max_audio_size.time = DEFAULT_AUDIO_MAX_SIZE_TIME;

  mqueue->is_audio_only_hls = FALSE;
#endif
...
#define SET_CHILD_PROPERTY(mq,format) G_STMT_START {	        \
    ...
    while (tmp) {						\
      ...
      if(q->is_audio_q == TRUE || q->is_video_q == TRUE){		\
	  	if(q->is_video_q == TRUE || mq->is_audio_only_hls == TRUE){		\
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
...
#ifdef GST_MQ_MODIFICATION
      mq->max_video_size.bytes = g_value_get_uint (value);
#endif
    ...
    case PROP_EXTRA_SIZE_TIME:
      ...
      break;
#ifdef GST_MQ_MODIFICATION
    case PROP_IS_AUDIO_ONLY_HLS_MQ:
      mq->is_audio_only_hls = g_value_get_boolean (value);
      GST_MULTI_QUEUE_MUTEX_LOCK(mq);
      mq->max_size.bytes = 2*DEFAULT_AUDIO_MAX_SIZE_BYTES;
      mq->max_audio_size.bytes = 2*DEFAULT_AUDIO_MAX_SIZE_BYTES;
      SET_CHILD_PROPERTY (mq, bytes);
      GST_MULTI_QUEUE_MUTEX_UNLOCK (mq);
      gst_multi_queue_post_buffering (mq);
      break;
...
static gint
get_audio_only_current_percent (GstMultiQueue * mq)
{
  GList *tmp;
  gdouble buff_per = 0.0;
  gint iBufferingPercent;
  ...
  for (tmp = mq->queues; tmp; tmp = g_list_next (tmp)) {
	...
	{
		buff_per = (size.bytes * 100 * 1.0);
		buff_per = buff_per / sq->max_size.bytes;
	}
	else
		buff_per = -1;
    }
    ...
    {
	if (sq->max_size.visible > 0)
	{
		buff_per = (size.visible * 100 * 1.0);
		buff_per = buff_per / sq->max_size.visible;
	}
	else
		buff_per = -1;
    }

    iBufferingPercent =(guint)buff_per;

    if (iBufferingPercent > 100)
	iBufferingPercent = 100;

    GST_DEBUG_OBJECT (mq,
        "queue %d: bytes %u/%u, visible %u/%u, time %" G_GUINT64_FORMAT "/%"
        G_GUINT64_FORMAT",iBufferingPercent = %d", sq->id, size.bytes, sq->max_size.bytes,size.visible, sq->max_size.visible,
        sq->cur_time, sq->max_size.time,iBufferingPercent);

  }

  return iBufferingPercent;
}
#endif
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_CURR_SIZE_BYTES:
      ...
      break;
    case PROP_AUDIO_ONLY_BUFFERING_PERCENT:
      iBufferingPercent = get_audio_only_current_percent(mq);
      g_value_set_int (value, iBufferingPercent);
      GST_DEBUG_OBJECT (mq, "PROP_CURR_BUFFERING_PERCENT : %d", iBufferingPercent);
      break;
#endif
    case PROP_USE_BUFFERING:
...
update_buffering (GstMultiQueue * mq, GstSingleQueue * sq)
{
...
#ifdef GST_MQ_MODIFICATION
  GList *iter;
  gint q1_perc = -1, q2_perc = -1;
  gint q1_bytes = -1, q2_bytes = -1;
  GstDataQueueSize size;
#endif
...
#ifdef GST_MQ_MODIFICATION
  GST_DEBUG_OBJECT (mq, "mq->is_audio_only_hls = %d,mq->buffering= %d,mq->percent=%d%%,low_percent=%d%%,high_percent=%d%% !!!",mq->is_audio_only_hls,mq->buffering,mq->percent,mq->low_percent,mq->high_percent);
  if (mq->is_audio_only_hls)
  {
	gint iBufferingPercent = get_audio_only_current_percent(mq);
	GST_DEBUG_OBJECT (mq, "iBufferingPercent = %d",iBufferingPercent);
	if (iBufferingPercent < 10)
	{
	    GST_DEBUG_OBJECT (mq, "Audio-Only-HLS Queue has reached Buffering LowMark !! Triggering Signal");
           g_signal_emit (mq, gst_multi_queue_signals[SIGNAL_LOWBUFFERING], 0);
	}
  }
    ...
    for (iter = mq->queues; iter; iter = g_list_next (iter)) {
      ...
      if (oq->is_eos)
	  	GST_WARNING_OBJECT (mq, "EOS is already reached !! not putting Queue Logic !!");
      else
      {
		  ...
		  GST_WARNING_OBJECT (mq, "Queue3 is coming.. need to recheck logic !");
	      gint per = get_percentage(oq);
	      if(q1_perc == -1)
	        q1_perc = per;
	      else if(q2_perc == -1)
		  q2_perc = per;
  ...
  if (mq->buffering) {
...
#ifdef GST_MQ_MODIFICATION
    if(q1_bytes == 0 || q2_bytes == 0){
	percent = 0;
	mq->buffering = TRUE;
    }
    else if(q1_perc > 0 || q2_perc > 0)
    ...
    if (is_buffering && percent < mq->low_percent) {
      GST_DEBUG_OBJECT (mq, "Start Buffering !");
      mq->buffering = TRUE;
      SET_PERCENT (mq, percent);
    }
#ifdef GST_MQ_MODIFICATION
    else if (q1_bytes == 0 || q2_bytes == 0){
      GST_DEBUG_OBJECT (mq, "Start Buffering, queue is Empty !");
      mq->buffering = TRUE;
      SET_PERCENT (mq, 0);
    }
#endif
  ...
  switch (type) {
    ...
    case GST_EVENT_FLUSH_STOP:
      ...
      case GST_EVENT_CAPS:{
        ...
        if(caps)
          GST_WARNING_OBJECT (mq, "Got caps: %" GST_PTR_FORMAT, caps);
        else
          GST_WARNING_OBJECT (mq, "Got caps: NULL");
        gchar *caps_str = gst_caps_to_string(caps);
        if (strstr(caps_str, "audio") && strstr(caps_str, "hls_streaming=(boolean)true"))
        {
          sq->is_audio_q = TRUE;
          GST_MULTI_QUEUE_MUTEX_LOCK (mq);
          sq->max_size.bytes= mq->max_audio_size.bytes;
          sq->max_size.time = mq->max_audio_size.time;
          update_buffering (mq, sq);
          gst_data_queue_limits_changed (sq->queue);
          GST_MULTI_QUEUE_MUTEX_UNLOCK (mq);
          gst_multi_queue_post_buffering (mq);
        }
        else if(strstr(caps_str, "video") && strstr(caps_str, "hls_streaming=(boolean)true"))
        {
          sq->is_video_q = TRUE;
        }
        break;
...
#ifdef GST_MQ_MODIFICATION
  if(sq->is_audio_q == TRUE){
  	sq->max_size.bytes = mqueue->max_audio_size.bytes;
	sq->max_size.time = mqueue->max_audio_size.time;
  }
  else
#endif
  {
