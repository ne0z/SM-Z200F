...
#ifdef GST_EXT_BASEPARSER_MODIFICATION
  GstPadMode expected_pad_mode; /* to get pad mode early */
    ...
    GstBuffer * buf)
{
  ...
  /* parse->priv->discont set to FALSE before invoked this function, so we should use GST_BUFFER_IS_DISCONT(buf)*/
#ifdef GST_EXT_BASEPARSER_MODIFICATION
...
#endif /* GST_BASEPARSE_MODIFICATION */
...
/*Modification : Move to handle_buffer to get upstream size earlier for ADIF duration*/
...
gst_base_parse_src_query_default (GstBaseParse * parse, GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    ...
    case GST_QUERY_DURATION:
    {
      ...
      GST_DEBUG_OBJECT (parse, "duration query :%"GST_PTR_FORMAT, query);
...
void
gst_base_parse_get_pad_mode (GstBaseParse * parse, GstPadMode * pad_mode)
{
  GST_BASE_PARSE_INDEX_LOCK (parse);
