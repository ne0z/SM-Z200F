...
gst_base_sink_perform_qos (GstBaseSink * sink, gboolean dropped)
{
  ...
  if (priv->avg_rate >= 0.0) {
    ...
    gst_base_sink_send_qos (sink, type, priv->avg_rate, priv->current_rstart, diff);
  }
...
#ifdef GST_EXT_BASESINK_MODIFICATION
      /* when we get here we could be flushing again when the event handler calls
       * _wait(). We have to ignore this object in that case. */
      if (basesink->flushing) {
        GST_DEBUG_OBJECT (basesink, "we are flushing, drop the eos event");
...
#ifndef GST_EXT_BASESINK_MODIFICATION
/* we don't handle qos message now */
