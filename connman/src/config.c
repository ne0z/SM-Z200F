...
#define SERVICE_KEY_IPV4_METHOD			"IPv4Method"
#define SERVICE_KEY_IPv4              	"IPv4"
#define SERVICE_KEY_SUBNET_MASK     	"SubnetMask"
#define SERVICE_KEY_GATEWAY_ADDRESS   	"GatewayAddress"
#define SERVICE_KEY_DNS1 	   			"DNS1"
#define SERVICE_KEY_DNS2 	   			"DNS2"
#define SERVICE_KEY_PROXY_ADDRESS      "ProxyAddress"
...
#if defined TIZEN_EXT
	SERVICE_KEY_HIDDEN,
	SERVICE_KEY_IPV4_METHOD,
	SERVICE_KEY_IPv4,
	SERVICE_KEY_SUBNET_MASK,
	SERVICE_KEY_GATEWAY_ADDRESS,
	SERVICE_KEY_DNS1,
	SERVICE_KEY_DNS2,
	SERVICE_KEY_PROXY_ADDRESS,
...
free_only:
	...
	g_free(config_service->dns1);
	g_free(config_service->dns2);
	g_free(config_service->proxy_address);
#endif
...
static int check_family(const char *address, int expected_family)
{
	...
	if (family < 0) {
		DBG("Fail to get family of address %s (%d/%s)", address,
...
static enum connman_ipconfig_method parse_ip_method(char *ip_method)
{
	if (g_ascii_strcasecmp(ip_method, "static") == 0)
		return CONNMAN_IPCONFIG_METHOD_MANUAL;
	else if (g_ascii_strcasecmp(ip_method, "dhcp") == 0)
			...
			struct connman_config_service *service)
{
	...
	str = g_key_file_get_string(keyfile, group, SERVICE_KEY_IPV4_METHOD, NULL);
	if (str)
		service->ipv4_method = parse_ip_method(str);
	else
		service->ipv4_method = CONNMAN_IPCONFIG_METHOD_DHCP;
	if (service->ipv4_method == CONNMAN_IPCONFIG_METHOD_MANUAL) {
		str = g_key_file_get_string(keyfile, group, SERVICE_KEY_IPv4, NULL);
		if (str) {
			if (check_family(str, AF_INET) < 0)
				goto err;

			g_free(service->ipv4_address);
			service->ipv4_address = str;
		}

		str = g_key_file_get_string(keyfile, group, SERVICE_KEY_SUBNET_MASK, NULL);
		if (str) {
			if ((check_family(str, AF_INET)) < 0)
				goto err;

			g_free(service->ipv4_netmask);
			service->ipv4_netmask = str;
		}

		str = g_key_file_get_string(keyfile, group, SERVICE_KEY_GATEWAY_ADDRESS, NULL);
		if (str) {
			if ((check_family(str, AF_INET)) < 0)
				goto err;

			g_free(service->ipv4_gateway);
			service->ipv4_gateway = str;
		}

		str = g_key_file_get_string(keyfile, group, SERVICE_KEY_DNS1, NULL);
		if (str) {
			if (check_family(str, AF_INET) < 0)
				goto err;
		...
		str = g_key_file_get_string(keyfile, group, SERVICE_KEY_DNS2, NULL);
		if (str) {
			if (check_family(str, AF_INET) < 0)
				goto err;
		...
		str = g_key_file_get_string(keyfile, group, SERVICE_KEY_PROXY_ADDRESS, NULL);
		if (str) {
			g_free(service->proxy_address);
			service->proxy_address = str;
		}
	}
						...
						struct connman_config *config)
{
	...
	if (!load_service_generic(keyfile, group, config, service)) {
		DBG("Fail to fetch IP Configurations from config file");
		err = -EINVAL;
		goto err;
	}
#endif
...
static void provision_service(gpointer key, gpointer value, gpointer user_data)
{
	...
	if (config->ipv4_method == CONNMAN_IPCONFIG_METHOD_MANUAL) {
		struct connman_ipaddress *address = NULL;
		...
		if (connman_network_set_ipaddress(network, address) < 0)
			DBG("Unable to set IPv4 address to network %p",network);
		...
		if (config->dns1)
			__connman_service_nameserver_append(service, config->dns1, FALSE);

		if (config->dns2)
			__connman_service_nameserver_append(service, config->dns2, FALSE);

		if (config->proxy_address)
			connman_service_set_proxy(service, config->proxy_address, FALSE);
