...
#if defined TIZEN_EXT
		GSList *item = NULL;
		item = g_slist_find(server_list_sec, server);
		if(item){
			DBG("server sec item is found");
...
#if !defined TIZEN_EXT
	/* For dns packet, do not bind to interface,
	 * it's going through with routing rule */
					...
					int protocol)
{
	...
	if (server_create_socket(data) != 0) {
#if defined TIZEN_EXT
		struct server_data *server = data;
		connman_resolver_remove(server->index, NULL, server->server);
#endif
		destroy_server(data);
		return NULL;
	}
...
#if defined TIZEN_EXT
				/*
				 * always enabled for global ifindex.
				 */
				data->index == GLOBAL_IFINDEX ||
#endif
				__connman_service_index_is_split_routing(
								data->index)) {
			data->enabled = TRUE;
#if defined TIZEN_EXT
			DBG("Adding DNS server %s, index %d",
					data->server, data->index);
#else
			DBG("Adding DNS server %s", data->server);
#endif
		}
...
#if defined TIZEN_EXT
		/*
		 * Do not disable for global ifindex.
		 */
		else if (data->index == GLOBAL_IFINDEX) {
			DBG("global DNS server %s", data->server);
			data->enabled = TRUE;
		}
#endif
		else {
