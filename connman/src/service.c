...
static const char *error2string(enum connman_service_error error)
{
	...
	case CONNMAN_SERVICE_ERROR_CAPTIVEPORTAL_SIGN_FAILED:
		return "captiveportal-sign-failed";
#endif
	}
...
static enum connman_service_error string2error(const char *error)
{
	if (g_strcmp0(error, "dhcp-failed") == 0)
		...
		return CONNMAN_SERVICE_ERROR_AUTH_FAILED;
	else if (g_strcmp0(error, "internet-unreachable") == 0)
		return CONNMAN_SERVICE_ERROR_INTERNET_FAILED;
	else if (g_strcmp0(error, "captiveportal-sign-failed") == 0)
		return CONNMAN_SERVICE_ERROR_CAPTIVEPORTAL_SIGN_FAILED;
#endif
...
#if defined TIZEN_EXT_IPTIME
#define IPTIME "iptime"
#define IPTIME_5G "iptime5G"

static int service_iptime_autoconnect_load(struct connman_service *service)
{
	...
	DBG("service %p name(%s)", service, ssid);

	if(g_strcmp0(IPTIME, ssid) != 0 && g_strcmp0(IPTIME_5G, ssid) != 0){
		DBG("ssid is not matched with blocked AP!");
		g_free(ssid);
		return 0;
	}
...
static int service_save(struct connman_service *service)
{
	...
	case CONNMAN_SERVICE_TYPE_CELLULAR:
		...
		if (service->state_ipv4 == CONNMAN_SERVICE_STATE_FAILURE ||
		...
		else if(service->state_ipv4 == CONNMAN_SERVICE_STATE_READY ||
			...
			/*
			 * Tizen does not remove failure field to indicate
	...
	if (service->nameservers_config != NULL) {
		...
		while (i != 0) {
			...
			if (service->type == CONNMAN_SERVICE_TYPE_CELLULAR) {
				if (TRUE == __connman_service_is_internet_profile(service)) {
...
#if defined TIZEN_EXT
connman_bool_t connman_service_is_ims_profile(
		struct connman_service *cellular)
{
	...
	if (g_str_has_suffix(cellular->path, ims_suffix) == TRUE)
		return TRUE;
...
void connman_update_nameservers(index)
{
	struct connman_service *service;
	...
	/* check if service is not internet */
	if (service->type == CONNMAN_SERVICE_TYPE_CELLULAR &&
			!__connman_service_is_internet_profile(service)) {
		DBG("service is not internet profile");
		return;
	}
...
#if defined TIZEN_EXT
static void ipv6_state_changed(struct connman_service *service)
{
	const char *str;
	...
	connman_dbus_property_changed_basic(service->path,
				CONNMAN_SERVICE_INTERFACE, "StateIPv6",
						DBUS_TYPE_STRING, &str);
}
#endif
...
void connman_service_set_is_captive(struct connman_service *service,
		connman_bool_t is_captive_portal)
{
	if (service == NULL)
		return;

	service->is_captive = is_captive_portal;
					...
					DBusMessage *msg, void *user_data)
{
	...
	if (g_str_equal(name, "AutoConnect") == TRUE) {
		...
		if(service->error == CONNMAN_SERVICE_ERROR_CAPTIVEPORTAL_SIGN_FAILED)
...
static connman_bool_t is_ignore(struct connman_service *service)
{
	...
	if (service->error == CONNMAN_SERVICE_ERROR_AUTH_FAILED ||
			service->error == CONNMAN_SERVICE_ERROR_INTERNET_FAILED)
		return TRUE;
#endif
...
#if defined TIZEN_EXT
static void service_added_signal(struct connman_service *service)
{
	DBG("service %p", service);
	...
	g_dbus_emit_signal(connection, CONNMAN_MANAGER_PATH,
			CONNMAN_MANAGER_INTERFACE, "ServiceAdded",
			DBUS_TYPE_OBJECT_PATH, &service->path,
			DBUS_TYPE_INVALID);
}

static void service_removed_signal(struct connman_service *service)
{
	DBG("service %p %s", service, service->path);
	...
	g_dbus_emit_signal(connection, CONNMAN_MANAGER_PATH,
			CONNMAN_MANAGER_INTERFACE, "ServiceRemoved",
			DBUS_TYPE_OBJECT_PATH, &service->path,
			DBUS_TYPE_INVALID);
}
#endif
...
#if defined TIZEN_EXT
	if (service->type == CONNMAN_SERVICE_TYPE_CELLULAR)
		service_removed_signal(service);
#endif
...
#if defined TIZEN_SUPPORT_WFA_CERTIFICATION
static void request_input_wps_none_security_cb (struct connman_service *service,
			connman_bool_t values_received,
			const char *name, int name_len,
			const char *identity, const char *passphrase,
			gboolean wps, const char *wpspin,
	...
	while (g_sequence_iter_is_end(iter) == FALSE) {
		...
		if (default_internet == TRUE) {
			...
			if (is_connected(default_service) == TRUE ||
				...
				err = connman_network_set_alwayon_control(default_service->network, TRUE);
				DBG("set alwayson control default service %p %s [%d]",
			...
			/* set idle to clear previous failure */
			default_service->state = CONNMAN_SERVICE_STATE_IDLE;
			default_service->state_ipv4 = CONNMAN_SERVICE_STATE_IDLE;
			default_service->state_ipv6 = CONNMAN_SERVICE_STATE_IDLE;
...
#if defined TIZEN_EXT
		/* Skip if profile is not internet profile */
		if (FALSE == __connman_service_is_internet_profile(service)) {
			continue;
		}
#endif
...
#if defined TIZEN_EXT
		if(service->type == CONNMAN_SERVICE_TYPE_CELLULAR)
			connman_network_set_alwayon_control(service->network, FALSE);
...
#if defined TIZEN_EXT
		def_service = connman_service_get_default_connection();
		service->disconnect_reason = connman_network_get_disconnect_reason(service->network);

		//sprd - AP Deauth (captive portal timeout) - 196
		DBG("service disconnect reason (%d)", service->disconnect_reason);
		if(service->is_captive && service->disconnect_reason == 196){
			service->autoconnect = FALSE;
		}
#else
		def_service = __connman_service_get_default();
...
#if defined TIZEN_EXT
#define IP_EXEC_PATH "/usr/sbin/ip"

int connman_execute_file(const char *file_path,
...
int connman_del_ims_gateway(int index)
{
	char *if_name = NULL;
	int rv = 0;
	char *args[9] = {0, };
	...
	/* check if service is IMS */
	if (!connman_service_is_ims_profile(service))
		return 0;

	if_name = connman_service_get_interface(service);
	if (NULL == if_name)
		return -1;

	DBG("IMS interface name = [%s]", if_name);

	args[0] = (char *)IP_EXEC_PATH;
	args[1] = "-6";
	args[2] = "route";
	args[3] = "del";
	args[4] = "::/0";
	...
	rv = connman_execute_file(IP_EXEC_PATH, args, envs);
	if (!rv) {
		DBG("Failed to execute ip command");
		g_free(if_name);
		return -1;
	}

	DBG("Successfully deleted IMS default route entry");
	g_free(if_name);
	return 0;
}
#endif
...
#if defined TIZEN_EXT
	/*
	 * Sometimes RTM_NEWADDR event triggers before __changed_context signal
	 * leaving IPv6 state never updated to READY in AUTO mode.
	 * Add IPv6 new_address as soon as IPv4 State becomes READY.
	 * */
	if (type == CONNMAN_IPCONFIG_TYPE_IPV4 &&
			new_state == CONNMAN_SERVICE_STATE_READY) {
		enum connman_ipconfig_method ipv6_method;
		struct connman_new_address *new_address;
		int index;
		...
		if (ipv6_method == CONNMAN_IPCONFIG_METHOD_AUTO &&
			...
			index = __connman_ipconfig_get_index(service->ipconfig_ipv6);
			new_address = __connman_ipconfig_get_new_address_from_index(index);
			if (new_address && new_address->family == AF_INET6) {
				DBG("RTM_NEWADDR event was already received, add new_address");
				__connman_ipconfig_newaddr(new_address->index, new_address->family,
						NULL, new_address->prefixlen, new_address->address);
				/* Delete default IMS gateway */
				connman_del_ims_gateway(index);
			}
		}
	}
		...
		case CONNMAN_SERVICE_SECURITY_NONE:
			...
			if (service->userconnect == TRUE &&
				...
				err = __connman_agent_request_passphrase_input(service,
						request_input_wps_none_security_cb, NULL);
				if (err == -EINPROGRESS)
					return err;
			}
#endif
...
#if defined TIZEN_EXT
	if (service && service->type == CONNMAN_SERVICE_TYPE_WIFI &&
			!service->favorite) {
		/**
		 * Reset ipconfig method from static to default method DHCP
		 * after clearing ipconfig->address.
		 */
		DBG("reset ip_config method of service %p to DHCP", service);
		__connman_ipconfig_reset_method(service->ipconfig_ipv4,
				CONNMAN_IPCONFIG_METHOD_DHCP);
		__connman_ipconfig_reset_method(service->ipconfig_ipv6,
				CONNMAN_IPCONFIG_METHOD_DHCP);
	}
#endif
...
#if defined TIZEN_EXT_IPTIME
	if (service->type == CONNMAN_SERVICE_TYPE_WIFI)
		service_iptime_autoconnect_load(service);
#endif
