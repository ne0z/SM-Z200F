...
static void set_connected(struct connman_network *network)
{
	...
	case CONNMAN_IPCONFIG_METHOD_AUTO:
#if defined TIZEN_EXT
		if (ipv6_method > CONNMAN_IPCONFIG_METHOD_OFF) {
			/* needs to clear associating state
			 * if ipv6 already set */
			break;
		} else
			return;
...
int connman_network_set_alwayon_control(struct connman_network *network, connman_bool_t value)
{
	int err;
	...
	err = network->driver->control(network, value);
