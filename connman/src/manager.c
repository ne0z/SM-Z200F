					...
					DBusMessage *msg, void *data)
{
	...
	if (g_str_equal(name, "OfflineMode") == TRUE) {
	...
	else if (g_str_equal(name, "Nameservers.Add") == TRUE) {
		/*
		 * To add nameserver globally, working for all interface.
		 */
		...
		while (i != 0) {
			i--;
			connman_resolver_append(GLOBAL_IFINDEX, NULL, nameservers[i]);
		}

		g_strfreev(nameservers);

	} else if (g_str_equal(name, "Nameservers.RemoveAll") == TRUE) {
		/*
		 * To remove all global nameservers.
		 */
		DBG("Nameservers.RemoveAll");
		connman_resolver_remove_all(GLOBAL_IFINDEX);
	}
#endif
	else
...
static const GDBusSignalTable manager_signals[] = {
	...
	{ GDBUS_SIGNAL("ServicesChanged",
			GDBUS_ARGS({ "changed", "a(oa{sv})" },
					{ "removed", "ao" })) },
#if defined TIZEN_EXT
	{ GDBUS_SIGNAL("ServiceAdded",
			GDBUS_ARGS({ "path", "o" })) },
	{ GDBUS_SIGNAL("ServiceRemoved",
			GDBUS_ARGS({ "path", "o" })) },
#endif
	{ },
};
