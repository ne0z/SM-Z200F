...
static GHashTable *new_address_hash = NULL;
#endif
...
#if defined TIZEN_EXT
static void free_new_address(gpointer data)
{
	struct connman_new_address *new_address = data;

	connman_info("index %d {remove} address %s", new_address->index,
			new_address->address);

	g_free(new_address->address);
	g_free(new_address);
}
#endif
...
#if defined TIZEN_EXT
	//dbg not support the statistics in Tizen
	return;
#endif
				...
				unsigned char prefixlen, const char *address)
{
	...
	if (family == AF_INET6) {
		/*
		 * Sometimes RTM_NEWADDR event triggers before service ready
		 * leaving IPv6 state never updated to READY in AUTO mode.
		 * So add new address to new address list, so that it will be
		 * added again after service ready to configure IPv6.
		 * */
		new_address = g_hash_table_lookup(new_address_hash, GINT_TO_POINTER(index));
		if (!new_address) {
			new_address = g_try_new0(struct connman_new_address, 1);
			if (new_address) {
				new_address->index = index;
				new_address->family = family;
				new_address->prefixlen = prefixlen;
				new_address->address = g_strdup(address);

				g_hash_table_insert(new_address_hash, GINT_TO_POINTER(index), new_address);

				connman_info("index %d {add} ipv6 address %s/%u family %d to new_address list",
						index, address, prefixlen, family);
			}
		}
	}
#endif
...
#if defined TIZEN_EXT
	/*
	 * Do not prepend ipaddress to ipdevice->address_list if RTM_NEWADDR event
	 * received before service got created otherwise it will block to
	 * add this new address next time when service created.
	 * */
				...
				unsigned char prefixlen, const char *address)
{
	...
	if (family == AF_INET6) {
		new_address = g_hash_table_lookup(new_address_hash, GINT_TO_POINTER(index));
		if (new_address)
			g_hash_table_remove(new_address_hash, GINT_TO_POINTER(index));
	}
#endif
	ipdevice = g_hash_table_lookup(ipdevice_hash, GINT_TO_POINTER(index));
	if (ipdevice == NULL)
...
#ifdef TIZEN_EXT
struct connman_new_address *__connman_ipconfig_get_new_address_from_index(int index)
{
	struct connman_new_address *new_address;
	if (index < 0)
		return NULL;
...
#if defined TIZEN_EXT
int __connman_ipconfig_reset_method(struct connman_ipconfig *ipconfig,
		enum connman_ipconfig_method method)
{
	if (ipconfig == NULL)
		return 0;

	DBG("current method %d new method %d", ipconfig->method, method);
...
#if defined TIZEN_EXT
	/* If service is cellular, telephony will handle setting disable_ipv6 entry in proc */
		...
		GKeyFile *keyfile, const char *identifier, const char *prefix)
{
	...
	if (ipconfig->address->local)
	    DBG("loaded local_address : %s", ipconfig->address->local);
	g_free(key);
...
#if defined TIZEN_EXT
		/* clear the previous ip properties */
		key = g_strdup_printf("%snetmask_prefixlen", prefix);
		g_key_file_remove_key(keyfile, identifier, key, NULL);
		g_free(key);
...
int __connman_ipconfig_init(void)
{
	...
	ipdevice_hash = g_hash_table_new_full(g_direct_hash, g_direct_equal,
							NULL, free_ipdevice);
#if defined TIZEN_EXT
	new_address_hash = g_hash_table_new_full(g_direct_hash, g_direct_equal,
							NULL, free_new_address);
#endif
...
#if defined TIZEN_EXT
	g_hash_table_destroy(new_address_hash);
	new_address_hash = NULL;
#endif
}
