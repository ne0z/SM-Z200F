...
#else
	g_list_free_full (option_value, g_free);
#endif
}
...
#if defined TIZEN_EXT
		/* for roaming case, not need to alert dhcp fail */
		if (dhcp_client->roaming == FALSE)
#endif
							...
							gpointer user_data)
{
	...
	case INIT_SELECTING:
		...
		if (*message_type != DHCPOFFER)
			return TRUE;
#else
		if (message_type != NULL && *message_type != DHCPOFFER)
			return TRUE;
#endif
	...
	case REBINDING:
		...
		if (*message_type == DHCPACK) {
			dhcp_client->retry_times = 0;
#else
		if (message_type != NULL && *message_type == DHCPACK) {
			dhcp_client->retry_times = 0;
#endif
...
#else
		} else if (message_type != NULL && *message_type == DHCPNAK) {
#endif
			dhcp_client->retry_times = 0;
