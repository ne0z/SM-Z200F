...
unsigned int g_supplicant_network_get_rsn_selected(GSupplicantNetwork *network)
{
	if (network == NULL || network->best_bss == NULL)
		return 0;
...
#if defined TIZEN_EXT
	if(bss->psk == TRUE && bss->rsn_keymgmt != 0)
		bss->rsn_selected = TRUE;
	else
		bss->rsn_selected = FALSE;
#if 0
	SUPPLICANT_DBG("bss:[%s] Security:%d, keym:%d rsn:%d(psk:%d,8021x:%d,ft8021x:%d,ftpsk:%d,cckm:%d,wpakm:%d,rsnkm:%d)",
			bss->ssid, bss->security, bss->keymgmt, bss->rsn_selected,
			bss->psk, bss->ieee8021x, bss->ft_ieee8021x, bss->ft_psk, bss->cckm,
			bss->wpa_keymgmt, bss->rsn_keymgmt);
#endif
#endif
}
