...
#define SCAN_EXPIRED_TIME	5000	/* in milliseconds */
#endif

#if defined TIZEN_EXT
#define FAVORITE_MAXIMUM_RETRIES 1
#else
...
static int network_connect(struct connman_network *network)
{
	...
	if (wifi->disconnecting == TRUE) {
		...
		cur_time = g_get_real_time() / 1000;
		if (cur_time - wifi->last_scan_time > SCAN_EXPIRED_TIME){
			throw_wifi_scan(wifi->device, scan_callback);
			wifi->scan_pending_network = wifi->network;

			DBG("Scan results are %d secs older, rescan and try connect",
					(int)((cur_time - wifi->last_scan_time) / 1000));
					...
					struct wifi_data *wifi)
{
	...
	if (++wifi->assoc_retry_count >= TIZEN_ASSOC_RETRY_COUNT) {
		...
		DBG("Network %p assoc_retry_count %d", network, wifi->assoc_retry_count);
...
static void interface_state(GSupplicantInterface *interface)
{
	...
	case G_SUPPLICANT_STATE_COMPLETED:
		...
		connman_bool_t scanning = connman_device_get_scanning(device);
		if (scanning == TRUE){
			connman_device_set_scanning(device, FALSE);
			connman_device_unref(device);
		}
...
#endif /*TIZEN_SUPPORT_WFA_CERTIFICATION*/
#endif
...
static void scan_finished(GSupplicantInterface *interface)
{
#if defined TIZEN_EXT
	struct wifi_data *wifi;
	connman_bool_t is_associating = FALSE;
	static connman_bool_t is_scanning = TRUE;
#endif
	...
	wifi = g_supplicant_interface_get_data(interface);
	if (wifi) {
		wifi->last_scan_time = g_get_real_time() / 1000;

		if (wifi->scan_pending_network) {
			connman_network_set_connecting(wifi->network);
			network_connect(wifi->scan_pending_network);
			wifi->scan_pending_network = NULL;
		}
