...
enum connman_service_error {
	...
	CONNMAN_SERVICE_ERROR_CAPTIVEPORTAL_SIGN_FAILED = 9,
#endif
};
...
connman_bool_t connman_service_is_ims_profile(struct connman_service *cellular);
int connman_del_ims_gateway(int index);
void connman_update_nameservers(int index);
#endif
